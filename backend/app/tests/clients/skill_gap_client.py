...
class SkillGapAPIClient:
	...
	async def analyze_skill_gap(self, user_id: int, job_id: Optional[int] = None, role_title: Optional[str] = None) -> Response:
		"""
		...
		async with httpx.AsyncClient(timeout=self.timeout) as client:
			return await client.get(f"{self.base_url}/skills/gap-analysis", params=params)

	async def get_skill_recommendations(self, user_id: int) -> Response:
		"""Get personalized skill recommendations for a user."""
		async with httpx.AsyncClient(timeout=self.timeout) as client:
			return await client.get(f"{self.base_url}/skills/recommendations", params={"user_id": user_id})

	async def validate_skill_match(self, user_skills: List[str], required_skills: List[str]) -> Response:
		"""Validate skill matching accuracy."""
		async with httpx.AsyncClient(timeout=self.timeout) as client:
			return await client.post(f"{self.base_url}/skills/validate-match", json={"user_skills": user_skills, "required_skills": required_skills})

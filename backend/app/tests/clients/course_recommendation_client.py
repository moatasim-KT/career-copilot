...
class CourseRecommendationAPIClient:
    ...
    async def get_course_recommendations(self, 
        user_id: int,
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            return await client.get(
                f"{self.base_url}/courses/recommendations",
                params=params
            )

    async def validate_course_relevance(self,
        user_id: int,
        course_id: int
    ) -> Response:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            return await client.post(
                f"{self.base_url}/courses/validate-relevance",
                json={
                    "user_id": user_id,
                    "course_id": course_id
                }
    ...
    async def get_course_completion_stats(self,
        course_id: int,
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            return await client.get(
                f"{self.base_url}/courses/{course_id}/stats",

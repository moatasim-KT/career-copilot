...
from app.middleware.metrics_middleware import record_contract_analysis, record_contract_analysis_failure
	...
	async def process_task(self, task_data: dict):
		...
		try:
			...
			await self.redis_client.hset(
				...
				mapping={"status": "completed", "completed_at": datetime.now().isoformat(), "worker_id": self.worker_id, "duration": duration},
			)
			...
			await self.redis_client.hset(
				f"analysis:status:{contract_id}",
				mapping={"status": "failed", "error": str(e), "failed_at": datetime.now().isoformat(), "worker_id": self.worker_id},
			)

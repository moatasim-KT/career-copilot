...
class EnhancedAlertManager:
    ...
    def create_alert(self, message: str, severity: str = "low", context: Dict[str, Any] = None):
        ...
        # Log alert
        logger.warning(
            f"Alert created: {message} (ID: {alert['id']}, Severity: {severity})"
        )
...
class EnhancedMetricsCollector:
    ...
    def get_metric_summary(self, name: str, duration_hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "latest": values[-1],
            "latest_timestamp": recent_points[-1].timestamp.isoformat() if hasattr(recent_points[-1], 'timestamp') else datetime.now(timezone.utc).isoformat(),
        }

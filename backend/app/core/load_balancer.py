...
class LoadBalancer:
    ...
    async def submit_request(self, data: Any, priority: int = 1, 
    ...
    async def _check_scaling_needs(self):
        ...
        if (current_load > self.scale_up_threshold or 
            queue_length > len(self.workers) * 5) and len(self.workers) < self.max_workers:
            await self._scale_up()
        
        # Scale down conditions
        elif (current_load < self.scale_down_threshold and 
              queue_length < len(self.workers) * 2 and 
    ...
    async def _scale_down(self):
        ...
        idle_workers = [w for w in self.workers.values() 
                       if w.status == WorkerStatus.IDLE and w.active_requests == 0]
    ...
    def _select_best_worker(self, request: Request) -> Optional[Worker]:
        ...
        available_workers = [w for w in self.workers.values() 
                           ...
                           and w.active_requests < w.max_requests]
        ...
        best_worker = min(available_workers, key=lambda w: 
                         (w.active_requests / w.max_requests) + 
                         (w.cpu_usage * 0.5) + 
                         (w.memory_usage * 0.5))
    ...
    async def _drain_requests(self):
        ...
        while (self.request_queue or self.priority_queue or 
               any(w.active_requests > 0 for w in self.workers.values())):
            await asyncio.sleep(1)

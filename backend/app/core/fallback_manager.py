...
class FallbackConfig:
    ...
    provider_capabilities: Dict[str, List[str]] = field(default_factory=lambda: {
...
class ProviderHealthMonitor:
    ...
    def should_perform_health_check(self) -> bool:
        ...
        return (datetime.utcnow() - self.last_health_check).total_seconds() > self.config.health_check_interval
...
class FallbackProviderManager:
    ...
    async def execute_with_fallback(
        ...
        **kwargs
        ...
        provider_order = self._get_provider_order(
            task_type, required_capabilities, preferred_provider
        )
        ...
        if self.config.strategy == FallbackStrategy.SEQUENTIAL:
            return await self._execute_sequential(
                provider_order, func_name, args, kwargs, execution_metadata
            )
        ...
        for provider_name in provider_order:
            ...
            try:
                ...
                if hasattr(provider, func_name):
                    ...
                    # Update health
                    self.health_monitor.update_provider_health(
                        provider_name, True, execution_time
                    )
                ...
                # Update health
                self.health_monitor.update_provider_health(
                    provider_name, False, execution_time, str(e)
                )
    ...
    async def _execute_parallel(
        ...
        try:
            ...
            if remaining_providers:
                return await self._execute_sequential(
                    remaining_providers, func_name, args, kwargs, metadata
                )
    ...
    async def _execute_adaptive(
        ...
        return await self._execute_sequential(
            adaptive_order, func_name, args, kwargs, metadata
        )

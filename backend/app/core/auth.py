...
async def get_current_user_optional(
	credentials: Optional[HTTPAuthorizationCredentials] = Depends(security), session: Session = Depends(get_db)
) -> Optional[User]:
	"""
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security), session: Session = Depends(get_db)) -> User:
	"""
...
async def get_current_user_from_credentials(credentials: HTTPAuthorizationCredentials, session: AsyncSession) -> User:
	"""
...
async def APIKey(api_key: str = Depends(HTTPBearer()), session: Session = Depends(get_db)) -> User:
	"""

...
class CircuitBreaker:
	...
	def _should_attempt_reset(self) -> bool:
		...
		return datetime.now(timezone.utc) >= (self.last_failure_time + timedelta(seconds=self.config.recovery_timeout))
	...
	async def execute_with_retry(
		...
		**kwargs,
		...
		if circuit_breaker_name:
			circuit_breaker = self.get_or_create_circuit_breaker(circuit_breaker_name, circuit_breaker_config)
		...
		for attempt in range(retry_config.max_attempts):
			...
			try:
				...
				attempts.append(
					RetryAttempt(
						attempt_number=attempt + 1, delay=0.0, exception=None, timestamp=datetime.now(timezone.utc), correlation_id=correlation_id
					)
				)
				...
				if not self._is_retryable_exception(e, retry_config):
					attempts.append(
						RetryAttempt(
							attempt_number=attempt + 1, delay=0.0, exception=e, timestamp=datetime.now(timezone.utc), correlation_id=correlation_id
						)
					)
				...
				if attempt < retry_config.max_attempts - 1:
					...
					attempts.append(
						RetryAttempt(
							attempt_number=attempt + 1, delay=delay, exception=e, timestamp=datetime.now(timezone.utc), correlation_id=correlation_id
						)
					)

					logger.warning(f"Attempt {attempt + 1} failed (correlation_id: {correlation_id}): {e}. Retrying in {delay:.2f}s")

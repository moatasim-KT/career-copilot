...
class TaskComplexityAnalyzer:
	...
	def analyze_task_complexity(self, prompt: str, context: Optional[str] = None, task_type: Optional[str] = None) -> TaskComplexity:
		"""
		...
		logger.info(f"Task complexity analysis: score={complexity_score:.3f}, level={complexity.value}, factors={factors}")

		return complexity

	def _extract_complexity_factors(self, prompt: str, context: Optional[str] = None, task_type: Optional[str] = None) -> ComplexityFactors:
		...
		technical_terms = sum(1 for term in self.technical_terms if term in full_text_lower)

		# Count legal concepts
		legal_concepts = sum(1 for concept in self.legal_concepts if concept in full_text_lower)

		# Analysis depth indicators
		analysis_depth = sum(1 for keyword in self.analysis_keywords if keyword in full_text_lower)

		# Reasoning requirements
		reasoning_required = sum(1 for keyword in self.reasoning_keywords if keyword in full_text_lower)

		# JSON output requirement
		json_output = any(keyword in full_text_lower for keyword in ["json", "structured", "format", "schema", "object", "array"])

		# Multi-step task indicators
		multi_step = any(
			keyword in full_text_lower
			for keyword in ["first", "then", "next", "finally", "step", "phase", "stage", "subsequently", "afterwards", "following", "proceed"]
		)

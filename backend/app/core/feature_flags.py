			...
			with self.lock:
				...
				for flag_name, flag_data in data.get("flags", {}).items():
					...
					flag = FeatureFlag(
						...
						created_at=datetime.fromisoformat(flag_data["created_at"]) if flag_data.get("created_at") else datetime.now(timezone.utc),
						updated_at=datetime.fromisoformat(flag_data["updated_at"]) if flag_data.get("updated_at") else datetime.now(timezone.utc),
...
class FeatureFlagManager:
	...
	async def _evaluate_rollout(self, flag: FeatureFlag, user_context: Optional[UserContext] = None) -> bool:
		...
		if rollout.strategy == RolloutStrategy.USER_ATTRIBUTE:
			if (
				rollout.user_attribute
				and rollout.attribute_values
				and user_context.attributes.get(rollout.user_attribute) in rollout.attribute_values
			):
				return True
			return False
	...
	def _get_cache_key(self, flag_name: str, user_context: Optional[UserContext] = None) -> str:
		...
		context_parts = [user_context.user_id or "anonymous", user_context.role or "default", user_context.environment or self.environment]
	...
	async def create_flag(
		...
		flag = FeatureFlag(name=name, description=description, state=state, default_value=default_value, tags=tags or [], created_by=created_by)
...
async def is_feature_enabled(flag_name: str, user_context: Optional[UserContext] = None, default: bool = False) -> bool:
	"""Check if a feature flag is enabled."""
	manager = get_feature_flag_manager()
	return await manager.is_enabled(flag_name, user_context, default)


async def get_user_context(user_id: Optional[str] = None, email: Optional[str] = None, role: Optional[str] = None, **attributes) -> UserContext:
	"""Create a user context for feature flag evaluation."""
	return UserContext(user_id=user_id, email=email, role=role, attributes=attributes)

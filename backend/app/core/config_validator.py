...
ANY_INTERFACE_HOST = str(IPv4Address(0))
...
class ConfigurationValidator:
	...
	def _validate_api_configuration(self):
		...
		allowed_hosts = {ANY_INTERFACE_HOST, "127.0.0.1", "localhost"}
		if api_host and api_host not in allowed_hosts:
			self._add_issue(
				...
				f"Use '{ANY_INTERFACE_HOST}' for all interfaces or a valid IP/hostname",
			)
		...
		if api_port:
			...
			if api_port < 1024:
				self._add_issue(
					"warning", "security", "API_PORT", f"API port {api_port} requires root privileges", "Use a port >= 1024 for non-root deployment"
				)
			elif api_port > 65535:
		...
		if not database_url:
			...
			try:
				...
				if parsed.scheme not in ["postgresql", "postgresql+asyncpg", "mysql"]:
					self._add_issue(
						"warning", "invalid", "DATABASE_URL", f"Unsupported database scheme: {parsed.scheme}", "Use sqlite, postgresql, or mysql"
					)
			except Exception as e:
				self._add_issue(
					"error", "invalid", "DATABASE_URL", f"Invalid database URL format: {e}", "Example: postgresql+asyncpg://user:pass@host:port/db"
				)
	...
	def _validate_security_settings(self):
		...
		if jwt_secret:
			if jwt_secret == "your-secret-key-change-in-production":
				self._add_issue(
					"error", "security", "JWT_SECRET_KEY", "Default JWT secret key is being used", "Generate a secure random key for production"
				)

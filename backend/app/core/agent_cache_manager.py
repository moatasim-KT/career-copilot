...
class TimeoutConfig:
	...
	def get_timeout_for_execution(self, retry_count: int = 0, execution_history: List[float] | None = None) -> int:
...
class AgentCacheManager:
	...
	def __init__(self):
		...
		# Statistics
		self.cache_stats = {"hits": 0, "misses": 0, "timeouts": 0, "evictions": 0, "total_requests": 0}
	...
	def _generate_input_hash(self, task_input: Dict[str, Any]) -> str:
		...
		cache_input = {k: v for k, v in task_input.items() if k not in ["workflow_id", "execution_id", "timestamp"]}
	...
	async def get_cached_result(self, agent_type: AgentType, task_input: Dict[str, Any]) -> Optional[Dict[str, Any]]:
	...
	async def cache_result(
		self, agent_type: AgentType, task_input: Dict[str, Any], result: Dict[str, Any], execution_time: float, metadata: Dict[str, Any] | None = None
	) -> bool:
	...
	async def _check_and_evict_entries(self, agent_type: AgentType):
		...
		agent_entries = [(key, entry) for key, entry in self.cache_entries.items() if entry.agent_type == agent_type]
	...
	def get_timeout_for_agent(self, agent_type: AgentType, retry_count: int = 0) -> int:
	...
	async def execute_with_timeout(self, agent_type: AgentType, coro, retry_count: int = 0) -> Dict[str, Any]:
	...
	async def invalidate_cache(self, agent_type: Optional[AgentType] = None, pattern: Optional[str] = None) -> int:
		...
		try:
			if agent_type:
				...
				keys_to_remove = [key for key, entry in self.cache_entries.items() if entry.agent_type == agent_type]
				...
				keys_to_remove = [key for key in self.cache_entries.keys() if fnmatch.fnmatch(key, pattern)]
	...
	def get_cache_statistics(self) -> Dict[str, Any]:
		...
		for agent_type in AgentType:
			agent_entries = [entry for entry in self.cache_entries.values() if entry.agent_type == agent_type]
			...
			agent_stats[agent_type.value] = {
				...
				"avg_execution_time": (sum(entry.execution_time for entry in agent_entries) / len(agent_entries) if agent_entries else 0.0),
	...
	def update_cache_config(self, agent_type: AgentType, config_updates: Dict[str, Any]) -> bool:
	...
	def update_timeout_config(self, agent_type: AgentType, config_updates: Dict[str, Any]) -> bool:

...
class ServicePlugin(ABC):
	...
	def __init__(self, config: ServiceConfig):
		...
		self._health = ServiceHealth(status=ServiceStatus.UNKNOWN, last_check=datetime.now(), response_time=0.0)
		self._start_time = None
		self._callbacks: Dict[str, List[Callable]] = {"on_start": [], "on_stop": [], "on_health_change": [], "on_error": []}
	...
	async def _update_health(
		self, status: ServiceStatus, response_time: float = 0.0, error_message: Optional[str] = None, details: Optional[Dict] = None
	):
		"""Update health status."""
		old_status = self._health.status

		self._health = ServiceHealth(
			status=status, last_check=datetime.now(), response_time=response_time, error_message=error_message, details=details or {}
		)
...
class ServiceRegistry:
		...
		async with self._lock:
			...
			try:
				...
				if service.config.health_check_interval > 0:
					self._health_check_tasks[service_id] = asyncio.create_task(self._health_monitor(service))
	...
	def list_services(self, service_type: Optional[ServiceType] = None, status: Optional[ServiceStatus] = None) -> List[ServicePlugin]:
	...
	async def _health_monitor(self, service: ServicePlugin):
		...
		while True:
			try:
				...
				await service._update_health(health.status, response_time, health.error_message, health.details)

			except asyncio.CancelledError:
				break
			except Exception as e:
				logger.error(f"Health check error for {service.service_id}: {e}")
				await service._update_health(ServiceStatus.ERROR, error_message=str(e))
...
class ServiceLoadBalancer:
	...
	async def get_service_instance(self, service_type: ServiceType, strategy: str = "health_based") -> Optional[ServicePlugin]:

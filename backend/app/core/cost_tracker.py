...
class CostTracker:
	...
	def _load_default_budgets(self):
		...
		self.budget_limits = [
			# Global daily limit
			BudgetLimit(category=None, period=BudgetPeriod.DAILY, limit=Decimal("50.00"), alert_threshold=0.8, hard_limit=True),
			# Per-category daily limits
			BudgetLimit(
				category=CostCategory.CONTRACT_ANALYSIS, period=BudgetPeriod.DAILY, limit=Decimal("30.00"), alert_threshold=0.8, hard_limit=True
			),
			BudgetLimit(category=CostCategory.NEGOTIATION, period=BudgetPeriod.DAILY, limit=Decimal("10.00"), alert_threshold=0.8, hard_limit=False),
			BudgetLimit(category=CostCategory.COMMUNICATION, period=BudgetPeriod.DAILY, limit=Decimal("5.00"), alert_threshold=0.8, hard_limit=False),
			# Monthly limits
			BudgetLimit(category=None, period=BudgetPeriod.MONTHLY, limit=Decimal("1000.00"), alert_threshold=0.9, hard_limit=True),
		]
	...
	async def record_cost(
		...
		logger.info(f"Recorded cost: {cost_entry.provider}:{cost_entry.model} ${cost_entry.cost} ({cost_entry.total_tokens} tokens)")
	...
	async def check_budget_limits(self, category: CostCategory, estimated_cost: float, user_id: Optional[str] = None) -> List[BudgetStatus]:
		"""
	...
	async def _get_budget_status(self, budget_limit: BudgetLimit) -> BudgetStatus:
		...
		current_spend = await self._calculate_period_spend(budget_limit, period_start, period_end)
	...
	async def _calculate_period_spend(self, budget_limit: BudgetLimit, period_start: datetime, period_end: datetime) -> Decimal:
	...
	async def get_cost_summary(
		...
		for entry in filtered_entries:
			if entry.provider not in by_provider:
				by_provider[entry.provider] = {"cost": Decimal("0.00"), "tokens": 0, "requests": 0}
		...
		for entry in filtered_entries:
			...
			if cat not in by_category:
				by_category[cat] = {"cost": Decimal("0.00"), "tokens": 0, "requests": 0}
	...
	def remove_budget_limit(self, category: Optional[CostCategory], period: BudgetPeriod, user_id: Optional[str] = None):
		...
		self.budget_limits = [
			limit for limit in self.budget_limits if not (limit.category == category and limit.period == period and limit.user_id == user_id)
		]

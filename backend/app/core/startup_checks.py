...
class StartupValidator:
    ...
    async def check_database(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            all_tables_exist = all(
                status["exists"] for status in table_status.values()
            )
            ...
            passed = (
                health_status.get("database", False) and 
                details.get("basic_query", False) and
                all_tables_exist and
                migrations_up_to_date and
    ...
    async def check_file_system(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            passed = (
                details["data_dir_writable"] and
                details["logs_dir_writable"]
            )

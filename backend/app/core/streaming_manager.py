...
class StreamingCallbackHandler(AsyncCallbackHandler):
	...
	async def on_llm_new_token(self, token: str, **kwargs) -> None:
		...
		if self.session.mode == StreamingMode.REAL_TIME:
			...
			should_emit = len(self.buffer) >= self.session.buffer_size or (current_time - self.last_chunk_time) >= self.session.chunk_delay
...
class StreamingManager:
	...
	def create_streaming_session(
		self, provider: str, model: str, operation: str, mode: StreamingMode = StreamingMode.BUFFERED, buffer_size: int = 1024
	) -> StreamingSession:
		"""Create a new streaming session."""
		session_id = str(uuid.uuid4())

		session = StreamingSession(
			session_id=session_id, provider=provider, model=model, operation=operation, mode=mode, start_time=time.time(), buffer_size=buffer_size
		)

		self.active_sessions[session_id] = session

		# Record session start in performance metrics
		self.performance_collector.record_request_start(request_id=session_id, provider=provider, model=model, operation=operation, is_streaming=True)
	...
	def get_streaming_callback(self, session: StreamingSession) -> StreamingCallbackHandler:
		...
		def chunk_handler(chunk: StreamingChunk):
			...
			self.performance_collector.record_streaming_chunk(
				request_id=session.session_id, chunk_size=len(chunk.content), chunk_content=chunk.content, is_first_chunk=(chunk.sequence_number == 0)
			)
	...
	async def stream_llm_response(self, llm, messages: List[BaseMessage], session: StreamingSession) -> AsyncGenerator[StreamingChunk, None]:
	...
	async def stream_with_optimization(
		self, llm, messages: List[BaseMessage], session: StreamingSession, optimization_config: Optional[Dict[str, Any]] = None
	...
	def _calculate_optimal_buffer_size(self, messages: List[BaseMessage], session: StreamingSession) -> int:
	...
	def _predict_response_length(self, messages: List[BaseMessage], session: StreamingSession) -> int:
		"""Predict approximate response length in tokens."""
		# Simple heuristic based on input length and operation type
		input_tokens = sum(len(msg.content.split()) for msg in messages)

		multipliers = {"contract_analysis": 2.0, "legal_precedent": 1.5, "negotiation": 1.2, "communication": 0.8, "general": 1.0}
	...
	async def _finalize_session(self, session: StreamingSession, success: bool):
		...
		self.performance_collector.record_request_completion(
			request_context=request_context, success=success, token_usage=token_usage, cost=estimated_cost
		)
		...
		logger.info(
			f"Finalized streaming session {session.session_id}: "
	...
	def get_streaming_performance_summary(self) -> Dict[str, Any]:
		...
		summary = {"active_sessions": active_count, "session_details": self.get_active_sessions(), "performance_metrics": {}}
		...
		for session in self.active_sessions.values():
			...
			if key not in summary["performance_metrics"]:
				streaming_metrics = self.performance_collector.get_streaming_metrics(session.provider, session.model, session.operation)
	...
	async def cleanup_stale_sessions(self, max_age_seconds: int = 3600):
		...
		current_time = time.time()
		stale_sessions = [session_id for session_id, session in self.active_sessions.items() if (current_time - session.start_time) > max_age_seconds]

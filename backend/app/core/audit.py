...
class AuditEventType(str, Enum):
	...
	# deepcode ignore NoHardcodedPasswords: This is an event type constant, not a password
...
class AuditEvent(BaseModel):
	...
	model_config = ConfigDict(
		json_schema_extra={
			"examples": [
				{"timestamp": "2024-01-01T00:00:00", "event_type": "LOGIN_SUCCESS", "severity": "low", "action": "User login", "result": "success"}
			]
		}
	)
...
class AuditLogger:
	...
	def log_file_upload(
		self, request: Request, filename: str, file_size: int, result: str = "success", details: Optional[Dict[str, Any]] = None
	) -> None:
		"""Log file upload event."""
		event_details = {"filename": filename, "file_size_bytes": file_size, "file_size_mb": round(file_size / (1024 * 1024), 2)}
		if details:
	...
	def log_security_event(
		...
		self.log_request(request=request, event_type=event_type, action=action, result="security_violation", severity=severity, details=details)

	def log_system_event(self, event_type: AuditEventType, action: str, result: str = "success", details: Optional[Dict[str, Any]] = None) -> None:
		"""Log system-level event."""
		severity = AuditSeverity.MEDIUM if event_type in [AuditEventType.SYSTEM_START, AuditEventType.SYSTEM_SHUTDOWN] else AuditSeverity.LOW

		self.log_event(event_type=event_type, action=action, result=result, severity=severity, details=details)
...
class SecurityMonitor:
	...
	def check_rate_limit(self, ip_address: str, request: Request) -> bool:
		...
		if ip_address in self.failed_attempts:
			self.failed_attempts[ip_address] = [
				timestamp for timestamp in self.failed_attempts[ip_address] if current_time - timestamp < self.rate_limit_window
			]
	...
	def record_failed_attempt(self, ip_address: str, request: Request, reason: str) -> None:
		...
		# Clean old entries
		self.failed_attempts[ip_address] = [
			timestamp for timestamp in self.failed_attempts[ip_address] if current_time - timestamp < self.failed_attempt_window
		]
		...
		if len(self.failed_attempts[ip_address]) >= self.max_failed_attempts:
			...
			self.audit_logger.log_security_event(
				...
				details={"reason": reason, "attempt_count": len(self.failed_attempts[ip_address]), "threshold": self.max_failed_attempts},

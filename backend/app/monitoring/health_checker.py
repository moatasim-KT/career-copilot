...
class HealthChecker:
	...
	async def check_ai_service_health(self) -> ComponentHealth:
		...
		try:
			...
			unhealthy_services = [provider for provider, health in service_health.items() if health["state"] == "open"]
		...
		try:
			async with httpx.AsyncClient() as client:
				response = await client.get(
					"https://api.openai.com/v1/models", headers={"Authorization": f"Bearer {settings.openai_api_key.get_secret_value()}"}, timeout=10
				)

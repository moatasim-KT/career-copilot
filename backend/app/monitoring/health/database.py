...
class DatabaseHealthMonitor(BaseHealthChecker):
	...
	async def _check_postgresql(self) -> Dict[str, Any]:
		...
		try:
			...
			pool_usage = used_connections / total_connections if total_connections > 0 else 0
	...
	async def _check_chromadb(self) -> Dict[str, Any]:
		...
		try:
			...
			test_collection = client.create_collection(name=f"health_check_{int(time.time())}")
		...
		try:
			...
			async with engine.connect() as conn:
				...
				for query_name, query in test_queries:
					...
					if query_time > self.performance_thresholds["query_time_critical_ms"]:
						performance_metrics["critical"].append(f"{query_name}: {query_time:.1f}ms")
					elif query_time > self.performance_thresholds["query_time_warning_ms"]:
						performance_metrics["warnings"].append(f"{query_name}: {query_time:.1f}ms")

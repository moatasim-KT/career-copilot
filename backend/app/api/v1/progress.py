...
async def get_task_progress(task_id: str, current_user: User = Depends(get_current_user)):
...
async def get_task_list(
	...
	return TaskListResponse(tasks=paginated_tasks, total_count=total_count, active_count=active_count, completed_count=completed_count)
...
async def get_task_history(task_id: str, current_user: User = Depends(get_current_user)):
	...
	return {
		...
		"stage_durations": {stage.value: duration for stage, duration in task_progress.stage_durations.items()},
		"history": history,
	}
...
async def get_performance_analytics(
	days: int = Query(7, ge=1, le=30, description="Number of days to analyze"), current_user: User = Depends(get_current_user)
):

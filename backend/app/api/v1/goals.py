...
def create_goal(goal_data: GoalCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
	"""Create a new goal"""
...
def get_goal_dashboard(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
def get_progress_celebration(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
def get_goal(goal_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
def update_goal(goal_id: int, goal_update: GoalUpdate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
def add_goal_progress(goal_id: int, progress_data: GoalProgressCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
def delete_goal(goal_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
def create_milestone(milestone_data: MilestoneCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
def get_recent_milestones(
	days: int = Query(30, description="Number of days to look back"), current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
...
def auto_track_application_progress(application_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):

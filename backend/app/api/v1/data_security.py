...
async def migrate_to_encryption(
    ...
    if not settings.ENCRYPT_USER_DATA and not settings.ENCRYPT_FILES:
        raise HTTPException(
            status_code=400,
            detail="Encryption is not enabled in settings"
        )
...
async def migrate_to_compression(
    ...
    if not settings.ENABLE_COMPRESSION:
        raise HTTPException(
            status_code=400,
            detail="Compression is not enabled in settings"
        )
...
async def test_encryption(
    test_data: str = "Hello, World! This is a test of the encryption system.",
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
...
async def test_compression(
    test_data: str = "This is a test string for compression. " * 100,
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Compression test failed: {str(e)}"
        )
...
def _get_most_compressible_file_types(db: Session) -> list:
    ...
    return [
        {
            "mime_type": row[0],
            "average_compression_ratio": round(row[1], 4),
            "file_count": row[2]
        }
        for row in results
    ]
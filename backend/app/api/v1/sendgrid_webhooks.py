...
from ...services.email_service import EmailEventType, EmailProvider, EmailService
...
def verify_webhook_signature(payload: bytes, signature: str, public_key: str) -> bool:
	...
	try:
		...
		expected_signature = base64.b64encode(hmac.new(public_key.encode(), payload, hashlib.sha256).digest()).decode()
...
async def handle_sendgrid_webhook(
	...
	try:
		...
		if webhook_public_key and x_twilio_email_event_webhook_signature:
			if not verify_webhook_signature(payload, x_twilio_email_event_webhook_signature, webhook_public_key):
		...
		return {"status": "success", "processed_events": len(processed_events), "event_ids": processed_events}
...
async def process_sendgrid_event(event: SendGridEvent, email_service: EmailService):
	...
	event_status_mapping = {
		...
		"dropped": EmailEventType.BOUNCE,
		"spam_report": EmailEventType.SPAM_REPORT,
		"unsubscribe": EmailEventType.UNSUBSCRIBE,
		"blocked": EmailEventType.BOUNCE,
		"deferred": EmailEventType.BOUNCE,
	}

	event_type = event_status_mapping.get(event.event)
	if not event_type:
	...
	try:
		await email_service.email_analytics_service.record_email_event(
		...
		logger.info(f"Recorded email event for {tracking_id}: {event_type}")

	except Exception as e:
		logger.error(f"Failed to record email event for {tracking_id}: {e}")
		raise
...
async def test_webhook_endpoint():
	...
	return {"status": "ok", "message": "SendGrid webhook endpoint is accessible", "timestamp": datetime.now().isoformat()}


@router.post("/test-event")
async def test_webhook_event(test_event: Dict[str, Any], email_service: EmailService = Depends(get_email_service)):
	...
	try:
		...
		await process_sendgrid_event(event, email_service)

		return {"status": "success", "message": "Test event processed successfully", "event_id": event.sg_event_id}
...
async def webhook_health_check():
	...
	return {"status": "healthy", "service": "sendgrid_webhooks", "timestamp": datetime.now().isoformat()}

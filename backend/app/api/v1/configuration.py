...
from ...core.feature_flags import FeatureState, UserContext, get_feature_flag_manager
...
async def get_configuration_status():
	...
	try:
		...
		if not status:
			raise HTTPException(status_code=503, detail="Configuration system not initialized")
...
async def get_configuration_summary():
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get configuration summary: {e!s}")
...
async def reload_configuration():
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Configuration reload failed: {e!s}")
...
async def get_reload_history(limit: int = Query(20, ge=1, le=100)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get reload history: {e!s}")
...
async def validate_configuration():
	...
	try:
		...
		if not config_manager.config_data:
			raise HTTPException(status_code=503, detail="No configuration data available")
		...
		raise HTTPException(status_code=500, detail=f"Configuration validation failed: {e!s}")
...
async def list_feature_flags(tags: Optional[List[str]] = Query(None)):
	...
	try:
		...
		return FeatureFlagListResponse(
			flags=flags, total_count=len(flags), enabled_count=enabled_count, disabled_count=disabled_count, rollout_count=rollout_count
		)

	except Exception as e:
		logger.error(f"Failed to list feature flags: {e}")
		raise HTTPException(status_code=500, detail=f"Failed to list feature flags: {e!s}")
...
async def get_feature_flag(flag_name: str):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get feature flag: {e!s}")
...
async def check_feature_flag(flag_name: str, request: FeatureFlagCheckRequest):
	...
	try:
		...
		# Create user context
		user_context = UserContext(user_id=request.user_id, email=request.email, role=request.role, attributes=request.attributes)
		...
		raise HTTPException(status_code=500, detail=f"Failed to check feature flag: {e!s}")
...
async def update_feature_flag_state(flag_name: str, request: FeatureFlagUpdateRequest):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to update feature flag: {e!s}")


@router.post("/feature-flags/{flag_name}/rollout")
async def set_feature_flag_rollout(flag_name: str, percentage: float = Body(..., ge=0, le=100, description="Rollout percentage (0-100)")):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to set rollout: {e!s}")
...
async def clear_feature_flags_cache():
	...
	try:
		...
		return {"success": True, "message": "Feature flags cache cleared", "timestamp": datetime.now(timezone.utc).isoformat()}
...
async def rollback_configuration(snapshot_index: int = Body(-1, description="Snapshot index (-1 for latest)")):
	...
	try:
		...
		if not success:
			raise HTTPException(status_code=400, detail="Configuration rollback failed")
...
async def get_environment_info():
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get environment info: {e!s}")

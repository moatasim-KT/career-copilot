...
async def validate_services(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to initiate validation: {str(e)}")
    ...
    try:
        ...
        services_config = {
            "docusign": {
                ...
                "configured": bool(
                    getattr(settings, "docusign_client_id", "") or 
                    getattr(settings, "docusign_sandbox_client_id", "")
                )
            },
            "slack": {
            ...
            "gmail": {
                "enabled": getattr(settings, "gmail_enabled", False),
                "configured": bool(getattr(settings, "gmail_client_id", ""))
            },
            "google_drive": {
...
async def test_service(
    ...
    try:
        ...
        if service_name not in test_functions:
            raise HTTPException(
                status_code=400, 
                detail=f"Unknown service '{service_name}'. Available services: {list(test_functions.keys())}"
            )
    ...
    try:
        ...
        if not service.enabled:
            return {
                ...
                "message": "DocuSign is not enabled",
        ...
        return {
            "status": "error",
            "message": f"DocuSign test failed: {str(e)}",
            "timestamp": "2024-01-01T00:00:00Z"
        }
...
async def _test_gmail() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Gmail test failed: {str(e)}",
            "timestamp": "2024-01-01T00:00:00Z"
        }

...
from app.schemas.feedback import (
    FeedbackCreate, FeedbackUpdate, FeedbackResponse, FeedbackVoteCreate,
    OnboardingProgressUpdate, OnboardingProgressResponse,
    HelpArticleCreate, HelpArticleUpdate, HelpArticleResponse, HelpArticleSummary,
    HelpArticleVoteCreate, HelpSearchRequest, HelpSearchResponse,
    Tutorial, FeatureHighlight
)
...
async def get_user_feedback(
    ...
    feedback_service = FeedbackService(db)
    feedback_items = feedback_service.get_user_feedback(
...
async def vote_on_feedback(
    ...
    if not success:
        raise HTTPException(
            ...
            detail="Failed to vote on feedback"
        )
...
async def search_help_articles(
    search_request: HelpSearchRequest,
    db: Session = Depends(get_db)
):
...
async def get_feature_highlights(
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):

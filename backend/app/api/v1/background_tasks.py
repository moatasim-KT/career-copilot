...
async def schedule_task(request: TaskScheduleRequest, current_user: Optional[Dict] = Depends(get_current_user_optional)):
	...
	try:
		...
		return {"task_id": task_id, "status": "scheduled", "message": f"Task {task_id} has been scheduled for execution"}

	except Exception as e:
		logger.error(f"Failed to schedule task: {e}", exc_info=True)
		raise HTTPException(status_code=500, detail=f"Failed to schedule task: {e!s}")


@router.post("/contract-analysis", response_model=Dict[str, str])
async def schedule_contract_analysis(request: ContractAnalysisTaskRequest, current_user: Optional[Dict] = Depends(get_current_user_optional)):
	...
	try:
		...
		return {"task_id": task_id, "status": "scheduled", "message": f"Contract analysis task {task_id} has been scheduled"}

	except Exception as e:
		logger.error(f"Failed to schedule job application tracking task: {e}", exc_info=True)
		raise HTTPException(status_code=500, detail=f"Failed to schedule job application tracking: {e!s}")


@router.get("/status/{task_id}", response_model=TaskStatusResponse)
async def get_task_status(task_id: str, current_user: Optional[Dict] = Depends(get_current_user_optional)):
...
async def get_task_result(task_id: str, current_user: Optional[Dict] = Depends(get_current_user_optional)):
	"""Get task result."""
	try:
		task_queue_service = get_task_queue_service()
...
async def cancel_task(
	task_id: str, request: TaskCancellationRequest = TaskCancellationRequest(), current_user: Optional[Dict] = Depends(get_current_user_optional)
):
	...
	try:
		...
		cancelled = await task_queue_service.cancel_task(task_id=task_id, reason=request.reason, force=getattr(request, "force", False))
		...
		if not cancelled:
			...
			if result:
				if result.status == TaskStatus.RUNNING:
					raise HTTPException(status_code=400, detail="Task is currently running. Use force=true to request cancellation.")
				elif result.status in [TaskStatus.COMPLETED, TaskStatus.CANCELLED]:
					raise HTTPException(status_code=400, detail=f"Task is already {result.status.value}")
...
async def list_tasks(
	...
	try:
		...
		return TaskListResponse(tasks=task_responses, total_count=total_count, page=page, page_size=page_size, has_next=end_idx < total_count)
...
async def get_queue_stats(current_user: Optional[Dict] = Depends(get_current_user_optional)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get queue stats: {e!s}")


@router.get("/retry-history/{task_id}")
async def get_task_retry_history(task_id: str, current_user: Optional[Dict] = Depends(get_current_user_optional)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get retry history: {e!s}")


@router.post("/retry/{task_id}")
async def retry_task_manually(task_id: str, current_user: Optional[Dict] = Depends(get_current_user_optional)):
	...
	try:
		...
		if result.status not in [TaskStatus.FAILED, TaskStatus.TIMEOUT]:
			raise HTTPException(status_code=400, detail=f"Task cannot be retried (current status: {result.status.value})")

		if result.retry_count >= result.max_retries:
			raise HTTPException(status_code=400, detail=f"Task has exceeded maximum retries ({result.max_retries})")
		...
		result.next_retry_at = datetime.now(timezone.utc)  # Immediate retry
		...
		raise HTTPException(status_code=500, detail=f"Failed to retry task: {e!s}")


@router.get("/monitoring/metrics")
async def get_task_monitoring_metrics(current_user: Optional[Dict] = Depends(get_current_user_optional)):
	...
	try:
		...
		total_queued = stats.urgent_queue_size + stats.high_queue_size + stats.normal_queue_size + stats.low_queue_size

		# Get recent task trends (last hour)
		recent_tasks = []
		for result in task_queue_service.task_storage.results.values():
			if result.created_at and result.created_at > datetime.now(timezone.utc) - datetime.timedelta(hours=1):
...
async def get_task_queue_health():
	...
	try:
		...
		return {"status": "unhealthy", "service": "task_queue", "error": str(e), "timestamp": datetime.now(timezone.utc).isoformat()}

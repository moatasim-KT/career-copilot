...
from ...services.linkedin_scraper import linkedin_scraper
...
async def scrape_linkedin_jobs(request: LinkedInJobSearchRequest) -> List[Dict[str, Any]]:
    ...
    try:
        jobs = await linkedin_scraper.scrape_jobs(request.keywords, request.location)
        return jobs
...
async def get_linkedin_job_description(request: LinkedInJobDetailRequest) -> Dict[str, str]:
    ...
    try:
        description = await linkedin_scraper.scrape_job_description(request.job_url)

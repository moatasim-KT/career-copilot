...
async def configure_slack(config: SlackConfigurationRequest):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Configuration failed: {e!s}")
...
async def send_message(message_request: SlackMessageRequest):
	...
	try:
		...
		# Track analytics
		await analytics.track_slack_event(
			SlackEvent(
		...
		return {"success": True, "message_ts": result["message_ts"], "channel": result["channel"], "permalink": result.get("permalink")}
	...
	try:
		...
		await analytics.track_slack_event(
			SlackEvent(
				...
				metadata={"type": "contract_notification", "risk_score": notification.risk_score, "contract_name": notification.contract_name},
			)
		)

		return {"success": True, "message": "Contract notification sent successfully", "result": result}
...
async def send_risk_alert(channel: str, contract_name: str, risk_score: float, urgent_clauses: List[str], alert_level: str = "HIGH"):
	...
	try:
		...
		result = await service.send_risk_alert(
			channel=channel, contract_name=contract_name, risk_score=risk_score, urgent_clauses=urgent_clauses, alert_level=alert_level
		)
		...
		await analytics.track_slack_event(
			SlackEvent(
				...
				timestamp=datetime.now(),
				channel_id=channel,
				metadata={"type": "risk_alert", "alert_level": alert_level, "risk_score": risk_score, "contract_name": contract_name},
			)
		)

		return {"success": True, "message": "Risk alert sent successfully", "result": result}

	except Exception as e:
		logger.error(f"Error sending risk alert: {e}")
		raise HTTPException(status_code=500, detail=f"Failed to send alert: {e!s}")
	...
	try:
		...
		await analytics.track_slack_event(
			SlackEvent(
				...
				metadata={"action_id": payload.get("actions", [{}])[0].get("action_id"), "callback_id": payload.get("callback_id")},
			)
		)
	...
	try:
		...
		await analytics.track_slack_event(
			SlackEvent(
				...
				metadata={"command": form_data.get("command", "").lstrip("/"), "text": form_data.get("text", "")},
			)
		)
		...
		return {"response_type": "ephemeral", "text": f"❌ Command processing failed: {e!s}"}
...
async def get_user_analytics(user_id: str):
	...
	try:
		...
		return {"success": True, "user_analytics": user_data}

	except Exception as e:
		logger.error(f"Error getting user analytics: {e}")
		raise HTTPException(status_code=500, detail=f"Failed to get user analytics: {e!s}")
...
async def get_channel_analytics(channel_id: str):
	...
	try:
		...
		return {"success": True, "channel_analytics": channel_data}

	except Exception as e:
		logger.error(f"Error getting channel analytics: {e}")
		raise HTTPException(status_code=500, detail=f"Failed to get channel analytics: {e!s}")
...
async def get_command_analytics():
	...
	try:
		...
		return {"success": True, "command_analytics": command_data, "command_statistics": command_stats}

	except Exception as e:
		logger.error(f"Error getting command analytics: {e}")
		raise HTTPException(status_code=500, detail=f"Failed to get command analytics: {e!s}")


@router.post("/analytics/report")
async def generate_analytics_report(start_date: datetime, end_date: datetime, include_details: bool = False):
	"""Generate comprehensive analytics report"""
	try:
		analytics = await get_analytics_service()
		report = await analytics.generate_analytics_report(start_date, end_date, include_details)
...
async def get_slack_health():
	...
	try:
		...
		# Overall health
		all_healthy = all(service_health.get("healthy", False) for service_health in health_data.values())

		return {"healthy": all_healthy, "services": health_data, "timestamp": datetime.now().isoformat()}
...
async def upload_file_to_slack(
	channel: str, file_path: str, filename: str, title: Optional[str] = None, initial_comment: Optional[str] = None, thread_ts: Optional[str] = None
):
	...
	try:
		...
		result = await service.upload_file_to_channel(
			channel=channel, file_path=file_path, filename=filename, title=title, initial_comment=initial_comment, thread_ts=thread_ts
		)
		...
		await analytics.track_slack_event(
			SlackEvent(
				...
				timestamp=datetime.now(),
				channel_id=channel,
				metadata={"type": "file_upload", "filename": filename, "file_id": result.get("file_id")},
			)
		)

		return {"success": True, "message": "File uploaded successfully", "result": result}
...
async def upload_contract_to_slack(
	channel: str, contract_file_path: str, contract_name: str, analysis_summary: Optional[str] = None, thread_ts: Optional[str] = None
):
	...
	try:
		...
		await analytics.track_slack_event(
			SlackEvent(
				...
				timestamp=datetime.now(),
				channel_id=channel,
				metadata={"type": "contract_upload", "contract_name": contract_name, "file_id": result.get("file_id")},
			)
		)

		return {"success": True, "message": "Contract uploaded successfully", "result": result}

	except Exception as e:
		logger.error(f"Error uploading contract to Slack: {e}")
		raise HTTPException(status_code=500, detail=f"Contract upload failed: {e!s}")


@router.post("/create-approval-workflow")
async def create_approval_workflow(
	channel: str, contract_name: str, approvers: List[str], approval_deadline: Optional[datetime] = None, thread_ts: Optional[str] = None
):
	...
	try:
		...
		result = await service.create_approval_workflow(
			channel=channel, contract_name=contract_name, approvers=approvers, approval_deadline=approval_deadline, thread_ts=thread_ts
		)
		...
		return {"success": True, "message": "Approval workflow created successfully", "result": result}

	except Exception as e:
		logger.error(f"Error creating approval workflow: {e}")
		raise HTTPException(status_code=500, detail=f"Workflow creation failed: {e!s}")

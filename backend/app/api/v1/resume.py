...
async def get_parsing_status(
    ...
    try:
        ...
        resume_upload = db.query(ResumeUpload).filter(
...
async def get_profile_suggestions(
    ...
    try:
        ...
        suggestions = await resume_parser.suggest_profile_updates(
            resume_upload.parsed_data or {},
            current_profile
        )
...
async def apply_profile_suggestions(
    ...
    try:
        ...
        if request.apply_suggestions:
            ...
            return {
                "success": True,
                "message": "Profile suggestions applied successfully",
        ...
        logger.error(f"Error applying profile suggestions: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def parse_resume_background(upload_id: int, file_path: str, filename: str, user_id: int):
    ...
    try:
        ...
        logger.error(f"Error in background resume parsing: {str(e)}")
...
async def parse_job_description(
    ...
    try:
        ...
        if not request.job_url and not request.description_text:
            raise HTTPException(
                status_code=400, 
                detail="Either job_url or description_text must be provided"
            )
        
        # Parse the job description
        parsed_data = await job_description_parser.parse_job_description(
        ...
        return JobDescriptionParseResponse(
            success=True,
            parsed_data=ParsedJobDescription(**parsed_data),
            suggestions=suggestions
        )
        
    except ValueError as e:
        logger.warning(f"Job description parsing validation error: {str(e)}")
        return JobDescriptionParseResponse(
            success=False,
            error_message=str(e)
        )
        ...
        return JobDescriptionParseResponse(
            success=False,
            error_message="Internal server error during job description parsing"
        )
...
async def generate_content(
    ...
    try:
        ...
        if request.content_type == "cover_letter":
            ...
            content_generation = await content_generator.generate_resume_tailoring(
                user=current_user,
                job=job,

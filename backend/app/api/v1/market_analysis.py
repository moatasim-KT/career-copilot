...
async def get_salary_trends(
	...
	"""
	try:
		analysis = market_analysis_service.analyze_salary_trends(db=db, user_id=current_user.id, role_filter=role_filter, days=days)
		...
		raise HTTPException(status_code=500, detail=f"Failed to analyze salary trends: {e!s}")
...
async def get_job_market_patterns(
	...
	"""
	try:
		analysis = market_analysis_service.analyze_job_market_patterns(db=db, user_id=current_user.id, days=days)
		...
		raise HTTPException(status_code=500, detail=f"Failed to analyze job market patterns: {e!s}")
...
async def get_opportunity_alerts(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
	...
	"""
	try:
		alerts = market_analysis_service.generate_opportunity_alerts(db=db, user_id=current_user.id)
		...
		raise HTTPException(status_code=500, detail=f"Failed to generate opportunity alerts: {e!s}")
...
async def get_market_dashboard(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
	...
	"""
	try:
		dashboard_data = market_analysis_service.create_market_dashboard_data(db=db, user_id=current_user.id)

		return dashboard_data

	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Failed to create market dashboard: {e!s}")
...
async def save_market_analysis(
	...
	try:
		...
		# Save to database
		success = market_analysis_service.save_analysis(db=db, user_id=current_user.id, analysis_data=analysis_data)
		...
		raise HTTPException(status_code=500, detail=f"Failed to save market analysis: {e!s}")
...
async def get_skill_demand_forecast(
	...
	try:
		...
		market_patterns = market_analysis_service.analyze_job_market_patterns(db=db, user_id=current_user.id, days=days)
		...
		raise HTTPException(status_code=500, detail=f"Failed to generate skill demand forecast: {e!s}")
...
async def get_competitive_analysis(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
	...
	try:
		...
		market_patterns = market_analysis_service.analyze_job_market_patterns(db=db, user_id=current_user.id, days=90)
		...
		raise HTTPException(status_code=500, detail=f"Failed to generate competitive analysis: {e!s}")
...
async def get_advanced_user_analytics(request: UserAnalyticsRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
	...
	try:
		# Parse timeframe
		timeframe_days = {"30d": 30, "90d": 90, "1y": 365}.get(request.timeframe, 30)
		...
		chart_data = {
			"conversion_funnel": [
				{"stage": stage["stage"], "count": stage["count"], "rate": stage["conversion_rate"]} for stage in conversion_funnel
			],
			"performance_benchmarks": [
				{"metric": bench["metric"], "user": bench["user_value"], "market": bench["market_average"]} for bench in performance_benchmarks
			],
			"success_probability_gauge": [{"category": "Success Probability", "value": success_probability}],
		}
		...
		raise HTTPException(status_code=500, detail=f"Failed to generate advanced user analytics: {e!s}")

...
async def oauth_login(provider: str, redirect_url: Optional[str] = Query(None)):
	...
	try:
		...
		logger.error(f"OAuth login initiation failed for {provider}: {e!s}")
...
async def oauth_callback(
	...
	try:
		...
		logger.error(f"OAuth user creation error for {provider}: {e!s}")
		raise HTTPException(status_code=400, detail=str(e))
	except Exception as e:
		logger.error(f"Unexpected OAuth callback error for {provider}: {e!s}")
...
async def disconnect_oauth(request: OAuthDisconnectRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
	...
	try:
		...
		logger.error(f"OAuth disconnect error for {request.provider}: {e!s}")
...
async def set_password_for_oauth_user(
	...
	try:
		...
		logger.error(f"Failed to set password for OAuth user {current_user.id}: {e!s}")

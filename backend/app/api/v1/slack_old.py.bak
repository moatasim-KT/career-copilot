...
async def send_notification(request: SlackNotificationRequest, current_user: User = Depends(get_current_user)):
	"""Send a general notification to Slack"""
	try:
		slack_service = SlackService()

		success = await slack_service.send_notification(
			title=request.title, message=request.message, priority=request.priority, channel=request.channel
		)
...
async def send_contract_analysis_alert(request: ContractAnalysisNotificationRequest, current_user: User = Depends(get_current_user)):
...
async def send_collaborative_workflow_notification(request: CollaborativeWorkflowRequest, current_user: User = Depends(get_current_user)):
...
async def create_channel(channel_name: str, is_private: bool = False, current_user: User = Depends(get_current_user)):
...
async def create_notification_preference(request: NotificationPreferenceRequest, current_user: User = Depends(get_current_user)):
	"""Create or update notification preference"""
	try:
		slack_service = SlackService()

		success = await slack_service.update_user_notification_preference(
			user_id=current_user.id, notification_type=request.notification_type, channel_id=request.channel_id, is_enabled=request.is_enabled
		)

...
async def get_health_status(service: EmailTemplateManager = Depends(get_email_template_service)):
...
async def create_template(template_data: EmailTemplateCreate, service: EmailTemplateManager = Depends(get_email_template_service)):
...
async def list_templates(template_type: Optional[TemplateType] = None, service: EmailTemplateManager = Depends(get_email_template_service)):
	"""List all available email templates"""
	try:
		templates = await service.list_templates(template_type=template_type)

		return TemplateListResponse(templates=templates, total_count=len(templates), template_types=[t.value for t in TemplateType])
...
async def get_template(template_id: str, service: EmailTemplateManager = Depends(get_email_template_service)):
...
async def validate_template(template_id: str, variables: Dict[str, Any], service: EmailTemplateManager = Depends(get_email_template_service)):
	...
	try:
		...
		try:
			await service.render_template(template_id=template_id, variables=variables, recipient="test@example.com")

			return TemplateValidationResult(valid=True, variables_found=template.variables, missing_variables=[])
			...
			return TemplateValidationResult(
				valid=False, errors=[str(render_error)], variables_found=template.variables, missing_variables=missing_vars
			)
...
async def render_template(
	template_id: str, render_request: TemplateRenderRequest, service: EmailTemplateManager = Depends(get_email_template_service)
):
	...
	try:
		...
		return TemplateRenderResponse(
			message=message, tracking_id=message.tracking_id, variables_used=render_request.variables, render_time=render_time
		)
...
async def upload_attachment(
	...
	attachment_type: str = Form("document"),
	...
	try:
		...
		if len(content) > service.max_attachment_size:
			raise HTTPException(status_code=413, detail=f"File too large: {len(content)} bytes (max: {service.max_attachment_size})")
		...
		return AttachmentUploadResponse(attachment_id=attachment_id, filename=file.filename, size=len(content), content_hash=content_hash)
...
async def add_file_attachment(
	...
	try:
		...
		message = EmailMessage(to=["recipient@example.com"], subject="Test Message", tracking_id=message_id)

		updated_message = await service.add_attachment_from_file(
			message=message, file_path=file_path, attachment_type=attachment_type, filename=filename, description=description
		)

		return {"success": True, "message": "Attachment added successfully", "attachment_count": len(updated_message.attachments)}
...
async def create_pdf_report_attachment(
	...
	try:
		attachment = await service.create_pdf_report_attachment(report_data=report_data, filename=filename)
...
async def create_docx_redline_attachment(
	...
	try:
		attachment = await service.create_docx_redline_attachment(
			original_content=original_content, redlined_content=redlined_content, filename=filename
		)
...
async def track_delivery_status(
	...
	try:
		...
		record = await service.track_delivery_status(
			tracking_id=tracking_id, status=delivery_status, provider=provider, message_id=message_id, error_message=error_message, metadata=metadata
		)

		return {"success": True, "tracking_id": record.tracking_id, "status": record.status, "updated_at": record.created_at.isoformat()}
...
async def get_delivery_status(tracking_id: str, service: EmailTemplateManager = Depends(get_email_template_service)):
...
async def get_delivery_statistics(
	...
	try:
		stats = await service.get_delivery_statistics(start_date=start_date, end_date=end_date, template_id=template_id)

		return DeliveryStatistics(**stats, period_start=start_date, period_end=end_date)
...
async def send_bulk_emails(
	bulk_request: BulkEmailRequest, background_tasks: BackgroundTasks, service: EmailTemplateManager = Depends(get_email_template_service)
):
	...
	try:
		...
		background_tasks.add_task(_process_bulk_email_job, job_id, bulk_request, tracking_ids, service)
...
async def preview_template(template_id: str, variables: Optional[str] = None, service: EmailTemplateManager = Depends(get_email_template_service)):
	...
	try:
		...
		# Render template
		message = await service.render_template(template_id=template_id, variables=template_vars, recipient="preview@example.com")

		return {"subject": message.subject, "html_body": message.html_body, "text_body": message.text_body, "variables_used": template_vars}
...
async def _process_bulk_email_job(job_id: str, bulk_request: BulkEmailRequest, tracking_ids: List[str], service: EmailTemplateManager):
	...
	try:
		...
		for i, recipient_data in enumerate(bulk_request.recipients):
			try:
				...
				message = await service.render_template(
					template_id=bulk_request.template_id, variables=variables, recipient=recipient_data["email"], tracking_id=tracking_ids[i]
				)
...
async def delivery_webhook(provider: str, webhook_data: Dict[str, Any], service: EmailTemplateManager = Depends(get_email_template_service)):
	...
	try:
		...
		if provider == "gmail":
			...
			status_mapping = {"delivered": "delivered", "open": "opened", "click": "clicked", "bounce": "bounced", "dropped": "failed"}
		...
		if tracking_id and status != "unknown":
			...
			try:
				...
				await service.track_delivery_status(
					tracking_id=tracking_id, status=delivery_status, provider=EmailProvider(provider), metadata=webhook_data
				)
			except ValueError:
...
async def email_open_tracking(tracking_id: str, service: EmailTemplateManager = Depends(get_email_template_service)):

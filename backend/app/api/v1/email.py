...
from ...services.email_service import EmailProvider, EmailService, UnifiedEmailMessage
...
async def send_email(request: EmailRequest, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user)):
	"""Send a basic email"""
	try:
...
async def send_template_email(request: TemplateEmailRequest, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to send template email: {e!s}")


@router.post("/send-contract-analysis", response_model=EmailResponse)
async def send_contract_analysis_email(
	request: ContractAnalysisEmailRequest, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user)
):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to send job application tracking email: {e!s}")


@router.post("/send-notification", response_model=EmailResponse)
async def send_notification_email(
	request: NotificationEmailRequest, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user)
):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to send notification email: {e!s}")


@router.post("/send-risk-alert", response_model=EmailResponse)
async def send_risk_alert_email(request: RiskAlertEmailRequest, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to send risk alert email: {e!s}")


@router.get("/quota/{user_id}")
async def get_quota_status(user_id: str, current_user: User = Depends(get_current_user)):
...
async def get_email_templates(current_user: User = Depends(get_current_user)):
	...
	try:
		...
		return [EmailTemplateResponse(**template.dict()) for template in templates]

	except Exception as e:
		logger.error(f"Failed to get email templates: {e}")
		raise HTTPException(status_code=500, detail=f"Failed to get email templates: {e!s}")


@router.get("/delivery-status/{message_id}", response_model=DeliveryStatusResponse)
async def get_delivery_status(message_id: str, current_user: User = Depends(get_current_user)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get delivery status: {e!s}")


@router.get("/statistics/{user_id}")
async def get_email_statistics(user_id: str, days: int = 30, current_user: User = Depends(get_current_user)):
	...
	try:
		...
		stats = await email_service.get_email_metrics(start_date=datetime.now() - timedelta(days=days), end_date=datetime.now())
		return stats
		...
		raise HTTPException(status_code=500, detail=f"Failed to get email statistics: {e!s}")
...
async def test_email_connection(current_user: User = Depends(get_current_user)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to test email connections: {e!s}")
...
async def get_email_service_status(current_user: User = Depends(get_current_user)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to get email service status: {e!s}")


@router.post("/reset-provider-health")
async def reset_provider_health(provider: Optional[str] = None, current_user: User = Depends(get_current_user)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to reset provider health: {e!s}")

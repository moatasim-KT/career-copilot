...
async def extract_skills(request: SkillExtractionRequest, current_user: User = Depends(get_current_user)):
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Skill extraction failed: {e!s}")


@router.post("/match-skills", response_model=SkillMatchingResponse)
async def match_skills(request: SkillMatchingRequest, current_user: User = Depends(get_current_user)):
	...
	try:
		score, details = skill_matching_service.calculate_skill_match_score(
			request.user_skills, request.job_skills, use_semantic=request.use_semantic
		)
		return SkillMatchingResponse(score=score, details=details)
	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Skill matching failed: {e!s}")


@router.post("/skill-similarity", response_model=SkillSimilarityResponse)
async def calculate_skill_similarity(request: SkillSimilarityRequest, current_user: User = Depends(get_current_user)):
	"""Calculate semantic similarity between two skills"""
	try:
		similarity = skill_matching_service.calculate_semantic_similarity(request.skill1, request.skill2)
...
async def batch_extract_skills(request: BatchSkillExtractionRequest, current_user: User = Depends(get_current_user)):
	...
	try:
		...
		most_common = [{"skill": skill, "count": count} for skill, count in skill_counts.most_common(10)]

		return BatchSkillExtractionResponse(results=results, total_unique_skills=len(unique_skills), most_common_skills=most_common)
	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Batch extraction failed: {e!s}")


@router.post("/analyze-skill-gaps")
async def analyze_skill_gaps(request: SkillMatchingRequest, current_user: User = Depends(get_current_user)):
	"""Analyze skill gaps and provide learning recommendations"""
	try:
		analysis = skill_matching_service.analyze_skill_gaps(request.user_skills, request.job_skills)
		return analysis
	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Skill gap analysis failed: {e!s}")
...
async def get_skill_vocabulary(current_user: User = Depends(get_current_user)):
	...
	try:
		...
		return {"vocabulary": vocabulary, "size": len(vocabulary)}
	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Failed to get vocabulary: {e!s}")

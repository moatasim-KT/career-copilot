...
async def generate_weekly_report(
    ...
    """
    try:
        report = scheduled_analytics_reports_service.generate_weekly_report(
            db=db,
...
async def generate_monthly_report(
    ...
    """
    try:
        report = scheduled_analytics_reports_service.generate_monthly_report(
            db=db,
            user_id=current_user.id
        )
...
async def email_weekly_report(
    ...
    try:
        ...
        background_tasks.add_task(
            scheduled_analytics_reports_service.send_report_email,
            report,
            current_user.email
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to schedule weekly report email: {str(e)}")
...
async def email_monthly_report(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to schedule monthly report email: {str(e)}")
...
async def schedule_all_weekly_reports(
    ...
    try:
        ...
        background_tasks.add_task(
            scheduled_analytics_reports_service.schedule_weekly_reports,
            db
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to schedule bulk weekly reports: {str(e)}")
...
async def schedule_all_monthly_reports(
    ...
    try:
        ...
        # Schedule bulk report generation in background
        background_tasks.add_task(
            scheduled_analytics_reports_service.schedule_monthly_reports,
            db
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to schedule bulk monthly reports: {str(e)}")
    ...
    try:
        ...
        if 'error' in success_rates:
            return {
                ...
                'message': 'Insufficient data for preview - submit some applications first'
            }
        ...
        preview = {
            ...
            'sample_insights': [
                "Performance trend analysis for the week",
                "Comparison with market benchmarks",
                "Application activity assessment"
            ],
            'sample_recommendations': [
                "Optimize application targeting",
                "Improve interview preparation",
                "Adjust weekly application goals"
            ],

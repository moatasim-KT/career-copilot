...
async def get_available_job_sources(
    ...
    try:
        ...
        for source_info in sources_info:
            job_sources.append(JobSourceInfo(
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving job sources: {str(e)}")
...
async def create_job_source_preferences(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error creating job source preferences: {str(e)}")
...
async def update_job_source_preferences(
    preferences: UserJobPreferencesUpdate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error updating job source preferences: {str(e)}")


@router.get("/api/v1/job-sources/preferences", response_model=UserJobPreferencesResponse)
async def get_job_source_preferences(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving job source preferences: {str(e)}")
...
async def get_job_source_analytics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving job source analytics: {str(e)}")
...
async def get_source_quality_metrics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving source quality metrics: {str(e)}")
...
async def update_source_priority(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error updating source priority: {str(e)}")
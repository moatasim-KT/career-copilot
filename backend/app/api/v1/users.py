...
async def update_user_profile(
    ...
    try:
        ...
        success = await settings_service.update_profile(
            current_user.id,
        ...
        return SuccessResponse(
            ...
            data={"updated_fields": profile_update.model_dump(exclude_none=True)}
        )
...
async def change_password(
    ...
    try:
        ...
        success = await settings_service.change_password(
            current_user.id,
...
async def get_ai_model_preference(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve AI model preference"
        )
...
async def get_risk_thresholds(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve risk thresholds"
        )
...
async def update_integration_setting(
    ...
    try:
        ...
        success = await settings_service.update_integration_setting(
            current_user.id,
            integration_name,
            setting_key,
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Failed to update integration setting"
            )

...
async def get_progress_summary(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
	...
	try:
		...
		summary = {
			...
			"monthly_metrics": {
				"applications_this_month": monthly_report.get("summary_metrics", {}).get("total_applications", 0)
				...
				"avg_apps_per_week": monthly_report.get("summary_metrics", {}).get("avg_applications_per_week", 0)
				if "error" not in monthly_report
				else 0,
			},
			"key_insights": [],
...
async def get_comprehensive_report(
	...
	try:
		...
		# Overall assessment
		if "error" not in progress_report:
			apps_count = progress_report.get("metrics", {}).get("applications_submitted", 0) or progress_report.get("summary_metrics", {}).get(
...
async def get_historical_reports(
	...
	try:
		...
		for report in historical_reports:
			reports.append({"id": report.id, "generated_at": report.generated_at.isoformat(), "data": report.data})

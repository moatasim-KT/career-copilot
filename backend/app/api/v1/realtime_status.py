        ...
        try:
            ...
            while await request.is_disconnected() == False:
                ...
                if not current_summary:
                    yield {
                        "event": "error",
                        "data": json.dumps({"error": "Task no longer exists"})
                    }
                    break
                ...
                if (current_progress != last_progress or 
                    ...
                    yield {
                        "event": "progress",
                        "data": json.dumps(current_summary)
                    }
                ...
                if current_stage in ["completed", "failed", "cancelled"]:
                    yield {
                        ...
                        "data": json.dumps({
                            "task_id": task_id,
                            "final_status": current_stage,
                            "final_data": current_summary
                        })
                    }
                    break
...
async def sse_dashboard_stream(
    ...
    async def event_generator():
        ...
        try:
            yield {
                "event": "connected",
                "data": json.dumps({"message": "Connected to dashboard stream"})
            }
    ...
    for stage, stage_data in stage_progress.items():
        ...
        if stage_data.get("completed"):
            ...
            if progress_range[1] > progress_range[0]:
                stage_progress_pct = (
                    (current_progress - progress_range[0]) / 
                    (progress_range[1] - progress_range[0]) * 100
                )

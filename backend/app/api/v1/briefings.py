...
from ...tasks.email_tasks import (
    send_morning_briefing_task,
    send_evening_summary_task,
...
async def get_briefing_preferences(
    ...
    try:
        ...
        return {
            ...
            "optimal_timing": {
                "morning": optimal_morning.strftime("%H:%M"),
                "evening": optimal_evening.strftime("%H:%M")
            },
...
async def send_morning_briefing_now(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Morning briefing scheduled",
            "task_id": task.id
        }
...
async def send_evening_summary_now(
    current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Evening summary scheduled",
            "task_id": task.id
        }
...
async def track_engagement(
    ...
    try:
        ...
        if engagement.email_type not in valid_email_types:
            raise HTTPException(
                status_code=400, 
                detail=f"Invalid email type. Must be one of: {valid_email_types}"
            )
        ...
        track_email_engagement.delay(
            current_user.id,
            engagement.email_type,
            engagement.action
        )
...
async def preview_morning_briefing(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "status": "success",
            "preview": briefing_data,
            "message": "Morning briefing preview generated"
        }
...
async def preview_evening_summary(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            ...
            "message": "Evening summary preview generated"
        }

...
def handle_calendar_oauth_callback(
    ...
    try:
        ...
        existing = (
            ...
            .filter(CalendarCredential.user_id == current_user.id, CalendarCredential.provider == provider)
            .first()
        )
...
def create_calendar_event(
    ...
    try:
        ...
        if event_data.provider == "google":
            result = service.create_google_event(
                credential.access_token, credential.refresh_token, calendar_event_data
            )
        else:  # outlook
...
def get_calendar_credentials(
    ...
    credentials = (
        db.query(CalendarCredential).filter(CalendarCredential.user_id == current_user.id).all()
    )
    return credentials

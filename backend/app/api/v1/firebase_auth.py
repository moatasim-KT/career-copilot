...
async def verify_firebase_token(request: FirebaseTokenRequest, oauth_service=Depends(get_oauth_service)):
	"""
	...
	try:
		...
		if not firebase_user:
			return FirebaseTokenResponse(valid=False, message="Invalid Firebase ID token")
		...
		try:
			...
			jwt_token = await oauth_service.create_access_token(data={"sub": firebase_user.uid, "email": firebase_user.email})
...
async def create_custom_token(
	...
	try:
		custom_token = await oauth_service.create_custom_token(request.uid, request.additional_claims)

		if not custom_token:
			raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create custom token")

		return CustomTokenResponse(custom_token=custom_token, expires_in=3600)

	except Exception as e:
		logger.error(f"Error creating custom token: {e}")
		raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error creating custom token")


@router.get("/firebase/user/{uid}")
async def get_firebase_user_info(uid: str, current_user=Depends(get_current_user_optional), oauth_service=Depends(get_oauth_service)):
	"""
	...
	try:
		...
		if not firebase_user:
			raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Firebase user not found")
...
async def set_user_claims(
	...
	try:
		...
		if not success:
			raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to set custom claims")
		...
		raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error setting custom claims")


@router.post("/firebase/revoke-tokens/{uid}")
async def revoke_user_tokens(
	uid: str, current_user=Depends(get_current_user_optional), oauth_service=Depends(get_oauth_service), iam_manager=Depends(get_iam_manager)
):
	"""
	...
	try:
		...
		raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error revoking tokens")


@router.get("/firebase/me")
async def get_current_firebase_user(request: Request, current_user=Depends(get_current_user)):
	"""

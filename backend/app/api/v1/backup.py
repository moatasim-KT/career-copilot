...
@router.post("/create")
async def create_backup(request: BackupRequest, background_tasks: BackgroundTasks) -> Dict[str, Any]:
	"""
	...
	try:
		...
		if request.include_files:
			background_tasks.add_task(_create_backup_task, request.include_files, request.compress, request.description)
			...
			backup_info = backup_service.create_full_backup(include_files=request.include_files, compress=request.compress)
...
async def list_backups() -> Dict[str, Any]:
	...
	try:
		...
		return {"backups": backups, "total_count": len(backups), "timestamp": backup_service._get_current_timestamp()}
...
async def get_backup_info(backup_name: str) -> Dict[str, Any]:
	...
	try:
		...
		logger.error(f"Failed to get backup info: {e!s}")
		raise HTTPException(status_code=500, detail=f"Failed to get backup info: {e!s}")


@router.post("/restore")
async def restore_backup(request: RestoreRequest, background_tasks: BackgroundTasks) -> Dict[str, Any]:
	"""
	...
	if not request.confirm:
		raise HTTPException(status_code=400, detail="Restore operation requires confirmation. Set 'confirm' to true.")

	try:
		logger.warning(f"Starting restore from backup: {request.backup_name}")

		# Run restore in background for safety
		background_tasks.add_task(_restore_backup_task, request.backup_name, request.restore_database, request.restore_files)
		...
		logger.error(f"Failed to start restore: {e!s}")
		raise HTTPException(status_code=500, detail=f"Restore failed: {e!s}")


@router.delete("/cleanup")
async def cleanup_old_backups(keep_days: int = Query(30, ge=1, le=365, description="Number of days to keep backups")) -> Dict[str, Any]:
	"""
	...
	try:
		...
		logger.error(f"Failed to cleanup backups: {e!s}")
...
async def delete_backup(backup_name: str) -> Dict[str, Any]:
	...
	"""
	try:
		delete_result = backup_service.delete_backup(backup_name)
		...
		return {
			...
			"deleted_items": delete_result.get("deleted_items", []),
			"timestamp": delete_result.get("timestamp", backup_service._get_current_timestamp()),
		}
	except ValueError as exc:
		logger.warning(f"Invalid backup deletion request: {backup_name} - {exc}")
		raise HTTPException(status_code=400, detail="Invalid backup name provided") from exc
	except FileNotFoundError as exc:
		raise HTTPException(status_code=404, detail=str(exc)) from exc
	except HTTPException:
		raise
	except Exception as e:
		logger.error(f"Failed to delete backup: {e!s}")
		raise HTTPException(status_code=500, detail=f"Failed to delete backup: {e!s}")
...
async def export_database_json() -> Dict[str, Any]:
	...
	try:
		...
		logger.error(f"Failed to export database: {e!s}")
		raise HTTPException(status_code=500, detail=f"Database export failed: {e!s}")


async def _create_backup_task(include_files: bool, compress: bool, description: str | None = None):
	"""Background task for creating backups"""
	try:
		backup_info = backup_service.create_full_backup(include_files=include_files, compress=compress)
		...
		logger.error(f"Background backup failed: {e!s}")
...
async def _restore_backup_task(backup_name: str, restore_database: bool, restore_files: bool):
	...
	try:
		restore_info = backup_service.restore_from_backup(backup_name=backup_name, restore_database=restore_database, restore_files=restore_files)

		logger.info(f"Background restore completed: {backup_name}")

	except Exception as e:
		logger.error(f"Background restore failed: {e!s}")

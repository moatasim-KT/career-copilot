...
async def get_user_connection_status(
    ...
    if current_user.id != user_id and not current_user.is_superuser:
        raise HTTPException(
            ...
            detail="Not authorized to check other users' connection status"
        )
...
async def send_system_notification(
    ...
    if not current_user.is_superuser:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to send system notifications"
        )
...
async def send_job_match_notification(
    request: JobMatchNotificationRequest,
    current_user: User = Depends(get_current_user)
):
    """
...
async def send_application_status_notification(
    ...
    try:
        await websocket_service.send_application_status_update(
            user_id=request.user_id,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to send application status notification"
        )
...
async def disconnect_user(
    ...
    if current_user.id != user_id and not current_user.is_superuser:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to disconnect other users"
        )
...
async def get_job_match_thresholds(
    current_user: User = Depends(get_current_user),
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get job match thresholds"
        )
...
async def update_job_match_thresholds(
    request: JobMatchThresholdsRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    ...
    if not current_user.is_superuser:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to update job match thresholds"
        )
    ...
    try:
        ...
        matching_service.update_match_thresholds(
            ...
            medium_match=request.medium_match_threshold
        )
        ...
        return {
            "success": True,
            "message": "Job match thresholds updated successfully",
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to update job match thresholds"
        )
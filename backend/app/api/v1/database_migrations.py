...
def get_migration_manager() -> DatabaseOptimizationService:
	from ...core.database_optimization import get_optimization_service

	return get_optimization_service()


DatabaseMigrationManager = DatabaseOptimizationService
...
async def get_migration_status(
	current_user: User = Depends(get_current_user), migration_manager: DatabaseMigrationManager = Depends(get_migration_manager)
):
	"""
	...
	try:
		...
		return BaseResponse(success=True, message="Migration status retrieved", data=status)

	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Failed to get migration status: {e!s}")
...
async def run_migrations(
	...
	try:
		...
		return BaseResponse(success=results["success"], message=results.get("message", "Migrations completed"), data=results)

	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Failed to run migrations: {e!s}")
...
async def rollback_migrations(
	...
	try:
		...
		return BaseResponse(success=results["success"], message=results.get("message", "Rollback completed"), data=results)

	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Failed to rollback migrations: {e!s}")
...
async def create_migration(
	...
	try:
		version = await migration_manager.create_migration(
			name=request.name, description=request.description, up_sql=request.up_sql, down_sql=request.down_sql, dependencies=request.dependencies
		)

		return BaseResponse(
			success=True,
			message=f"Migration created successfully: {version}",
			data={"version": version, "name": request.name, "description": request.description},
		)

	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Failed to create migration: {e!s}")
...
async def validate_migrations(
	...
	try:
		...
		raise HTTPException(status_code=500, detail=f"Failed to validate migrations: {e!s}")
...
async def get_migration_history(
	...
	try:
		...
		return BaseResponse(
			success=True, message=f"Retrieved {len(history)} migration records", data={"history": history, "total_count": len(history)}
		)

	except Exception as e:
		raise HTTPException(status_code=500, detail=f"Failed to get migration history: {e!s}")

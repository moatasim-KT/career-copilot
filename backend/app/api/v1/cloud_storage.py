...
from ...services.cloud_storage_manager import CloudStorageManager, StorageProvider, BackupPolicy
...
async def list_backups(
    ...
    try:
        ...
        backups = await storage_manager.list_user_backups(
            user_id=current_user.id,
            provider=provider,
            limit=limit
        )
...
async def restore_backup(
    ...
    try:
        ...
        analysis_data = await storage_manager.restore_contract_analysis(
        ...
        if not analysis_data:
            raise HTTPException(
                ...
                detail="Backup not found or could not be restored"
            )
...
async def share_backup(
    ...
    try:
        ...
        success = await storage_manager.share_backup(
            backup_file_id=backup_id,
            email_address=share_request.email_address,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to share backup: {str(e)}"
        )
...
async def get_storage_metrics(
    ...
    try:
        ...
        if not metrics:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Storage metrics not available"
            )
...
async def cleanup_old_backups(
    ...
    try:
        ...
        result = await storage_manager.cleanup_old_backups(
            user_id=current_user.id,
            provider=provider,
            force_cleanup=force
        )
...
async def export_backup_metadata(
    ...
    try:
        ...
        metadata = await storage_manager.export_backup_metadata(
            user_id=current_user.id,
            provider=provider
        )
        ...
        if not metadata:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Backup metadata not available"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to export backup metadata: {str(e)}"
        )
...
async def get_backup_policy(current_user: User = Depends(get_current_user)):
    ...
    try:
        storage_manager = CloudStorageManager()
        return {
            "backup_policy": storage_manager.backup_policy.dict(),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get backup policy: {str(e)}"
        )
...
async def estimate_cost(request: CostEstimateRequest) -> Dict[str, Any]:
    ...
    """
    try:
        estimate = await ai_service_manager.get_cost_estimate(
...
async def analyze_complexity(request: ComplexityAnalysisRequest) -> Dict[str, Any]:
    ...
    """
    try:
        analysis = await ai_service_manager.get_complexity_analysis(
            prompt=request.prompt,
            context=request.context
        )
        return analysis
...
async def get_budget_status(
    user_id: Optional[str] = Query(None, description="User ID for user-specific budgets")
) -> Dict[str, Any]:
    """
...
async def remove_budget_limit(
    ...
    """
    try:
        cost_tracker.remove_budget_limit(
            category=category,
            period=period,
            user_id=user_id
        )

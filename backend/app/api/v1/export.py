...
async def export_jobs(
	...
	try:
		...
		if format == ExportFormat.JSON:
			result = await export_service_v2.export_jobs_json(db, current_user.id, filters, page=page, page_size=page_size)
...
async def export_applications(
	...
	try:
		...
		if format == ExportFormat.JSON:
			result = await export_service_v2.export_applications_json(db, current_user.id, filters, page=page, page_size=page_size)
			if not result.get("success"):
				raise HTTPException(status_code=500, detail=result.get("error"))
			return result

...
async def get_notification_preferences(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
	...
	if not result["success"]:
		raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=result.get("message", "Failed to get notification preferences"))
...
async def update_notification_preferences(
	...
	result = await scheduled_notification_service.update_user_notification_preferences(user_id=current_user.id, preferences=preferences.dict(), db=db)

	if not result["success"]:
		raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=result.get("message", "Failed to update notification preferences"))

	return {"success": True, "message": "Notification preferences updated successfully", "preferences": result["preferences"]}
...
async def opt_in_notifications(
	...
	return {"success": True, "message": f"Successfully opted into: {', '.join(result['opted_in_types'])}", "opted_in_types": result["opted_in_types"]}

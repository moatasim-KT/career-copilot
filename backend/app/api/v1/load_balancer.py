...
async def manual_scaling(scaling_request: ScalingRequest):
    ...
    try:
        ...
        if scaling_request.action == "scale_up":
            ...
            idle_workers = [w for w in load_balancer.workers.values() 
                           if w.status.value == "idle" and w.active_requests == 0]
            ...
            return ScalingResponse(
                success=False,
                message=f"Invalid scaling action: {scaling_request.action}",
        ...
        return ScalingResponse(
            success=False,
            message=f"Scaling operation failed: {str(e)}",
            current_workers=0
        )
...
async def optimize_load_balancer(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Load balancer optimization started in background",
            "timestamp": datetime.utcnow().isoformat()
        }
...
async def restart_load_balancer():
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Load balancer restarted successfully",
            "timestamp": datetime.utcnow().isoformat()
        }
        
    except Exception as e:
        logger.error(f"Failed to restart load balancer: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to restart load balancer: {str(e)}")

...
async def get_agent_cache_statistics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get agent cache statistics: {str(e)}")
...
async def get_agent_cache_config(
    agent_type: AgentType = Path(..., description="Agent type")
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "timeout_config": timeout_config.__dict__
            },
            "timestamp": datetime.utcnow().isoformat()
        }
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get agent cache config: {str(e)}")
...
async def update_agent_cache_config(
    ...
    try:
        ...
        return {
            ...
            "data": {
                "agent_type": agent_type.value,
                "updated_config": updated_config.__dict__,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update agent cache config: {str(e)}")
...
async def update_agent_timeout_config(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update agent timeout config: {str(e)}")
...
async def get_agent_timeout(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get agent timeout: {str(e)}")

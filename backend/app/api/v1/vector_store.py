...
async def store_contract_embeddings(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to store contract embeddings: {str(e)}"
        )
...
async def search_similar_contracts(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to search similar contracts: {str(e)}"
        )
...
async def search_legal_precedents(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to search legal precedents: {str(e)}"
        )
...
async def batch_store_embeddings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to process batch embeddings: {str(e)}"
        )
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                ...
                detail=f"No embeddings found for contract {contract_id}"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to delete contract embeddings: {str(e)}"
        )
    ...
    try:
        ...
        if health_info["status"] == "healthy":
            ...
            raise HTTPException(
                ...
                detail=f"Vector store is unhealthy: {health_info.get('error', 'Unknown error')}"
            )
...
async def get_contract_embeddings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get contract embeddings: {str(e)}"
        )
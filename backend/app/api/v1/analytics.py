...
async def get_timeline(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
	applications = db.query(Application).filter(Application.user_id == current_user.id).order_by(Application.applied_date.desc()).limit(50).all()

	return [{"date": app.applied_date, "status": app.status, "job_id": app.job_id} for app in applications]


@router.get("/analytics/status")
async def get_status_breakdown(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
	status_counts = (
		db.query(Application.status, func.count(Application.id)).filter(Application.user_id == current_user.id).group_by(Application.status).all()
	)
...
async def get_interview_trends(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def get_comprehensive_dashboard(days: int = 90, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
	...
	applications_by_date = (
		db.query(func.date(Application.applied_date).label("date"), func.count(Application.id).label("count"))
		.filter(Application.user_id == current_user.id, Application.applied_date >= start_date, Application.applied_date <= end_date)
	...
	for i in range(0, days, 7):
		...
		week_applications = (
			...
			.filter(Application.user_id == current_user.id, Application.applied_date >= week_start, Application.applied_date <= week_end)
			.count()
		)
	...
	return {
		"summary": summary,
		"application_trends": [{"date": date.isoformat(), "applications": count} for date, count in applications_by_date],

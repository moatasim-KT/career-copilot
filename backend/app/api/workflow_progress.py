...
async def get_all_active_workflows(
    ...
    try:
        ...
        if state_filter:
            filtered_workflows = {
                wf_id: wf_data for wf_id, wf_data in all_workflows.items()
                if wf_data.get("workflow_state") == state_filter
            }
        else:
    ...
    try:
        ...
        if workflow_id:
            ...
            if agent_name not in workflow_progress.agent_progress:
                raise HTTPException(
                    status_code=404,
                    detail=f"Agent {agent_name} not found in workflow {workflow_id}"
                )
            ...
            raise HTTPException(
                status_code=400,
                detail="workflow_id parameter is required to get agent progress"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve agent progress: {str(e)}"
        )
...
async def cancel_workflow(
    ...
    try:
        ...
        if workflow_progress.workflow_state in [WorkflowState.COMPLETED, WorkflowState.FAILED, WorkflowState.CANCELLED]:
            raise HTTPException(
                status_code=400,
                detail=f"Workflow {workflow_id} is already {workflow_progress.workflow_state.value} and cannot be cancelled"
            )

...
class DocumentPerformanceMetrics(BaseModel):
	...
	avg_response_time_days: float | None = Field(None, description="Average response time in days")
...
class DocumentSuggestion(BaseModel):
	...
	last_used: datetime | None = Field(None, description="When document was last used")
...
class DocumentOptimizationRequest(BaseModel):
	...
	target_job_id: int | None = Field(None, description="Optional target job for context")

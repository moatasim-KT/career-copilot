    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if enable_timeout:
                ...
                result = await agent_cache_manager.execute_with_timeout(
...
def timeout_handler(
    ...
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        async def wrapper(*args, **kwargs):
            timeout_seconds = custom_timeout or agent_cache_manager.get_timeout_for_agent(
                agent_type, retry_count
            )
...
class CacheInvalidationManager:
    ...
    def add_invalidation_rule(
        ...
        self.invalidation_rules[trigger_agent] = {
...
class CacheWarmupManager:
    ...
    def add_warmup_task(
        ...
        warmup_inputs: List[Dict[str, Any]],
        priority: int = 1
    ):
        """Add cache warmup task."""
        self.warmup_tasks[agent_type] = {
            "function": warmup_func,
...
def configure_cache_invalidation_rules():
    ...
    cache_invalidation_manager.add_invalidation_rule(
        trigger_agent=AgentType.RISK_ASSESSOR,
        affected_agents=[AgentType.NEGOTIATION, AgentType.COMMUNICATION]
    )

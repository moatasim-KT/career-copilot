...
def parse_resume_async(self, resume_upload_id: int, file_path: str, filename: str, user_id: int) -> Dict[str, Any]:
	...
	try:
		...
		self.update_state(state="PROGRESS", meta={"current": 10, "total": 100, "status": "Starting resume parsing..."})

		# Initialize parser service
		parser_service = ResumeParserService()

		# Validate file
		self.update_state(state="PROGRESS", meta={"current": 20, "total": 100, "status": "Validating file..."})
		...
		# Parse the resume
		self.update_state(state="PROGRESS", meta={"current": 50, "total": 100, "status": "Parsing resume content..."})

		parsed_data = parser_service.parse_resume_sync(file_path, filename, user_id)

		# Update database with results
		self.update_state(state="PROGRESS", meta={"current": 80, "total": 100, "status": "Saving parsed data..."})
		...
		return {"status": "success", "resume_upload_id": resume_upload_id, "parsed_data": parsed_data, "message": "Resume parsed successfully"}

	except Exception as e:
		logger.error(f"Error parsing resume {filename}: {e!s}")
...
def process_resume_parsing_queue():
	...
	try:
		...
		pending_uploads = db.query(ResumeUpload).filter(ResumeUpload.parsing_status == "pending").limit(10).all()
		...
		for upload in pending_uploads:
			try:
				...
				# Submit parsing task
				parse_resume_async.delay(resume_upload_id=upload.id, file_path=upload.file_path, filename=upload.filename, user_id=upload.user_id)
				...
				upload.parsing_status = "failed"
				upload.error_message = f"Task submission failed: {e!s}"
				db.commit()
...
def cleanup_old_resume_files(days_old: int = 30):
	...
	try:
		...
		old_uploads = (
			db.query(ResumeUpload).filter(ResumeUpload.created_at < cutoff_date, ResumeUpload.parsing_status.in_(["completed", "failed"])).all()
		)
...
def batch_parse_resumes(self, resume_upload_ids: list) -> Dict[str, Any]:
	...
	try:
		...
		for i, upload_id in enumerate(resume_upload_ids):
			try:
				...
				result = parse_resume_async.delay(
					resume_upload_id=upload.id, file_path=upload.file_path, filename=upload.filename, user_id=upload.user_id
				)
		...
		return {"status": "success", "total": total_resumes, "processed": processed, "successful": successful, "failed": failed}

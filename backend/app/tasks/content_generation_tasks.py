...
def generate_cover_letter_async(
	self, user_id: int, job_id: int, tone: str = "professional", custom_instructions: Optional[str] = None
) -> Dict[str, Any]:
	"""
	...
	try:
		...
		self.update_state(state="PROGRESS", meta={"current": 10, "total": 100, "status": "Loading user and job data..."})
		...
		# Update progress
		self.update_state(state="PROGRESS", meta={"current": 30, "total": 100, "status": "Generating cover letter..."})

		# Generate cover letter
		content_generation = content_service.generate_cover_letter_sync(
			user=user, job=job, tone=tone, custom_instructions=custom_instructions, db=db
		)

		# Update progress
		self.update_state(state="PROGRESS", meta={"current": 90, "total": 100, "status": "Finalizing generation..."})

		self.update_state(state="SUCCESS", meta={"current": 100, "total": 100, "status": "Cover letter generated successfully"})
		...
		logger.error(f"Error generating cover letter for user {user_id}, job {job_id}: {e!s}")
...
def generate_resume_tailoring_async(self, user_id: int, job_id: int, resume_sections: Dict[str, str]) -> Dict[str, Any]:
	"""
	...
	try:
		...
		# Update progress
		self.update_state(state="PROGRESS", meta={"current": 30, "total": 100, "status": "Analyzing resume and job requirements..."})

		# Generate resume tailoring
		content_generation = content_service.generate_resume_tailoring_sync(user=user, job=job, resume_sections=resume_sections, db=db)

		# Update progress
		self.update_state(state="PROGRESS", meta={"current": 90, "total": 100, "status": "Finalizing suggestions..."})

		self.update_state(state="SUCCESS", meta={"current": 100, "total": 100, "status": "Resume tailoring completed successfully"})
		...
		logger.error(f"Error generating resume tailoring for user {user_id}, job {job_id}: {e!s}")
		logger.error(f"Traceback: {traceback.format_exc()}")
...
def batch_generate_content(self, content_requests: list) -> Dict[str, Any]:
	...
	try:
		...
		for i, request in enumerate(content_requests):
			try:
				...
				if content_type == "cover_letter":
					...
					result = generate_resume_tailoring_async.delay(user_id=user_id, job_id=job_id, resume_sections=request.get("resume_sections", {}))
				else:
				...
				try:
					...
					results.append({"request_index": i, "status": "success", "result": task_result})
					successful += 1
				except Exception as e:
					logger.error(f"Batch content generation failed for request {i}: {e}")
					results.append({"request_index": i, "status": "failed", "error": str(e)})
					failed += 1
		...
		return {"status": "success", "total": total_requests, "processed": processed, "successful": successful, "failed": failed, "results": results}
...
def cleanup_old_content(days_old: int = 90):
	...
	try:
		...
		old_content = (
			...
			.filter(ContentGeneration.created_at < cutoff_date, ContentGeneration.status.in_(["generated", "user_modified"]))
			.all()
		)
...
def pregenerate_popular_content():
	...
	try:
		...
		popular_jobs = db.query(Job).join(Job.applications).group_by(Job.id).order_by(func.count(Job.applications).desc()).limit(10).all()

		# Get active users
		active_users = db.query(User).filter(User.skills.isnot(None)).limit(20).all()
			...
			for user in active_users:
				try:
					...
					existing_content = (
						db.query(ContentGeneration)
						.filter(
							ContentGeneration.user_id == user.id, ContentGeneration.job_id == job.id, ContentGeneration.content_type == "cover_letter"
						)
						.first()
					)
					...
					generate_cover_letter_async.delay(user_id=user.id, job_id=job.id, tone="professional")

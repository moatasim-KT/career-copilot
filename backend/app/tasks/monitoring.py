...
class TaskMonitor:
	...
	@staticmethod
	def log_task_start(task_name: str, task_id: str, args: tuple | None = None, kwargs: dict | None = None):
	...
	def log_task_success(task_name: str, task_id: str, result: Any, runtime: float):
		...
		logger.info(
			f"Task {task_name} [{task_id}] completed successfully in {runtime:.2f}s",
			extra={
				"task_name": task_name,
				"task_id": task_id,
	...
	def log_task_failure(task_name: str, task_id: str, error: Exception, traceback_str: str):
		...
		logger.error(
			f"Task {task_name} [{task_id}] failed: {error!s}",
			extra={
				"task_name": task_name,
				"task_id": task_id,
				"error": str(error),
	...
	@staticmethod
	def save_task_metrics(db: Session, task_name: str, task_id: str, status: str, runtime: float | None = None, error: str | None = None):
		...
		try:
			...
			logger.error(f"Failed to save task metrics: {e!s}")
			db.rollback()
...
def task_postrun_handler(sender=None, task_id=None, task=None, args=None, kwargs=None, retval=None, state=None, **kwds):
	...
	if state == "SUCCESS":
		...
		try:
			...
			logger.error(f"Failed to save success metrics: {e!s}")
...
def task_failure_handler(sender=None, task_id=None, exception=None, traceback=None, einfo=None, **kwds):
	...
	try:
		...
		logger.error(f"Failed to save failure metrics: {e!s}")
...
def send_failure_alert(task_name: str, task_id: str, exception: Exception):
	...
	try:
		...
		logger.critical(f"CRITICAL TASK FAILURE: {task_name} [{task_id}] - {exception!s}")
		...
		try:
			...
			logger.error(f"Failed to instantiate or use NotificationService for alert: {e!s}")

	except Exception as e:
		logger.error(f"Failed to send failure alert: {e!s}")
...
def monitor_task_health() -> Dict[str, Any]:
	...
	try:
		...
		try:
			...
			# Save health report
			health_analytics = Analytics(user_id=None, type="task_system_health", data=health_report)
			db.add(health_analytics)
			db.commit()

			logger.info(f"Task health check completed: {health_report['health_status']} ({success_rate:.1f}% success rate)")
...
def get_task_status(task_id: str) -> Dict[str, Any]:
	...
	try:
		...
		try:
			db = next(get_db())
			try:
				task_metrics = db.query(Analytics).filter(Analytics.type.like("task_metrics_%"), Analytics.data["task_id"].astext == task_id).first()
			...
			logger.warning(f"Could not fetch task metrics from database: {e!s}")
...
def system_health_check() -> Dict[str, Any]:
	...
	try:
		...
		try:
			...
			health_report["components"]["database"] = {"status": "error", "error": str(e)}
			health_report["overall_status"] = "unhealthy"
		...
		try:
			...
			health_report["components"]["redis"] = {"status": "degraded", "error": str(e)}
		...
		try:
			db = next(get_db())
			try:
				health_analytics = Analytics(user_id=None, type="system_health_check", data=health_report)
				db.add(health_analytics)
				db.commit()
			finally:
				db.close()
		except Exception as e:
			logger.error(f"Failed to save health report: {e!s}")

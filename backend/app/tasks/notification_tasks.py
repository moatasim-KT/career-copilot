...
def send_email_async(self, user_id: int, subject: str, content: str, email_type: str = "general") -> Dict[str, Any]:
	"""
	...
	try:
		...
		self.update_state(state="PROGRESS", meta={"current": 10, "total": 100, "status": "Loading user data..."})
		...
		# Update progress
		self.update_state(state="PROGRESS", meta={"current": 50, "total": 100, "status": "Sending email..."})

		# Send email
		success = notification_service._send_email(to_email=user.email, subject=subject, content=content)

		if not success:
			raise Exception("Email sending failed")

		self.update_state(state="SUCCESS", meta={"current": 100, "total": 100, "status": "Email sent successfully"})

		logger.info(f"Successfully sent {email_type} email to user {user_id}")

		return {"status": "success", "user_id": user_id, "email_type": email_type, "message": "Email sent successfully"}

	except Exception as e:
		logger.error(f"Error sending email to user {user_id}: {e!s}")
...
def send_job_alerts_async(self, user_id: int, job_matches: List[Dict]) -> Dict[str, Any]:
	...
	try:
		# Update task progress
		self.update_state(state="PROGRESS", meta={"current": 10, "total": 100, "status": "Preparing job alerts..."})
		...
		# Update progress
		self.update_state(state="PROGRESS", meta={"current": 50, "total": 100, "status": "Generating alert email..."})
		...
		self.update_state(state="SUCCESS", meta={"current": 100, "total": 100, "status": "Job alerts sent successfully"})

		logger.info(f"Successfully sent job alerts to user {user_id} for {len(job_matches)} matches")

		return {"status": "success", "user_id": user_id, "matches_sent": len(job_matches), "message": "Job alerts sent successfully"}

	except Exception as e:
		logger.error(f"Error sending job alerts to user {user_id}: {e!s}")
		logger.error(f"Traceback: {traceback.format_exc()}")
...
def batch_send_notifications(self, notification_requests: List[Dict]) -> Dict[str, Any]:
	"""
	...
	try:
		...
		return {"status": "success", "total": total_requests, "processed": processed, "successful": successful, "failed": failed}

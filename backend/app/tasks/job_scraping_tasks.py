"""
Background tasks for job scraping and ingestion
"""
...
def scrape_jobs_for_user_async(self, user_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 10, "total": 100, "status": "Loading user preferences..."}
        )
        ...
        if not user.skills or not user.preferred_locations:
            ...
            return {
                "status": "skipped",
                "reason": "User profile incomplete (missing skills or locations)"
            }
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 30, "total": 100, "status": "Scraping jobs from external sources..."}
        )
        
        # Scrape jobs
        scraped_jobs = job_scraper.scrape_jobs(
            skills=user.skills,
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 60, "total": 100, "status": "Deduplicating jobs..."}
        )
        ...
        self.update_state(
            state="SUCCESS",
            meta={"current": 100, "total": 100, "status": "Job scraping completed successfully"}
        )
    ...
    try:
        ...
        self.update_state(
            ...
            meta={
                "current": total_users,
                "total": total_users,
                "status": "Batch job scraping completed",
                "successful": successful,
                "failed": failed
            }
        )

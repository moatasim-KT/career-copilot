...
def generate_daily_recommendations(self):
    ...
    try:
        ...
        for user in users:
            ...
            try:
                ...
                if recommendations:
                    ...
                    analytics = Analytics(
                        user_id=user.id,
                        type="recommendation_generation_metrics",
                        data=metrics_data
                    )
                    db.add(analytics)
                ...
                results['failed_generations'] += 1
                error_msg = f"Failed to generate recommendations for user {user.id}: {str(e)}"
        ...
        logger.error(f"Critical error in daily recommendation generation: {str(e)}")
...
@celery_app.task(
    name="optimize_recommendation_performance",
    bind=True,
    autoretry_for=(Exception,),
    ...
    try:
        ...
        for user in users:
            try:
                ...
                logger.error(f"Failed to optimize recommendations for user {user.id}: {str(e)}")
        ...
        system_analytics = Analytics(
            user_id=None,
            type="system_recommendation_optimization",
            data=optimization_results
        )
        db.add(system_analytics)
        db.commit()
        
        logger.info(f"Recommendation optimization completed: {optimization_results['users_needing_optimization']}/{len(users)} users need optimization")
        
        return optimization_results
        
    except Exception as e:
        logger.error(f"Critical error in recommendation optimization: {str(e)}")
...
@celery_app.task(
    name="cleanup_old_recommendation_cache",
    bind=True,
    autoretry_for=(Exception,),
...
def cleanup_old_recommendation_cache(self):
    ...
    try:
        ...
        cache_deleted = db.query(Analytics).filter(
            Analytics.type == "recommendation_cache",
            Analytics.generated_at < cache_cutoff
        ).delete()
        ...
        interaction_deleted = db.query(Analytics).filter(
            Analytics.type == "recommendation_interactions",
            Analytics.generated_at < interaction_cutoff
        ).delete()
        ...
        metrics_deleted = db.query(Analytics).filter(
            Analytics.type.in_([
                "recommendation_generation_metrics",
                "daily_recommendation_task_metrics",
                "system_recommendation_optimization"
            ]),
        ...
        error_deleted = db.query(Analytics).filter(
            Analytics.type == "recommendation_generation_error",
            Analytics.generated_at < error_cutoff
        ).delete()
        ...
        total_deleted = (cache_deleted + interaction_deleted + 
                        metrics_deleted + error_deleted)
        
        logger.info(f"Cache cleanup completed: {total_deleted} records deleted")
        
        # Store cleanup metrics
        cleanup_analytics = Analytics(
            user_id=None,
            type="recommendation_cache_cleanup_metrics",
        ...
        logger.error(f"Error in recommendation cache cleanup: {str(e)}")
...
def generate_personalized_recommendations(self, user_id: int, force_refresh: bool = False):
    ...
    try:
        ...
        analytics = Analytics(
            user_id=user_id,
            type="personalized_recommendation_metrics",
...
@celery_app.task(
    name="track_recommendation_performance",
    bind=True,
    autoretry_for=(Exception,),
    retry_kwargs={'max_retries': 2, 'countdown': 60}
)
def track_recommendation_performance(self):
    ...
    try:
        ...
        generation_metrics = db.query(Analytics).filter(
            Analytics.type == "recommendation_generation_metrics",
            Analytics.generated_at >= week_ago
        ).all()
        ...
        interaction_data = db.query(Analytics).filter(
            Analytics.type == "recommendation_interactions",
            Analytics.generated_at >= week_ago
        ).all()
        ...
        tracking_analytics = Analytics(
            user_id=None,
            type="recommendation_performance_tracking",
            data=performance_data
        )
        db.add(tracking_analytics)
        db.commit()
        
        logger.info(f"Recommendation performance tracking completed: {total_interactions} interactions, {performance_data['total_recommendations_generated']} recommendations")
        
        return performance_data
        
    except Exception as e:
        logger.error(f"Error in recommendation performance tracking: {str(e)}")
        raise self.retry(countdown=60, max_retries=2)
    finally:
        db.close()
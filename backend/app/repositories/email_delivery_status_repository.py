...
from ..services.email_service import EmailDeliveryStatus
...
class EmailDeliveryStatusRepository(BaseRepository):
	...
	async def create_status(self, message_id: str, recipient: str, status: str, error_message: Optional[str] = None) -> EmailDeliveryStatus:
	...
	async def get_status_by_message_id(self, message_id: str) -> Optional[EmailDeliveryStatus]:
		...
		result = await self.session.execute(select(EmailDeliveryStatus).where(EmailDeliveryStatus.message_id == message_id))
		return result.scalars().first()

	async def update_status(self, message_id: str, status: str, error_message: Optional[str] = None) -> Optional[EmailDeliveryStatus]:
		...
		if status_to_update:
			...
			if status == "delivered":
				...
				status_to_update.delivered_at = datetime.now(timezone.utc)
			await self.session.commit()

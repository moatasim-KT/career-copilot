...
from ..models.database_models import AnalysisHistory, AgentExecution
...
class AnalysisRepository:
	...
	def get_analysis_history(self, analysis_id: UUID) -> Optional[AnalysisHistory]:
		...
		return self.db.query(AnalysisHistory).filter(AnalysisHistory.id == analysis_id, AnalysisHistory.deleted_at.is_(None)).first()
	...
	def get_analysis_history_with_executions(self, analysis_id: UUID) -> Optional[AnalysisHistory]:
		...
		return (
			...
			.options(joinedload(AnalysisHistory.agent_executions))
			.filter(AnalysisHistory.id == analysis_id, AnalysisHistory.deleted_at.is_(None))
			.first()
		)
	...
	async def calculate_agent_performance(self, agent_id: Optional[str] = None, time_window: Optional[timedelta] = None) -> AgentPerformanceMetrics:
		"""
		...
		if not executions:
			return AgentPerformanceMetrics(
				...
				error_rate=0.0,
				retry_stats=RetryStatistics(total_retries=0, average_retries=0.0, max_retries=0),
			)
	...
	async def calculate_provider_performance(self, provider: str, time_window: Optional[timedelta] = None) -> ProviderPerformanceMetrics:
		"""
		...
		"""
		query = self.db.query(AgentExecution).filter(AgentExecution.provider == provider)
		...
		if not executions:
			return ProviderPerformanceMetrics(
				...
				average_latency=0.0,
				error_rate=0.0,
				cost_metrics={"total_cost": Decimal("0.0"), "average_cost": Decimal("0.0")},
			)
	...
	async def calculate_retry_statistics(self, time_window: Optional[timedelta] = None) -> RetryStatistics:
		"""
		...
		if not retries:
			return RetryStatistics(total_retries=0, average_retries=0.0, max_retries=0)

		return RetryStatistics(total_retries=sum(retries), average_retries=float(sum(retries) / len(retries)), max_retries=max(retries))

	def list_analysis_history(
		self, filters: Optional[AnalysisHistoryFilter] = None, skip: int = 0, limit: int = 100, include_executions: bool = False
	...
	def get_analysis_history_by_contract(self, contract_id: UUID) -> List[AnalysisHistory]:
		...
		return (
			self.db.query(AnalysisHistory)
			.filter(AnalysisHistory.contract_id == contract_id, AnalysisHistory.deleted_at.is_(None))
	...
	def list_agent_executions(
		self, filters: Optional[AgentExecutionFilter] = None, skip: int = 0, limit: int = 100
	...
	def get_executions_by_analysis(self, analysis_id: UUID) -> List[AgentExecution]:
		...
		return self.db.query(AgentExecution).filter(AgentExecution.analysis_id == analysis_id).order_by(AgentExecution.started_at).all()
	...
	def get_analysis_performance_metrics(
		...
		query = self.db.query(AnalysisHistory).filter(AnalysisHistory.deleted_at.is_(None))
		...
		if not analyses:
			return AnalysisPerformanceMetrics(
				total_analyses=0, success_rate=Decimal("0"), agent_performance={}, provider_performance={}, analyses_by_day={}, cost_by_day={}
			)
		...
		for analysis in analyses:
			if analysis.status == "failed":
				...
				executions = self.db.query(AgentExecution).filter(AgentExecution.analysis_id == analysis.id).all()
	...
	def get_agent_performance_metrics(
		self, agent_name: Optional[str] = None, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None
	...
	def get_cost_analysis(
		...
		query = self.db.query(AgentExecution).filter(AgentExecution.cost.isnot(None))
	...
	def _calculate_agent_performance_for_analyses(self, analyses: List[AnalysisHistory]) -> Dict[str, AgentPerformanceMetrics]:
		...
		executions = self.db.query(AgentExecution).filter(AgentExecution.analysis_id.in_(analysis_ids)).all()
	...
	def _calculate_provider_performance_for_analyses(self, analyses: List[AnalysisHistory]) -> Dict[str, ProviderPerformanceMetrics]:
		...
		for provider, provider_executions in provider_groups.items():
			...
			provider_performance[provider] = ProviderPerformanceMetrics(
				...
				error_rate=error_rate,
				cost_metrics={"total_cost": total_cost, "average_cost": avg_cost},
	...
	def _calculate_retry_statistics_for_executions(self, executions: List[AgentExecution]) -> Dict[str, Any]:
		...
		if not executions:
			return {"total_retries": 0, "average_retries": 0.0, "max_retries": 0, "retry_success_rate": 0.0, "common_retry_reasons": {}}

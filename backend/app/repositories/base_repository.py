...
class BaseRepository(Generic[ModelType]):
	...
	async def list_all(self, limit: Optional[int] = None, offset: Optional[int] = None, order_by: Optional[str] = None) -> List[ModelType]:
		"""
	...
	async def list_by_field(self, field_name: str, value: Any, limit: Optional[int] = None, offset: Optional[int] = None) -> List[ModelType]:
		"""
	...
	async def bulk_update(self, updates: List[Dict[str, Any]]) -> int:
		...
		for update_data in updates:
			...
			result = await self.session.execute(update(self.model).where(self.model.id == record_id).values(**update_data))
			total_updated += result.rowcount

		return total_updated

	async def execute_optimized_query(self, query: Select, use_cache: bool = False, cache_ttl: int = 300) -> Any:
		"""
		...
		try:
			...
			if execution_time > 1.0 and self._enable_query_logging:
				logger.warning(f"Slow query detected ({execution_time:.2f}s): {str(query)[:200]}...")

			return result

		except Exception as e:
			execution_time = time.time() - start_time
			logger.error(f"Query failed after {execution_time:.2f}s: {str(e)[:200]}...")
			raise

	async def get_with_relationships(self, id: UUID, relationships: List[str] | None = None) -> Optional[ModelType]:
		"""
	...
	async def search_text(self, search_term: str, text_fields: List[str], limit: Optional[int] = None, use_full_text: bool = True) -> List[ModelType]:
		"""
		...
		if use_full_text and hasattr(self.session.bind, "dialect") and "postgresql" in str(self.session.bind.dialect):
			...
			for field_name in text_fields:
				if hasattr(self.model, field_name):
					field = getattr(self.model, field_name)
					search_conditions.append(func.to_tsvector("english", field).match(search_term))
	...
	async def get_statistics(self, group_by_field: str | None = None) -> Dict[str, Any]:
		"""
	...
	async def optimize_table(self) -> Dict[str, Any]:
		...
		try:
			...
			return {"table_name": table_name, "analyzed": False, "error": str(e)}

...
class AuditRepository(BaseRepository[AuditLog]):
	...
	async def log_user_login(
		self, user_id: UUID, ip_address: str, user_agent: str, success: bool, session_id: Optional[str] = None, request_id: Optional[str] = None
	) -> AuditLog:
		"""
	...
	async def log_user_logout(self, user_id: UUID, ip_address: str, session_id: Optional[str] = None, request_id: Optional[str] = None) -> AuditLog:
		"""
		Log a user logout.
	...
	async def log_security_event(
		...
		return await self.log_event(
			event_type=event_type,
			user_id=user_id,
			event_data={"description": description, "severity": severity},
	...
	async def get_user_activity(
		self, user_id: UUID, limit: Optional[int] = None, offset: Optional[int] = None, event_type: Optional[str] = None
	) -> List[AuditLog]:
		"""
	...
	async def get_security_events(self, severity: Optional[str] = None, hours: int = 24, limit: Optional[int] = None) -> List[AuditLog]:
		"""
		...
		query = select(AuditLog).where(and_(AuditLog.created_at >= time_threshold, AuditLog.result == "SECURITY_EVENT"))

		if severity:
			query = query.where(AuditLog.event_data["severity"].astext == severity)
	...
	async def get_failed_login_attempts(self, hours: int = 24, ip_address: Optional[str] = None) -> List[AuditLog]:
		"""
		...
		query = select(AuditLog).where(and_(AuditLog.event_type == "user_login", AuditLog.result == "FAILURE", AuditLog.created_at >= time_threshold))
	...
	async def get_audit_statistics(self, days: int = 30) -> Dict:
		...
		# Total events
		total_result = await self.session.execute(select(func.count(AuditLog.id)).where(AuditLog.created_at >= date_threshold))
		total_events = total_result.scalar()

		# Events by type
		type_result = await self.session.execute(
			select(AuditLog.event_type, func.count(AuditLog.id)).where(AuditLog.created_at >= date_threshold).group_by(AuditLog.event_type)
		)
		events_by_type = dict(type_result.fetchall())

		# Events by result
		result_result = await self.session.execute(
			select(AuditLog.result, func.count(AuditLog.id)).where(AuditLog.created_at >= date_threshold).group_by(AuditLog.result)
		)
		events_by_result = dict(result_result.fetchall())

		# Unique users
		users_result = await self.session.execute(
			select(func.count(func.distinct(AuditLog.user_id))).where(and_(AuditLog.created_at >= date_threshold, AuditLog.user_id.isnot(None)))
		)
		unique_users = users_result.scalar()

		# Unique IP addresses
		ips_result = await self.session.execute(
			select(func.count(func.distinct(AuditLog.ip_address))).where(and_(AuditLog.created_at >= date_threshold, AuditLog.ip_address.isnot(None)))
		)
	...
	async def cleanup_old_logs(self, days_old: int = 90) -> int:
		...
		result = await self.session.execute(select(AuditLog.id).where(AuditLog.created_at < cutoff_date))
		...
		if log_ids:
			...
			await self.session.execute(delete(AuditLog).where(AuditLog.id.in_(log_ids)))

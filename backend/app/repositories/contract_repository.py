...
class ContractRepository(BaseRepository[ContractAnalysis]):
	...
	async def create_analysis(
		self, user_id: UUID, filename: str, file_hash: str, file_size: int, contract_text: Optional[str] = None
	) -> ContractAnalysis:
		"""
		...
		return await self.create(
			user_id=user_id, filename=filename, file_hash=file_hash, file_size=file_size, contract_text=contract_text, analysis_status="pending"
		)

	async def update_analysis_status(self, analysis_id: UUID, status: str, error_message: Optional[str] = None) -> Optional[ContractAnalysis]:
		"""
	...
	async def get_user_analyses(
		self, user_id: UUID, limit: Optional[int] = None, offset: Optional[int] = None, status_filter: Optional[str] = None
	...
	async def get_analysis_with_user(self, analysis_id: UUID) -> Optional[ContractAnalysis]:
		...
		result = await self.session.execute(
			select(ContractAnalysis).options(selectinload(ContractAnalysis.user)).where(ContractAnalysis.id == analysis_id)
		)
		return result.scalar_one_or_none()

	async def get_analyses_by_status(self, status: str, limit: Optional[int] = None, offset: Optional[int] = None) -> List[ContractAnalysis]:
		"""
	...
	async def get_analysis_statistics(self, user_id: Optional[UUID] = None, days: int = 30) -> Dict:
		"""
		...
		# Base query
		base_query = select(ContractAnalysis).where(ContractAnalysis.created_at >= date_threshold)

		if user_id:
			base_query = base_query.where(ContractAnalysis.user_id == user_id)

		# Total analyses
		total_result = await self.session.execute(select(func.count(ContractAnalysis.id)).select_from(base_query.subquery()))
		total_analyses = total_result.scalar()

		# Completed analyses
		completed_result = await self.session.execute(base_query.where(ContractAnalysis.analysis_status == "completed"))
		completed_analyses = len(completed_result.scalars().all())

		# Failed analyses
		failed_result = await self.session.execute(base_query.where(ContractAnalysis.analysis_status == "failed"))
		...
		avg_risk_result = await self.session.execute(
			select(func.avg(ContractAnalysis.risk_score)).select_from(base_query.subquery()).where(ContractAnalysis.analysis_status == "completed")
		)
	...
	async def get_high_risk_analyses(self, risk_threshold: float = 0.7, limit: Optional[int] = None) -> List[ContractAnalysis]:
		"""
		...
		query = (
			...
			.where(and_(ContractAnalysis.analysis_status == "completed", ContractAnalysis.risk_score >= risk_threshold))
	...
	async def cleanup_old_analyses(self, days_old: int = 90) -> int:
		...
		result = await self.session.execute(
			select(ContractAnalysis.id).where(
				and_(ContractAnalysis.created_at < cutoff_date, ContractAnalysis.analysis_status.in_(["completed", "failed"]))
			)
		)
		...
		if analysis_ids:
			...
			await self.session.execute(delete(ContractAnalysis).where(ContractAnalysis.id.in_(analysis_ids)))

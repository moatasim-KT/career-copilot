...
class UserRepository(BaseRepository[User]):
	...
	async def create_user(self, username: str, email: str, hashed_password: str, is_superuser: bool = False) -> User:
		"""
		Create a new user.
	...
	async def search_users(self, search_term: str, limit: Optional[int] = None, offset: Optional[int] = None) -> List[User]:
		...
		"""
		query = select(User).where(or_(User.username.ilike(f"%{search_term}%"), User.email.ilike(f"%{search_term}%")))
	...
	async def get_user_with_api_keys(self, user_id: UUID) -> Optional[User]:
		...
		"""
		result = await self.session.execute(select(User).options(selectinload(User.api_keys)).where(User.id == user_id))
		return result.scalar_one_or_none()
	...
	async def get_user_with_analyses(self, user_id: UUID) -> Optional[User]:
		...
		"""
		result = await self.session.execute(select(User).options(selectinload(User.contract_analyses)).where(User.id == user_id))
		return result.scalar_one_or_none()

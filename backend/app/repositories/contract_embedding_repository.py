...
class ContractEmbeddingRepository(BaseRepository[ContractEmbedding]):
    ...
    async def get_embeddings_by_contract(
        self,
        contract_analysis_id: UUID
        ...
        try:
            return await self.get_multi(
                filters={"contract_analysis_id": contract_analysis_id},
    ...
    async def delete_embeddings_by_contract(
        self,
        contract_analysis_id: UUID
    ) -> int:
    ...
    async def get_embeddings_by_type(
        self,
        contract_type: str,
        limit: int = 100
    ) -> List[ContractEmbedding]:
    ...
    async def get_embeddings_by_risk_level(
        self,
        risk_level: str,
        limit: int = 100
    ) -> List[ContractEmbedding]:
    ...
    async def get_embeddings_by_jurisdiction(
        self,
        jurisdiction: str,
        limit: int = 100
    ) -> List[ContractEmbedding]:
    ...
    async def get_embedding_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unique_contracts = self.db.query(
                func.count(func.distinct(ContractEmbedding.contract_analysis_id))
            ).scalar()
            ...
            contract_type_stats = self.db.query(
                ContractEmbedding.contract_type,
                func.count(ContractEmbedding.id).label('count')
            ).filter(
                ContractEmbedding.contract_type.isnot(None)
            ).group_by(
                ContractEmbedding.contract_type
            ).all()
            
            # Risk level distribution
            risk_level_stats = self.db.query(
                ContractEmbedding.risk_level,
                func.count(ContractEmbedding.id).label('count')
            ).filter(
                ContractEmbedding.risk_level.isnot(None)
            ).group_by(
                ContractEmbedding.risk_level
            ).all()
            
            # Jurisdiction distribution
            jurisdiction_stats = self.db.query(
                ContractEmbedding.jurisdiction,
                func.count(ContractEmbedding.id).label('count')
            ).filter(
                ContractEmbedding.jurisdiction.isnot(None)
            ).group_by(
                ContractEmbedding.jurisdiction
            ).all()
            ...
            avg_processing_time = self.db.query(
                func.avg(ContractEmbedding.processing_time_ms)
            ).filter(
                ContractEmbedding.processing_time_ms.isnot(None)
            ).scalar() or 0.0
            ...
            return {
                ...
                "contract_type_distribution": {
                    stat.contract_type: stat.count for stat in contract_type_stats
                },
                "risk_level_distribution": {
                    stat.risk_level: stat.count for stat in risk_level_stats
                },
                "jurisdiction_distribution": {
                    stat.jurisdiction: stat.count for stat in jurisdiction_stats
                },
    ...
    async def cleanup_orphaned_embeddings(self) -> int:
        ...
        try:
            ...
            orphaned_query = self.db.query(ContractEmbedding).filter(
                ~ContractEmbedding.contract_analysis_id.in_(
                    self.db.query(ContractAnalysis.id)
                )
            )

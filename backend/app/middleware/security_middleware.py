"""
def None  # get_jwt_token_manager(): return None
from ..core.auth import get_jwt_token_manager
...
from ..services.audit_trail_service import get_audit_trail_service, AuditAction, AuditResult
...
class ConsolidatedSecurityMiddleware(BaseHTTPMiddleware):
	...
	def __init__(self, app):
		super().__init__(app)
		self.jwt_manager = None  # get_jwt_token_manager()
		...
		self.ai_endpoints = ["/api/v1/ai/", "/api/v1/chat/", "/api/v1/completion/", "/api/v1/embedding/"]
		...
		try:
			...
			if self.rate_limiter.is_rate_limited(identifier, ip_address):
				await self.audit_service.log_request(
					...
					details={"ip_address": ip_address, "user_agent": user_agent, "path": str(request.url.path), "method": request.method},
				)
		...
		try:
			...
			if "multipart/form-data" in content_type or "application/octet-stream" in content_type:
				...
				if len(body) > 0:
					...
					scan_result = await self.malware_scanner.scan_file(body, "uploaded_file")
					...
					if scan_result.status in [ScanStatus.INFECTED, ScanStatus.SUSPICIOUS]:
						...
						raise HTTPException(
							status_code=status.HTTP_400_BAD_REQUEST, detail=f"Malicious content detected: {', '.join(scan_result.threats_found)}"
						)
			...
			if body:
				...
				if prompt:
					validation_result = await self.ai_security_manager.validate_prompt(
						prompt,
						context={"path": request.url.path, "method": request.method, "client": request.client.host if request.client else None},
					)
					...
					if validation_result.risk_level == "critical":
						...
						raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Request blocked due to AI security concerns")
					...
					if validation_result.risk_level == "high":
						self._log_ai_security_event("High-risk AI request detected", validation_result, request)
		...
		try:
			if response.status_code == 200:
				...
				if response_body:
					...
					if output:
						validation_result = await self.ai_security_manager.validate_output(output, model_info=response_body.get("model", {}))
						...
						if validation_result.detected_threats:
							...
							self._log_ai_security_event("Threats detected in AI response", validation_result, request)

					# Monitor model behavior
					body = await self._get_request_body(request)
					monitoring_result = self.ai_security_manager.monitor_model_behavior(
						model_id=response_body.get("model", {}).get("id", "unknown"), request_data=body or {}, response_data=response_body
					)

					# Log anomalies
					if monitoring_result.get("anomalies"):
						self._log_ai_security_event("AI model behavior anomalies detected", monitoring_result, request)
	...
	async def _authenticate_request(self, request: Request, context: SecurityContext) -> None:
		...
		try:
			...
			if not validation_result.is_valid:
				...
				raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=validation_result.error, headers={"WWW-Authenticate": "Bearer"})
		...
		try:
			...
			if required_permissions:
				...
				if not access_result.allowed:
					...
					raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Insufficient permissions: {access_result.reason}")
	...
	async def _log_request_success(self, request: Request, context: SecurityContext, response: Response, duration_ms: float) -> None:
	...
	async def _log_request_failure(self, request: Request, context: SecurityContext, exception: HTTPException, duration_ms: float) -> None:
		"""Log failed request."""
		try:
	...
	async def _log_request_error(self, request: Request, context: SecurityContext, exception: Exception, duration_ms: float) -> None:
		"""Log request error."""
		try:
			await self.audit_service.log_request(
				request=request,

...
class LoggingMiddleware(BaseHTTPMiddleware):
	...
	async def dispatch(self, request: Request, call_next: Callable) -> Response:
		...
		# Log request
		logger.info(f"Request started: {method} {url} - IP: {client_ip} - UA: {user_agent} - CID: {correlation_id}")
		...
		try:
			...
			# Log error
			logger.error(f"Request failed: {method} {url} - Error: {e!s} - Time: {round(process_time * 1000, 2)}ms - CID: {correlation_id}")

			# Log security event for errors
			audit_logger.warning(f"Security: Request error - {method} {url} - Error: {e!s} - IP: {client_ip}")

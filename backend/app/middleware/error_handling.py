...
class ConsolidatedErrorHandlingMiddleware(BaseHTTPMiddleware):
	...
	async def dispatch(self, request: Request, call_next: Callable):
		...
		try:
			...
			return JSONResponse(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, content={**error_context.to_dict(), "request_id": request_id})
			...
			logger.error(f"Connection error [{request_id}]: {e!s}")
			error_context = handle_network_error(error=e, endpoint=str(request.url), retry_available=True)
			return JSONResponse(
				status_code=status.HTTP_503_SERVICE_UNAVAILABLE, content={**error_context.to_dict(), "request_id": request_id, "retry_after": 30}
			)

		except TimeoutError as e:
			logger.error(f"Timeout error [{request_id}]: {e!s}")
			return JSONResponse(
...
async def http_exception_handler(request: Request, exc: Exception):
	...
	return JSONResponse(
		...
		content={"error": "HTTP Error", "message": getattr(exc, "detail", str(exc)), "request_id": request_id},
	)

...
class TaskQueueManager:
	...
	def submit_resume_parsing(self, resume_upload_id: int, file_path: str, filename: str, user_id: int, priority: str = "normal") -> str:
		"""
		...
		result = parse_resume_async.apply_async(args=[resume_upload_id, file_path, filename, user_id], **task_options)
	...
	def submit_cover_letter_generation(
		self, user_id: int, job_id: int, tone: str = "professional", custom_instructions: Optional[str] = None, priority: str = "normal"
	) -> str:
		...
		result = generate_cover_letter_async.apply_async(args=[user_id, job_id, tone, custom_instructions], **task_options)
	...
	def submit_resume_tailoring(self, user_id: int, job_id: int, resume_sections: Dict[str, str], priority: str = "normal") -> str:
		"""Submit resume tailoring task"""
		from app.tasks.content_generation_tasks import generate_resume_tailoring_async

		task_options = self._get_task_options(priority)

		result = generate_resume_tailoring_async.apply_async(args=[user_id, job_id, resume_sections], **task_options)
	...
	def submit_email_notification(self, user_id: int, subject: str, content: str, email_type: str = "general", priority: str = "normal") -> str:
		"""Submit email notification task"""
		from app.tasks.notification_tasks import send_email_async

		task_options = self._get_task_options(priority)

		result = send_email_async.apply_async(args=[user_id, subject, content, email_type], **task_options)

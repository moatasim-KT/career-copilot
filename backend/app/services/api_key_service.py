...
class APIKeyManager:
	...
	async def validate_api_key(
		self, api_key: str, required_permission: Optional[str] = None, ip_address: Optional[str] = None
		...
		try:
			...
			if not api_key.startswith(self.key_prefix):
				return APIKeyValidationResult(is_valid=False, error_message="Invalid API key format")
			...
			if not key_data:
				...
				return APIKeyValidationResult(is_valid=False, error_message="Invalid API key")

			# Check if key is active
			if not key_data.is_active:
				metrics.record_authentication_attempt("api_key", "inactive_key")
				return APIKeyValidationResult(is_valid=False, error_message="API key is inactive")
			...
			if key_data.expires_at and key_data.expires_at < datetime.now(timezone.utc):
				...
				return APIKeyValidationResult(is_valid=False, error_message="API key has expired")
			...
			if not rate_limit_result["allowed"]:
				...
				return APIKeyValidationResult(
					is_valid=False, rate_limit_remaining=0, rate_limit_reset=rate_limit_result["reset_time"], error_message="Rate limit exceeded"
				)
			...
			if required_permission and required_permission not in key_data.permissions:
				...
				return APIKeyValidationResult(is_valid=False, error_message="Insufficient permissions")

			# Get user data
			user_data = await self._get_user_data(key_data.user_id)
			if not user_data or not user_data.get("is_active"):
				metrics.record_authentication_attempt("api_key", "inactive_user")
				return APIKeyValidationResult(is_valid=False, error_message="User account is inactive")
			...
			return APIKeyValidationResult(is_valid=False, error_message="Internal validation error")
	...
	async def get_api_key_usage(self, key_id: str, user_id: int, hours: int = 24) -> Dict[str, Any]:
		"""
	...
	async def _check_rate_limit(self, key_data: APIKeyData, ip_address: Optional[str]) -> Dict[str, Any]:
		...
		try:
			...
			if current_usage >= key_data.rate_limit:
				return {"allowed": False, "remaining": 0, "reset_time": hour_start + timedelta(hours=1)}

			# Increment usage
			self.rate_limit_tracking[rate_key] += 1

			return {"allowed": True, "remaining": key_data.rate_limit - current_usage - 1, "reset_time": hour_start + timedelta(hours=1)}

...
class GROQModelRouter:
    ...
    def _get_candidate_models(
        ...
        if len(candidates) < 2:  # Ensure we have alternatives
            for model in routing_rule.fallback_models:
                if (model not in candidates and 
                    self._is_model_available(model, routing_rule, constraints)):
                    candidates.append(model)
    ...
    def _is_model_available(
        ...
        if metrics:
            if (metrics.avg_response_time > routing_rule.max_response_time or
                metrics.avg_quality_score < routing_rule.min_quality_score):
                return False
    ...
    async def _performance_based_routing(
        ...
        return RoutingDecision(
            ...
            estimated_time=self.performance_metrics.get(
                f"{best_model.value}:{task_type.value}", 
                ModelPerformanceMetrics(best_model, task_type)
            ).avg_response_time or 2.0,
    ...
    async def _cost_optimized_routing(
        ...
        candidates_with_cost = [
            (model, self.groq_service.model_configs[model].cost_per_token)
            for model in candidates
        ]
        ...
        return RoutingDecision(
            ...
            estimated_time=self.performance_metrics.get(
                f"{selected_model.value}:{task_type.value}",
                ModelPerformanceMetrics(selected_model, task_type)
            ).avg_response_time or 3.0,
    ...
    async def _load_balanced_routing(
        ...
        candidates_with_load = [
            (model, self.load_balancer_state.get(model, 0))
            for model in candidates
        ]
    ...
    async def _default_model_selection(
        self, 
        task_type: GROQTaskType, 
        ...
        if success:
            ...
            if metrics.total_requests == 1:
                ...
                metrics.avg_response_time = (
                    (1 - alpha) * metrics.avg_response_time + alpha * response_time
                )
                metrics.avg_quality_score = (
                    (1 - alpha) * metrics.avg_quality_score + alpha * quality_score
                )
                metrics.avg_cost = (
                    (1 - alpha) * metrics.avg_cost + alpha * cost
                )
    ...
    def get_routing_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "circuit_breaker_states": {
                m.value: cb.get("state", "closed") 
                for m, cb in self.circuit_breakers.items()
            },
            "performance_metrics": {}
        }

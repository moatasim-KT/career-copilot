...
class ResourceManager:
	...
	def check_resource_limits(self, task: AnalysisTask) -> bool:
		...
		try:
			...
			task.resource_usage.peak_memory_mb = max(task.resource_usage.peak_memory_mb, memory_mb)
			task.resource_usage.last_update = datetime.now(timezone.utc)
...
class EnhancedWorkflowService:
	...
	async def start_analysis(
		...
		if enable_progress:
			task.progress_updates.append(
				ProgressUpdate(timestamp=datetime.now(timezone.utc), stage="queued", progress_percent=0.0, message="Task queued for processing")
			)
		...
		try:
			...
			lb_priority = {TaskPriority.LOW: 2, TaskPriority.NORMAL: 5, TaskPriority.HIGH: 8, TaskPriority.URGENT: 10}.get(priority, 5)
			...
			await load_balancer.submit_request(
				data=task, priority=lb_priority, estimated_duration=estimated_duration, callback=self._load_balancer_callback
			)
	...
	async def _execute_analysis(self, task: AnalysisTask, worker_name: str):
		...
		try:
			...
			task.progress_updates.append(
				ProgressUpdate(
					timestamp=datetime.now(timezone.utc), stage="starting", progress_percent=10.0, message=f"Analysis started by {worker_name}"
				)
			)

			# Update progress tracker
			await progress_tracker.update_progress(task.task_id, ProgressStage.INITIALIZING, 10.0, f"Analysis started by {worker_name}")

			# Create workflow instance
			workflow = ContractAnalysisWorkflow()

			# Execute with timeout and resource monitoring
			result = await asyncio.wait_for(self._execute_with_monitoring(workflow, task), timeout=task.timeout_seconds)
			...
			metrics_collector.record_contract_analysis(
				status="completed", model_used=result.get("model_used", "unknown"), duration=duration, risk_score=risk_score
			)
			...
			await progress_tracker.complete_task(task.task_id, success=True, final_message="Analysis completed successfully")
			...
			duration = time.time() - start_time
			metrics_collector.record_contract_analysis(status="timeout", model_used="unknown", duration=duration)

			# Complete progress tracking with failure
			await progress_tracker.complete_task(
				task.task_id, success=False, final_message=f"Analysis timed out after {task.timeout_seconds} seconds"
			)
			...
			duration = time.time() - start_time
			metrics_collector.record_contract_analysis(status="failed", model_used="unknown", duration=duration)

			# Complete progress tracking with failure
			await progress_tracker.complete_task(task.task_id, success=False, final_message=f"Analysis failed: {str(e)}")
	...
	async def _execute_with_monitoring(self, workflow: ContractAnalysisWorkflow, task: AnalysisTask):
		...
		try:
			...
			await progress_tracker.update_progress(task.task_id, ProgressStage.PROCESSING_DOCUMENT, 20.0, "Processing document and extracting text")

			# AI Analysis stage
			task.progress_updates.append(
				ProgressUpdate(
					timestamp=datetime.now(timezone.utc), stage="analyzing", progress_percent=40.0, message="Analyzing contract for risky clauses"
				)
			)

			await progress_tracker.update_progress(task.task_id, ProgressStage.AI_ANALYSIS, 40.0, "Analyzing contract for risky clauses")

			# Execute workflow
			result = await workflow.execute(task.contract_text, task.contract_filename)

			# Risk identification stage
			await progress_tracker.update_progress(task.task_id, ProgressStage.IDENTIFYING_RISKS, 70.0, "Identifying and categorizing risk factors")

			# Redline generation stage
			await progress_tracker.update_progress(task.task_id, ProgressStage.GENERATING_REDLINES, 85.0, "Generating redline suggestions")

			# Final processing stage
			task.progress_updates.append(
				ProgressUpdate(timestamp=datetime.now(timezone.utc), stage="processing", progress_percent=95.0, message="Processing analysis results")
			)

			await progress_tracker.update_progress(task.task_id, ProgressStage.PREPARING_RESULTS, 95.0, "Processing analysis results")
	...
	async def _send_callback(self, task: AnalysisTask):
		...
		try:
			...
			async with httpx.AsyncClient() as client:
				response = await client.post(task.callback_url, json=callback_data, timeout=10)
	...
	async def get_task_status(self, task_id: str) -> Optional[Dict[str, Any]]:
		...
		if task.end_time:
			...
			processing_duration = (datetime.now(timezone.utc) - task.start_time).total_seconds()
	...
	async def cancel_task(self, task_id: str) -> bool:
		...
		# Add progress update
		task.progress_updates.append(
			ProgressUpdate(timestamp=datetime.now(timezone.utc), stage="cancelled", progress_percent=0.0, message="Task cancelled by user")
		)
		...
		try:
			# Lazy import to avoid circular dependency
			            ...
			            await notification_service.send_notification(
			                ...
			                notification_type="contract_analysis_complete",
			                title=f"Contract Analysis Complete: {task.contract_filename}",
			                message=analysis_summary,
			                data={
			                    "contract_name": task.contract_filename,
			            ...
			            if risk_score >= 7.0:
			                ...
			                await notification_service.send_notification(
			                    ...
			                    title=f"🚨 HIGH RISK ALERT: {task.contract_filename}",
			                    message="High risk clauses detected in your contract analysis.",
			                    data={
			                        "contract_name": task.contract_filename,
			                        "risk_score": risk_score,
			...
			try:
				if hasattr(metrics_collector, "record_metric"):
					metrics_collector.record_metric(
						"load_balancer_task_duration", processing_time, {"task_id": task_data.task_id, "priority": task_data.priority.value}
					)
	...
	async def _cleanup_old_tasks(self):
		...
		old_tasks = [task_id for task_id, task in self.completed_tasks.items() if task.end_time and task.end_time < cutoff_time]

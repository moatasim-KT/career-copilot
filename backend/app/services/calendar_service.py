...
class CalendarService:
	...
	def get_google_auth_url(self, user_id: int, redirect_uri: str) -> str:
		...
		try:
			...
			auth_url, state = flow.authorization_url(access_type="offline", include_granted_scopes="true", prompt="consent", state=str(user_id))
	...
	def update_google_event(self, access_token: str, refresh_token: str, event_id: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
	...
	def create_outlook_event(self, access_token: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
		...
		try:
			...
			# Add attendees if provided
			if "attendees" in event_data:
				event["attendees"] = [{"emailAddress": {"address": email}, "type": "required"} for email in event_data["attendees"]]
	...
	def update_outlook_event(self, access_token: str, event_id: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
		...
		try:
			...
			response = requests.patch(f"https://graph.microsoft.com/v1.0/me/events/{event_id}", headers=headers, json=update_data)

...
class MarketAnalysisService:
	...
	def analyze_salary_trends(self, db: Session, user_id: int, role_filter: Optional[str] = None, days: int = 180) -> Dict:
		...
		query = db.query(Job).filter(Job.user_id == user_id, Job.salary_min.isnot(None), Job.salary_max.isnot(None), Job.created_at >= cutoff_date)
		...
		salary_insights = self._generate_salary_insights(monthly_averages, location_analysis, industry_salaries, salary_growth_rate)
	...
	def analyze_job_market_patterns(self, db: Session, user_id: int, days: int = 90) -> Dict:
		...
		jobs = db.query(Job).filter(Job.user_id == user_id, Job.created_at >= cutoff_date).all()
		...
		market_insights = self._generate_market_pattern_insights(growth_metrics, industry_distribution, location_analysis, seasonal_patterns)
	...
	def create_market_dashboard_data(self, db: Session, user_id: int) -> Dict:
		...
		chart_data = {"salary_by_location": [], "jobs_over_time": [], "company_distribution": [], "employment_type_pie": []}

		# Salary by location chart
		if "by_location" in salary_trends:
			for location, data in salary_trends["by_location"].items():
				chart_data["salary_by_location"].append({"location": location, "median_salary": data["median"], "job_count": data["count"]})

		# Jobs over time chart
		if "weekly_distribution" in market_patterns:
			for week, count in market_patterns["weekly_distribution"].items():
				chart_data["jobs_over_time"].append({"week": week, "count": count})
		...
		if "employment_types" in market_patterns:
			for emp_type, count in market_patterns["employment_types"].items():
				chart_data["employment_type_pie"].append({"type": emp_type, "count": count})
	...
	def save_analysis(self, db: Session, user_id: int, analysis_data: Dict) -> bool:
		...
		try:
			analytics = Analytics(user_id=user_id, type="market_trends", data=analysis_data)
			db.add(analytics)
			db.commit()
			return True
	...
	def _analyze_job_sources(self, jobs: List[Job]) -> Dict:
		...
		for source, data in source_data.items():
			...
			data["recent_percentage"] = (
				round(data["recent_count"] / sum(d["recent_count"] for d in source_data.values()) * 100, 1)
				if sum(d["recent_count"] for d in source_data.values()) > 0
				else 0
			)
			data["trend"] = (
				"increasing"
				if data["recent_percentage"] > data["percentage"]
				else "decreasing"
				if data["recent_percentage"] < data["percentage"]
				else "stable"
			)
	...
	def _analyze_role_patterns(self, jobs: List[Job]) -> Dict:
		...
		return {"role_keywords": dict(role_keywords), "seniority_distribution": dict(seniority_levels)}
	...
	def _generate_salary_insights(
		self, monthly_averages: List[Dict], location_analysis: Dict, industry_salaries: Dict, growth_rate: float
	) -> List[str]:
	...
	def _generate_market_pattern_insights(
		self, growth_metrics: Dict, industry_distribution: Dict, location_analysis: Dict, seasonal_patterns: Dict
	) -> List[str]:

...
class EmailService:
	...
	def __init__(self, config: Optional[EmailServiceConfig] = None):
		...
		# Provider health tracking
		self.provider_health = {EmailProvider.SMTP: ProviderHealth(), EmailProvider.GMAIL: ProviderHealth(), EmailProvider.SENDGRID: ProviderHealth()}

		# Template management
		self.templates_dir = Path("backend/app/templates/email")
		self.jinja_env = Environment(
			loader=FileSystemLoader([str(self.templates_dir)]), autoescape=select_autoescape(["html", "xml"]), trim_blocks=True, lstrip_blocks=True
		)
	...
	def _extract_template_variables(self, *templates: str) -> List[str]:
		...
		system_vars = {"recipient_email", "tracking_id", "current_date", "current_datetime", "template_id", "template_name", "now", "today"}
...
class EmailService:
	...
	async def send_email(self, message: UnifiedEmailMessage, user_id: str = "default_user") -> Dict[str, Any]:
		...
		if not await self._check_rate_limits():
			raise EmailServiceError("Rate limit exceeded", category=ErrorCategory.RATE_LIMIT, severity=ErrorSeverity.HIGH)
		...
		if self.config.enable_analytics:
			await self._record_email_event(tracking_id, EmailEventType.SEND, message.to[0], EmailProvider.SMTP)
	...
	async def _record_email_event(
		self, tracking_id: str, event_type: EmailEventType, recipient: str, provider: EmailProvider, metadata: Optional[Dict[str, Any]] = None
	):
		"""Record email event for analytics"""
		event = EmailEvent(tracking_id=tracking_id, event_type=event_type, recipient=recipient, provider=provider, metadata=metadata or {})
		...
		while True:
			try:
				...
				for tracking_id, events in list(self.events.items()):
					self.events[tracking_id] = [e for e in events if e.timestamp > cutoff_date]
	...
	async def send_risk_alert_email(
		self, recipient_email: str, contract_name: str, risk_score: float, urgent_clauses: List[Dict[str, Any]], user_id: str = "default_user"
	) -> Dict[str, Any]:
	...
	async def get_email_metrics(
		self, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None, provider: Optional[EmailProvider] = None
	) -> Dict[str, Any]:
		...
		for events_list in self.events.values():
			filtered_events.extend([e for e in events_list if start_date <= e.timestamp <= end_date and (not provider or e.provider == provider)])
...
class EmailService:
	...
	async def test_all_providers(self) -> Dict[str, Any]:
		...
		logger.info("Testing all email providers...")
		# Simulate testing providers
		time.sleep(1)
		self.provider_health[EmailProvider.SMTP].last_success = datetime.now()
		self.provider_health[EmailProvider.SMTP].available = True
		time.sleep(1)
		self.provider_health[EmailProvider.GMAIL].last_failure = datetime.now()
		self.provider_health[EmailProvider.GMAIL].available = False
		self.provider_health[EmailProvider.GMAIL].failure_count += 1
		time.sleep(1)
		self.provider_health[EmailProvider.SENDGRID].last_success = datetime.now()
		self.provider_health[EmailProvider.SENDGRID].available = True
		...
		return {
			...
			"detailed_results": {
				...
				"gmail": {"status": "failed", "message": "Gmail connection failed"},
	...
	async def get_service_status(self) -> Dict[str, Any]:
		...
		return {
			...
			"provider_health": {provider.value: health.available for provider, health in self.provider_health.items()},
			"configuration": {
...
# Create singleton instance for use in tasks

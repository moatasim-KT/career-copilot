...
from app.models.feedback import (
    Feedback, FeedbackVote, OnboardingProgress, 
    HelpArticle, HelpArticleVote, FeedbackStatus
)
from app.models.user import User
from app.schemas.feedback import (
    FeedbackCreate, FeedbackUpdate, FeedbackVoteCreate,
    OnboardingProgressUpdate, HelpArticleCreate, HelpArticleUpdate,
    HelpArticleVoteCreate, HelpSearchRequest
)
...
class FeedbackService:
    ...
    def get_feedback_with_votes(self, feedback_id: int, user_id: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        vote_count = self.db.query(func.sum(FeedbackVote.vote)).filter(
            FeedbackVote.feedback_id == feedback_id
        ).scalar() or 0
        ...
        user_vote = None
        if user_id:
            user_vote_obj = self.db.query(FeedbackVote).filter(
                and_(
                    FeedbackVote.feedback_id == feedback_id,
                    FeedbackVote.user_id == user_id
                )
...
class HelpService:
    ...
    def get_article_by_slug(self, slug: str) -> Optional[HelpArticle]:
        ...
        article = self.db.query(HelpArticle).filter(
            and_(HelpArticle.slug == slug, HelpArticle.is_published == True)
        ).first()
    ...
    def vote_on_article(
        ...
        article_id: int, 
        user_id: int, 
        vote_data: HelpArticleVoteCreate
    ) -> bool:
        """Vote on help article"""
        # Check if user already voted
        existing_vote = self.db.query(HelpArticleVote).filter(
            and_(
                HelpArticleVote.article_id == article_id,
                HelpArticleVote.user_id == user_id
            )
        ).first()
    ...
    def get_article_with_vote(self, article_id: int, user_id: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        user_vote = None
        if user_id:
            vote_obj = self.db.query(HelpArticleVote).filter(
                and_(
                    HelpArticleVote.article_id == article_id,
                    HelpArticleVote.user_id == user_id
                )
    ...
    def get_categories(self) -> List[str]:
        ...
        categories = self.db.query(HelpArticle.category).filter(

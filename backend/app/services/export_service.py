...
class ExportService:
	...
	async def export_applications_json(
		...
		try:
			...
			query = select(Application).options(selectinload(Application.job)).where(Application.user_id == user_id)
			...
			# Get total count
			count_result = await db.execute(select(Application.id).where(Application.user_id == user_id))
			total_count = len(count_result.all())
	...
	async def create_full_backup(
		...
		try:
			...
			apps_result = await db.execute(select(Application).options(selectinload(Application.job)).where(Application.user_id == user_id))

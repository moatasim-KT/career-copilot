	...
	async def call(self, func: Callable, *args, **kwargs):
		...
		if self.state == CircuitState.OPEN:
			if self._should_attempt_reset():
				...
				raise ExternalServiceError("Circuit breaker is OPEN - service unavailable", service_name=kwargs.get("service_name", "unknown"))
...
class ExternalServiceManager:
	...
	def _initialize_default_configs(self):
		...
		default_retry = RetryConfig(max_attempts=3, base_delay=1.0, max_delay=30.0, exponential_base=2.0, jitter=True)

		# Service-specific configurations
		self.retry_configs = {
			"docusign": RetryConfig(max_attempts=3, base_delay=2.0, max_delay=60.0, retry_on_status_codes=[429, 502, 503, 504]),
			"slack": RetryConfig(max_attempts=2, base_delay=1.0, max_delay=10.0, retry_on_status_codes=[429, 502, 503]),
			"gmail": RetryConfig(max_attempts=3, base_delay=1.0, max_delay=30.0, retry_on_status_codes=[429, 500, 502, 503, 504]),
			"google_drive": RetryConfig(max_attempts=3, base_delay=1.0, max_delay=30.0, retry_on_status_codes=[429, 500, 502, 503, 504]),
			"vector_store": default_retry,
		}

		# Circuit breaker configurations
		circuit_config = CircuitBreakerConfig(failure_threshold=5, recovery_timeout=60, expected_exception=ExternalServiceError)
	...
	def _initialize_services(self):
		...
		for service_name in services:
			self.service_health[service_name] = ServiceHealth(
				service_name=service_name, status=ServiceStatus.HEALTHY, last_check=datetime.now(timezone.utc)
			)
	...
	async def _execute_request(
		...
		for attempt in range(retry_config.max_attempts):
			try:
				...
				async with httpx.AsyncClient(timeout=timeout) as client:
					response = await client.request(method=method, url=url, headers=request_headers, json=json_data, data=data, params=params)
		...
		if isinstance(last_exception, httpx.TimeoutException):
			raise NetworkError(f"Request to {service_name} timed out after {retry_config.max_attempts} attempts", operation=f"{method} {url}")
		elif isinstance(last_exception, httpx.ConnectError):
			raise NetworkError(f"Failed to connect to {service_name} after {retry_config.max_attempts} attempts", operation=f"{method} {url}")
		else:
			raise ExternalServiceError(
				f"Request to {service_name} failed after {retry_config.max_attempts} attempts: {last_exception!s}", service_name=service_name
			)
	...
	def _update_service_health(self, service_name: str, success: bool, response_time: Optional[float] = None, error: Optional[str] = None):
	...
	async def _health_check_service(self, service_name: str):
		...
		try:
			...
			self._update_service_health(service_name, success, response_time, f"HTTP {response.status_code}" if not success else None)

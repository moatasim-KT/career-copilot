...
class LocalPDFSigningService:
	...
	def __init__(self):
		...
		logger.info(f"Enhanced PDF signing service initialized: enabled={self.enabled}, crypto={CRYPTOGRAPHY_AVAILABLE}, pypdf2={PYPDF2_AVAILABLE}")
		...
		try:
			...
			if not default_cert_path.exists() or not default_key_path.exists():
				...
				cert_pem, key_pem = self._generate_self_signed_certificate(
					subject_name="Career Copilot PDF Signing", organization="Career Copilot", country="US"
				)
	...
	def _generate_self_signed_certificate(self, subject_name: str, organization: str, country: str, validity_days: int = 365) -> Tuple[str, str]:
	...
	async def sign_pdf_document(self, signing_request: SigningRequest) -> SigningResult:
		...
		try:
			if not self.enabled:
				return SigningResult(success=False, errors=["PDF signing service is disabled"])

			# Validate request
			validation_result = await self._validate_signing_request(signing_request)
			if not validation_result["valid"]:
				return SigningResult(success=False, errors=validation_result["errors"], warnings=validation_result["warnings"])
			...
			if file_size_mb > self.max_document_size_mb:
				return SigningResult(success=False, errors=[f"Document too large: {file_size_mb:.1f}MB > {self.max_document_size_mb}MB"])
			...
			for i, signature in enumerate(signing_request.signatures):
				try:
					...
					return SigningResult(
						success=False, errors=[f"Failed to add signature for {signature.signer_name}: {e!s}"], signature_ids=signature_ids
					)

			# Add signature page if requested
			if signing_request.add_signature_page:
				signature_page = await self.create_signature_page(
					signing_request.signatures, f"Signatures for {Path(signing_request.document_path).stem}"
				)
			...
			return SigningResult(success=True, signed_document_path=output_path, signature_ids=signature_ids, metadata=metadata)

		except Exception as e:
			error_msg = f"Failed to sign PDF document: {e!s}"
			logger.error(error_msg)
			return SigningResult(success=False, errors=[error_msg])
		...
		try:
			...
			if signature.signature_image_data:
				try:
					...
					c.drawImage(ImageReader(image_buffer), x + 5, y + 5, width=signature.width - 10, height=signature.height - 20)
				except Exception as e:
				...
				try:
					c.drawImage(signature.signature_image_path, x + 5, y + 5, width=signature.width - 10, height=signature.height - 20)
				except Exception as e:
	...
	async def validate_certificate(self, cert_path: str, ca_cert_path: Optional[str] = None) -> Dict[str, Any]:
		...
		try:
			result = {"valid": False, "errors": [], "warnings": [], "info": None}
			...
			if not cert_info.is_valid:
				if cert_info.not_before > datetime.now(timezone.utc):
			...
			return {"valid": False, "errors": [f"Certificate validation error: {e}"], "warnings": [], "info": None}

	async def create_signing_certificate(
		self, subject_name: str, organization: str, email: str, country: str = "US", validity_days: int = 365
	) -> Dict[str, Any]:
		...
		try:
			...
			cert_pem, key_pem = self._generate_self_signed_certificate(
				subject_name=subject_name, organization=organization, country=country, validity_days=validity_days
			)
		...
		try:
			...
			for cert_file in self.certificates_dir.glob("*.crt"):
				try:
					...
					if cert_info:
						certificates.append({"file_path": str(cert_file), "file_name": cert_file.name, "certificate_info": cert_info.dict()})
				except Exception as e:
	...
	async def create_signing_session(self, document_path: str, signers: List[Dict[str, Any]], expires_in_hours: int = 24) -> Dict[str, Any]:
	...
	async def sign_document_in_session(self, session_id: str, signer_email: str, signature_data: Dict[str, Any]) -> Dict[str, Any]:
		"""Sign a document within a signing session"""
		try:
			session = await self.get_signing_session(session_id)
			if not session:
				return {"success": False, "error": "Signing session not found"}

			if session["status"] == "expired":
				return {"success": False, "error": "Signing session has expired"}
			...
			if not signer:
				return {"success": False, "error": "Signer not found in session"}

			if signer.get("signed", False):
				return {"success": False, "error": "Document already signed by this signer"}
			...
			signing_request = SigningRequest(
				document_id=session_id, document_path=session["document_path"], signatures=[signature_info], signing_mode="overlay"
			)
		...
		try:
			...
			if session:
				return DocumentStatus(
					...
					signatures_completed=session["signatures_completed"],
					signers=[
						{"name": s["name"], "email": s["email"], "signed": s.get("signed", False), "signed_at": s.get("signed_at")}
						for s in session["signers"]
					],
	...
	async def test_connection(self) -> Dict[str, Any]:
		...
		try:
			result = {
				...
				"dependencies": {"cryptography": CRYPTOGRAPHY_AVAILABLE, "pypdf2": PYPDF2_AVAILABLE},
				"features": [],
				"warnings": [],
				"errors": [],
			}
			...
			# Check directories
			for dir_name, dir_path in [("signatures", self.signatures_dir), ("certificates", self.certificates_dir), ("temp", self.temp_dir)]:
			...
			try:
				...
				test_signatures = [
					SignatureInfo(
						signer_name="Test Signer", signer_email="test@example.com", signature_text="Test Signature", signing_reason="Connection Test"
					)
				]
			...
			return {"success": False, "service": "Local PDF Signing", "error": f"Connection test failed: {e}", "enabled": self.enabled}
	...
	async def get_service_info(self) -> Dict[str, Any]:
		...
		try:
			return {
				...
				"directories": {"signatures": str(self.signatures_dir), "certificates": str(self.certificates_dir), "temp": str(self.temp_dir)},
				"certificate_info": {"default_cert_path": self.cert_path, "default_key_path": self.key_path, "ca_cert_path": self.ca_cert_path},
			}

		except Exception as e:
			logger.error(f"Failed to get service info: {e}")
			return {"service_name": "Local PDF Signing Service", "error": str(e)}
	...
	async def sign_document(self, document_path: str, signature_data: Dict[str, Any]) -> Optional[str]:
		...
		try:
			...
			signing_request = SigningRequest(
				document_id="legacy_sign", document_path=document_path, signatures=[signature_info], signing_mode="overlay"
			)

...
class GracefulDegradationService:
	...
	def handle_job_recommendation_failure(self, db: Session, user_id: int) -> List[Dict]:
		...
		try:
			...
			jobs = db.query(Job).filter(Job.user_id == user_id, Job.status == "not_applied").all()
	...
	def _calculate_local_job_score(self, job: Job, user_skills: List[str], user_locations: List[str]) -> float:
		...
		if user_skills and job.requirements:
			job_skills = self.handle_skill_extraction_failure(job.description or str(job.requirements))
		...
		if user_locations and job.location:
			location_match = any(loc.lower() in job.location.lower() or "remote" in job.location.lower() for loc in user_locations)
			if location_match:
				score += 0.3
	...
	def handle_market_analysis_failure(self, db: Session, user_id: int) -> Dict:
		...
		try:
			...
			if not jobs:
				return {"fallback": True, "message": "Insufficient data for market analysis", "trends": {}}
			...
			# Salary analysis
			salaries = [(job.salary_min + job.salary_max) / 2 for job in jobs if job.salary_min and job.salary_max]
			...
			return {"fallback": True, "error": "Market analysis temporarily unavailable", "trends": {}}
	...
	def handle_job_ingestion_failure(self, db: Session, user_id: int) -> Dict:
		...
		try:
			...
			return {"fallback": True, "error": "Job discovery temporarily unavailable"}
	...
	def _generate_search_suggestions(self, preferences: Dict) -> List[str]:
		...
		if not suggestions:
			suggestions = ["Search for roles matching your skills", "Look for remote opportunities", "Check startup job boards like AngelList"]
	...
	def check_api_health(self, api_name: str) -> Dict:
		...
		try:
			...
			status = {"api": api_name, "status": "degraded", "last_check": datetime.now().isoformat(), "fallback_available": True}
			...
			return {"api": api_name, "status": "unavailable", "error": str(e), "fallback_available": True}
	...
	def handle_complete_offline_mode(self, db: Session, user_id: int) -> Dict:
		...
		try:
			...
			return {"success": True, "offline_data": offline_data, "message": "System operating in complete offline mode"}
	...
	def _get_cached_jobs(self, db: Session, user_id: int) -> List[Dict]:
		...
		try:
			jobs = db.query(Job).filter(Job.user_id == user_id).order_by(Job.created_at.desc()).limit(100).all()

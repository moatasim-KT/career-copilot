...
class VectorStoreService:
	...
	async def store_contract_embeddings(
		self, contract_id: str, contract_text: str, metadata: dict[str, Any], chunk_size: int | None = None, overlap: int | None = None
	...
	async def search_similar_contracts(self, query: SimilaritySearchQuery) -> list[SimilaritySearchResult]:
		...
		try:
			...
			if query.date_range:
				...
				where_clause["created_at"] = {"$gte": start_date.isoformat(), "$lte": end_date.isoformat()}
	...
	async def search_legal_precedents(
		self, query_text: str, max_results: int = 5, similarity_threshold: float = 0.7, jurisdiction: str | None = None
		...
		try:
			...
			# Perform similarity search
			results = collection.query(
				query_texts=[query_text], n_results=max_results, where=where_clause, include=["documents", "metadatas", "distances"]
			)
	...
	async def batch_store_embeddings(self, request: BatchEmbeddingRequest) -> BatchEmbeddingResult:
		...
		try:
			...
			for i in range(0, total_contracts, request.batch_size):
				...
				for contract_data in batch:
					task = self._process_single_contract(contract_data, request.chunk_size, request.overlap_size)
				...
				for j, result in enumerate(batch_results):
					if isinstance(result, Exception):
						failed_contracts += 1
						errors.append({"contract_index": i + j, "contract_id": batch[j].get("contract_id", "unknown"), "error": str(result)})
					else:
			...
			logger.info(
				f"Batch embedding completed: {processed_contracts}/{total_contracts} contracts, {total_embeddings} embeddings in {processing_time:.2f}s"
			)
	...
	async def _process_single_contract(self, contract_data: dict[str, Any], chunk_size: int, overlap_size: int) -> list[ContractEmbedding]:
		...
		return await self.store_contract_embeddings(
			contract_id=contract_id, contract_text=contract_text, metadata=metadata, chunk_size=chunk_size, overlap=overlap_size
		)
	...
	async def delete_contract_embeddings(self, contract_id: str) -> bool:
		...
		try:
			...
			results = collection.get(where={"contract_id": contract_id}, include=["metadatas"])
	...
	async def health_check(self) -> dict[str, Any]:
		...
		try:
			...
			test_query = SimilaritySearchQuery(query_text="test query", max_results=1)

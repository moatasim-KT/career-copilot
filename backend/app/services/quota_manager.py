...
class QuotaManager:
	...
	def _initialize_quota_limits(self) -> Dict[str, QuotaLimit]:
		...
		return {
			...
			"usajobs": QuotaLimit(daily_limit=1000, hourly_limit=100, rate_limit_seconds=0.5, burst_limit=10),
			"github_jobs": QuotaLimit(daily_limit=500, hourly_limit=50, rate_limit_seconds=1.0, burst_limit=5),
			"remoteok": QuotaLimit(daily_limit=100, hourly_limit=20, rate_limit_seconds=3.0, burst_limit=2),
			"jobspresso": QuotaLimit(daily_limit=100, rate_limit_seconds=2.0, burst_limit=3),
			# RSS feeds (more lenient)
			"rss_feeds": QuotaLimit(daily_limit=1000, hourly_limit=100, rate_limit_seconds=0.5, burst_limit=20),
			# Web scraping (most restrictive)
			"indeed_scraper": QuotaLimit(daily_limit=200, hourly_limit=25, rate_limit_seconds=5.0, burst_limit=2),
		}
	...
	def _initialize_fallback_chains(self) -> Dict[str, List[str]]:
		...
		return {
			...
			"enterprise_jobs": ["adzuna", "usajobs", "indeed_scraper"],
		}
	...
	def optimize_source_selection(self, job_category: str, keywords: str, location: str = "") -> List[str]:

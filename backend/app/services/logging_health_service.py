...
class LoggingHealthService:
	...
	def check_log_directories(self) -> Dict[str, Any]:
		...
		for name, path in self.log_paths.items():
			if "dir" in name:
				status = {"exists": path.exists(), "is_directory": path.is_dir() if path.exists() else False, "writable": False, "readable": False}
	...
	def test_logging_functionality(self) -> Dict[str, Any]:
		...
		try:
			...
			test_results["standard_logging"] = {"success": False, "error": str(e)}
		...
		try:
			...
			test_results["audit_logging"] = {"success": False, "error": str(e)}
		...
		for name, path in self.log_paths.items():
			if "dir" in name and not path.exists():
				try:
					...
					creation_results[name] = {"success": True, "message": f"Created directory {path}"}
					logger.info(f"Created missing log directory: {path}")
				except Exception as e:
					creation_results[name] = {"success": False, "error": str(e)}

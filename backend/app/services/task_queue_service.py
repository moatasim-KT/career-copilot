...
class TaskQueue:
	...
	def __init__(self):
		self.queues = {TaskPriority.URGENT: deque(), TaskPriority.HIGH: deque(), TaskPriority.NORMAL: deque(), TaskPriority.LOW: deque()}
...
class TaskWorker:
	...
	async def _execute_task(self, task: TaskDefinition, task_storage: TaskStorage):
		...
		if not result:
			result = TaskResult(task_id=task.task_id, status=TaskStatus.RUNNING, started_at=datetime.now(timezone.utc), max_retries=task.max_retries)
		else:
			# Update for retry
		...
		try:
			...
			task_result = await asyncio.wait_for(handler(task, task_storage), timeout=task.timeout_seconds)
	...
	async def _handle_task_failure(
		self, task: TaskDefinition, result: TaskResult, progress: TaskProgress, task_storage: TaskStorage, exception: Exception, failure_type: str
	):
	...
	async def _schedule_task_retry(
		self, task: TaskDefinition, result: TaskResult, progress: TaskProgress, task_storage: TaskStorage, exception: Exception
	):
		...
		# Set next retry time
		result.next_retry_at = datetime.now(timezone.utc) + timedelta(seconds=delay)
	...
	async def _handle_task_cancellation(self, task: TaskDefinition, result: TaskResult, progress: TaskProgress, task_storage: TaskStorage):
		"""Handle task cancellation."""
		result.status = TaskStatus.CANCELLED
		result.completed_at = datetime.now(timezone.utc)
		result.cancelled_at = datetime.now(timezone.utc)
	...
	async def _send_task_notifications(self, task: TaskDefinition, result: TaskResult):
		...
		try:
			...
			if result.status == TaskStatus.COMPLETED:
				            await notification_service.send_notification(user_id=task.user_id, notification_type="task_completion", title=f"Task {task.task_type.value} Completed", message=f"Your task {task.task_type.value} (ID: {task.task_id}) has completed successfully.")			elif result.status in [TaskStatus.FAILED, TaskStatus.TIMEOUT]:
				            await notification_service.send_notification(user_id=task.user_id, notification_type="task_failure", title=f"Task {task.task_type.value} Failed", message=f"Your task {task.task_type.value} (ID: {task.task_id}) has failed. Error: {result.error}")			elif result.status == TaskStatus.CANCELLED:
				            await notification_service.send_notification(user_id=task.user_id, notification_type="task_cancellation", title=f"Task {task.task_type.value} Cancelled", message=f"Your task {task.task_type.value} (ID: {task.task_id}) has been cancelled.")
		except Exception as e:
...
class TaskQueueService:
	...
	def __init__(self, max_workers: int = 5):
		...
		# Statistics
		self.stats = {"tasks_processed": 0, "tasks_completed": 0, "tasks_failed": 0, "total_execution_time": 0.0}
	...
	async def schedule_task(self, task: TaskDefinition) -> str:
		...
		result = TaskResult(task_id=task.task_id, status=TaskStatus.PENDING, created_at=task.created_at)
	...
	async def get_queue_stats(self) -> TaskQueueStats:
		...
		completed_tasks = [r for r in self.task_storage.results.values() if r.status == TaskStatus.COMPLETED and r.execution_time]

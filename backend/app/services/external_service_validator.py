...
class ExternalServiceValidator:
	...
	def _validate_docusign_configuration(self) -> ValidationResult:
		...
		return ValidationResult(
			service_name="docusign", test_name="configuration_check", status=ValidationStatus.PASSED, message="DocuSign configuration is valid"
		)
	...
	def _validate_slack_configuration(self) -> ValidationResult:
		...
		if not webhook_url and not bot_token:
			return ValidationResult(
				...
				suggestions=["Set SLACK_WEBHOOK_URL for webhook integration", "Or set SLACK_BOT_TOKEN for bot integration"],
			)

		config_details = {"webhook_configured": bool(webhook_url), "bot_token_configured": bool(bot_token)}
	...
	async def _validate_gmail_service(self) -> ServiceValidationReport:
		...
		if config_result.status == ValidationStatus.NOT_CONFIGURED:
			return self._create_service_report(service_name, results)
		        ...
		        # Authentication test
		        auth_result = await self._test_gmail_authentication(email_service)
		        results.append(auth_result)
		
		        # Template engine test
		        template_result = self._test_gmail_template_engine(email_service)
	...
	def _validate_gmail_configuration(self) -> ValidationResult:
		...
		return ValidationResult(
			service_name="gmail", test_name="configuration_check", status=ValidationStatus.PASSED, message="Gmail configuration is valid"
		)
	    ...
	    async def _test_gmail_authentication(self, service: EmailService) -> ValidationResult:
	        ...
	        try:
	            # Test authentication
	            auth_result = await service.test_authentication()
	            response_time = (time.time() - start_time) * 1000
	...
	def _generate_validation_summary(self):
		...
		healthy_services = sum(1 for report in self.validation_results.values() if report.overall_status == ValidationStatus.PASSED)
		...
		for service_name, report in self.validation_results.items():
			logger.info(f"  {service_name}: {report.overall_status.value} ({report.passed_tests}/{report.total_tests} tests passed)")
	...
	def get_validation_summary(self) -> Dict[str, Any]:
		...
		for status in ValidationStatus:
			status_counts[status.value] = sum(1 for report in self.validation_results.values() if report.overall_status == status)

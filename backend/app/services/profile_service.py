...
from app.schemas.profile import (
    ...
    ApplicationHistoryItem, ApplicationHistoryResponse,
    ProgressTrackingStats, DocumentSummary, DocumentManagementResponse
)
from app.core.cache import user_profile_cache, cached, cache_invalidate
...
class ProfileService:
    ...
    def get_application_history(
        ...
        query = self.db.query(
            ...
            JobApplication.response_date,
            JobApplication.notes,
            JobApplication.documents,
            JobApplication.application_metadata,
        ...
        if date_from:
            query = query.filter(JobApplication.applied_at >= date_from)
        
        if date_to:
            query = query.filter(JobApplication.applied_at <= date_to)
        
        # Get total count
        total_count = query.count()
        
        # Apply pagination and ordering
        applications_data = query.order_by(desc(JobApplication.applied_at)).offset(
            (page - 1) * per_page
        ).limit(per_page).all()
    ...
    def get_progress_tracking_stats(self, user_id: int) -> ProgressTrackingStats:
        ...
        applications_with_response = self.db.query(JobApplication).filter(
            and_(
                JobApplication.user_id == user_id,
        ...
        interviews = self.db.query(JobApplication).filter(
            and_(
                JobApplication.user_id == user_id,
        ...
        offers = self.db.query(JobApplication).filter(
            and_(
                JobApplication.user_id == user_id,
                JobApplication.status.in_(["offer_received", "offer_accepted"])
            )
        ).count()
        ...
        response_times = self.db.query(
            func.extract('epoch', JobApplication.response_date - JobApplication.applied_at) / 86400
        ).filter(
            and_(
                JobApplication.user_id == user_id,
                JobApplication.response_date.isnot(None)
        ...
        weekly_application_goal = self.db.query(Goal).filter(
            and_(
                Goal.user_id == user_id,
                Goal.is_active == True,
                Goal.goal_type == "weekly_applications"
            )
        ).first()
        ...
        active_goals_count = self.db.query(Goal).filter(
            and_(Goal.user_id == user_id, Goal.is_active == True)
        ).count()
        
        completed_goals_this_month = self.db.query(Goal).filter(
            and_(
                Goal.user_id == user_id,
        ...
        milestones_achieved_this_month = self.db.query(Milestone).filter(
            and_(
                Milestone.user_id == user_id,
                Milestone.achievement_date >= month_start
            )
        ).count()
    ...
    def _calculate_application_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        interviews_scheduled = self.db.query(JobApplication).filter(
            and_(
                JobApplication.user_id == user_id,
                JobApplication.status.in_([
                    "phone_screen_scheduled", "interview_scheduled", "final_round"
                ])
            )
        ).count()
        
        offers_received = self.db.query(JobApplication).filter(
            and_(
                JobApplication.user_id == user_id,
                JobApplication.status.in_(["offer_received", "offer_accepted"])
            )

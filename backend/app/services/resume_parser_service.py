...
class ResumeParserService:
    ...
    def __init__(self):
        ...
        self.experience_patterns = {
            "junior": [
                r"\b(?:junior|entry.?level|0-2\s+years?|intern|graduate|new\s+grad)\b"
            ],
            "mid": [r"\b(?:mid.?level|intermediate|2-5\s+years?|3-7\s+years?)\b"],
            "senior": [
                r"\b(?:senior|lead|principal|architect|5\+\s+years?|7\+\s+years?|expert)\b"
            ],
        }

    async def parse_resume(
        self, file_path: str, filename: str, user_id: int
    ) -> Dict[str, Any]:
        """
    ...
    async def suggest_profile_updates(
        self, parsed_data: Dict[str, Any], current_profile: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            # Suggest new skills
            current_skills = set(
                skill.lower() for skill in current_profile.get("skills", [])
            )
            parsed_skills = set(
                skill.lower() for skill in parsed_data.get("skills", [])
            )
        ...
        try:
            ...
            try:
                ...
                if len(text.strip()) < 50:
                    return (
                        False,
                        "File appears to be empty or contains insufficient text",
                    )
            except Exception as e:

...
class GoalService:
	...
	def get_user_goals(self, user_id: int, active_only: bool = False, goal_type: Optional[str] = None) -> List[GoalResponse]:
	...
	def get_goal(self, goal_id: int, user_id: int) -> Optional[GoalResponse]:
		...
		goal = self.db.query(Goal).filter(and_(Goal.id == goal_id, Goal.user_id == user_id)).first()
	...
	def add_progress(self, goal_id: int, user_id: int, progress_data: GoalProgressCreate) -> Optional[GoalProgressResponse]:
		...
		goal = self.db.query(Goal).filter(and_(Goal.id == goal_id, Goal.user_id == user_id, Goal.is_active == True)).first()
		...
		progress_entry = GoalProgress(
			...
			details={"activities": progress_data.activities, "mood": progress_data.mood, "challenges": progress_data.challenges},
		)
	...
	def create_milestone(self, user_id: int, milestone_data: MilestoneCreate) -> MilestoneResponse:
		...
		milestone = Milestone(
			...
			achievement_date=datetime.now(timezone.utc),
	...
	def get_progress_celebration(self, user_id: int) -> Optional[ProgressCelebration]:
		...
		uncelebrated_milestone = (
			self.db.query(Milestone)
			.filter(
				and_(
					Milestone.user_id == user_id,
					Milestone.is_celebrated == False,
					Milestone.achievement_date >= datetime.now(timezone.utc) - timedelta(days=7),
				)
			)
			.order_by(desc(Milestone.achievement_date))
			.first()
		)
		...
		if uncelebrated_milestone:
			...
			uncelebrated_milestone.celebrated_at = datetime.now(timezone.utc)
			self.db.commit()
	...
	def get_weekly_summary(self, user_id: int, week_offset: int = 0) -> WeeklyGoalSummary:
		...
		active_goals = (
			self.db.query(Goal)
			.filter(and_(Goal.user_id == user_id, Goal.start_date <= week_end, or_(Goal.end_date >= week_start, Goal.is_active == True)))
			.all()
		)
		...
		progress_entries = (
			...
			.filter(and_(GoalProgress.user_id == user_id, GoalProgress.progress_date >= week_start, GoalProgress.progress_date <= week_end))
			.all()
		)
		...
		goals_completed = len(
			[
				...
				if goal.is_completed and goal.completed_at and goal.completed_at.date() >= week_start and goal.completed_at.date() <= week_end
			]
		)
	...
	def auto_track_application_progress(self, user_id: int, application_id: int) -> None:
		...
		for goal in application_goals:
			...
			progress_data = GoalProgressCreate(
				...
				source="automatic",
				activities=[{"type": "job_application", "application_id": application_id, "timestamp": datetime.now(timezone.utc).isoformat()}],
			)
	...
	def _goal_to_response(self, goal: Goal) -> GoalResponse:
		...
		recent_progress = (
			self.db.query(GoalProgress)
			.filter(and_(GoalProgress.goal_id == goal.id, GoalProgress.progress_date >= date.today() - timedelta(days=7)))
	...
	def _calculate_goal_summary_stats(self, user_id: int) -> GoalSummaryStats:
		...
		active_goals = self.db.query(Goal).filter(and_(Goal.user_id == user_id, Goal.is_active == True)).count()
		completed_goals = self.db.query(Goal).filter(and_(Goal.user_id == user_id, Goal.is_completed == True)).count()

		# Overdue goals
		overdue_goals = (
			self.db.query(Goal)
			.filter(and_(Goal.user_id == user_id, Goal.is_active == True, Goal.is_completed == False, Goal.end_date < date.today()))
			.count()
		)
		...
		recent_milestones = (
			self.db.query(Milestone)
			.filter(and_(Milestone.user_id == user_id, Milestone.achievement_date >= datetime.now(timezone.utc) - timedelta(days=30)))
			.count()
		)
	...
	def _get_recent_milestones(self, user_id: int, days: int = 30) -> List[MilestoneResponse]:
		...
		milestones = (
			self.db.query(Milestone)
			.filter(and_(Milestone.user_id == user_id, Milestone.achievement_date >= datetime.now(timezone.utc) - timedelta(days=days)))
			.order_by(desc(Milestone.achievement_date))
			.limit(10)
			.all()
		)
	...
	def _get_weekly_progress_chart(self, user_id: int, weeks: int = 12) -> List[Dict[str, Any]]:
		...
		for i in range(weeks):
			...
			progress_count = (
				self.db.query(func.sum(GoalProgress.value_added))
				.filter(and_(GoalProgress.user_id == user_id, GoalProgress.progress_date >= week_start, GoalProgress.progress_date <= week_end))
				.scalar()
				or 0
			)

			chart_data.append({"week_start": week_start.strftime("%Y-%m-%d"), "week_end": week_end.strftime("%Y-%m-%d"), "progress": progress_count})
	...
	def _get_goal_completion_trend(self, user_id: int, months: int = 6) -> List[Dict[str, Any]]:
		...
		for i in range(months):
			...
			trend_data.append({"month": month_start.strftime("%Y-%m"), "completed_goals": completed_count})
	...
	def _calculate_longest_streak(self, user_id: int) -> int:
		...
		progress_dates = (
			self.db.query(GoalProgress.progress_date)
			.filter(GoalProgress.user_id == user_id)
			.distinct()
			.order_by(desc(GoalProgress.progress_date))
			.all()
		)
	...
	def _check_milestone_achievements(self, goal: Goal, progress_added: int) -> None:
		...
		for milestone_data in milestones:
			if not milestone_data.get("achieved", False) and goal.current_value >= milestone_data["value"]:
				...
				milestone_data["achieved_at"] = datetime.now(timezone.utc).isoformat()
	...
	def _calculate_milestone_points(self, milestone: Milestone) -> int:
		...
		type_multiplier = {"goal_achievement": 1.5, "application_streak": 1.2, "consistency_milestone": 1.0, "improvement_milestone": 1.3}

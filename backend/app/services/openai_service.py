...
class EnhancedOpenAIService:
	...
	def chat_completion(
		self, messages: List[Dict[str, str]], model: Optional[str] = None, temperature: float = 0.7, max_tokens: Optional[int] = None, **kwargs
	) -> Dict[str, Any]:
		...
		try:
			...
			response = self.client.chat.completions.create(
				model=model, messages=messages, temperature=temperature, max_tokens=max_tokens, timeout=self.timeout, **kwargs
			)
	...
	def simple_completion(self, prompt: str, model: Optional[str] = None, max_tokens: int = 150) -> str:
		...
		try:
			response = self.chat_completion(messages=messages, model=model, max_tokens=max_tokens, temperature=0.7)

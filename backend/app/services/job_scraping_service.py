...
class JobScrapingService:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                headers = {
                    ...
                    'User-Agent': 'career-copilot@example.com'
                }
            ...
            logger.error(f"Error scraping from USAJobs: {str(e)}")
            return []
    ...
    async def _scrape_remoteok(self, preferences: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error scraping from RemoteOK: {str(e)}")
            return []
                    ...
                    try:
                        ...
                        try:
                            user_result = loop.run_until_complete(
                                scraping_service.ingest_jobs_for_user(user.id, max_jobs_per_user)
                            )
                        finally:
                            loop.close()

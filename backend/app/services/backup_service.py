...
class BackupService:
    ...
    def create_full_backup(self, include_files: bool = True, 
        ...
        with performance_tracker.track_operation("full_backup", threshold_seconds=30.0):
            ...
            try:
                ...
                if compress:
                    ...
                    backup_info["compressed_size_mb"] = round(
        ...
        if db_url.startswith("postgresql://"):
            ...
            try:
                ...
                return {
                    "status": "partial",
                    "json_export": str(json_backup_path),
                    "warning": "pg_dump not available, JSON export only"
                }
        else:
    ...
    def _backup_configuration(self, backup_path: Path) -> Dict[str, Any]:
        ...
        return {
            ...
            "config_summary": "config_summary.json"
        }
    ...
    def restore_from_backup(self, backup_name: str, 
        ...
        try:
            ...
            if backup_path.suffix == '.tar.gz':
                extract_path = self.backup_dir / f"restore_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            ...
            logger.error(f"Restore failed: {backup_name} - {str(e)}")
            raise
    ...
    def _restore_from_json(self, json_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "partial",
                "method": "json_import",
                "warning": "JSON restore is limited - use SQL dump for complete restore"
            }
            
        except Exception as e:
            db.rollback()
            raise Exception(f"JSON restore failed: {str(e)}")
        finally:
            db.close()

...
class ModelManager:
	...
	async def get_available_models(self) -> List[str]:
		...
		if self.last_model_check is None or now - self.last_model_check > timedelta(seconds=self.model_check_interval):
...
class EnhancedOllamaService:
	...
	async def generate_completion(
		self, messages: List[Dict[str, str]], model: Optional[str] = None, task_type: str = "generation", use_cache: bool = True, **kwargs
	) -> Dict[str, Any]:
		...
		try:
			...
			if use_cache:
				...
				cached_result = await cache_service(cache_key)
				...
				if cached_result:
					...
					return {**cached_result, "cached": True, "request_id": request_id, "response_time": response_time}
			...
			result = {
				...
				"performance": {
					...
					"tokens_per_second": completion_tokens / (response_data.get("eval_duration", 1) / 1e9)
					if response_data.get("eval_duration")
					else 0,
				},
				"cached": False,
			...
			if metrics_collector:
				metrics_collector.record_ai_request(
					model=selected_model, provider="ollama", tokens=total_tokens, cost=0.0, response_time=response_time, success=True
				)
			...
			if metrics_collector:
				metrics_collector.record_ai_request(
					model=model or "unknown", provider="ollama", tokens=0, cost=0.0, response_time=response_time, success=False
				)

			logger.error(f"Ollama completion failed: {request_id}, error: {e}")
			raise OllamaError(f"Completion failed: {e}")

	async def analyze_contract(self, contract_text: str, analysis_type: str = "comprehensive", model: Optional[str] = None) -> Dict[str, Any]:
...
- Mitigation recommendations""",
		...
		try:
			result = await self.generate_completion(messages=messages, model=model, task_type="analysis", temperature=0.1, max_tokens=1500)
		...
		try:
			...
			if models:
				try:
					...
					result = await self.generate_completion(messages=messages, model=models[0], max_tokens=5, use_cache=False)

					test_result = {"success": True, "model_used": result["model"], "request_id": result["request_id"]}

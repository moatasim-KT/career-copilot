...
class ScraperManager:
	...
	def _initialize_scrapers(self) -> Dict[str, BaseScraper]:
		...
		# Create rate limiter
		rate_limiter = RateLimiter(min_delay=self.config.rate_limit_min_delay, max_delay=self.config.rate_limit_max_delay)
	...
	async def search_all_sites(self, keywords: str, location: str = "", max_total_results: int = 100) -> List[JobCreate]:
		...
		results_per_site = min(self.config.max_results_per_site, max_total_results // len(self.scrapers))

		# Create scraping tasks
		tasks = []
		for site_name, scraper in self.scrapers.items():
			task = self._scrape_site(site_name, scraper, keywords, location, results_per_site)
			tasks.append(task)

		# Run scrapers with concurrency limit
		semaphore = asyncio.Semaphore(self.config.max_concurrent_scrapers)
		results = await asyncio.gather(*[self._run_with_semaphore(semaphore, task) for task in tasks], return_exceptions=True)
	...
	async def _scrape_site(self, site_name: str, scraper: BaseScraper, keywords: str, location: str, max_results: int) -> List[JobCreate]:

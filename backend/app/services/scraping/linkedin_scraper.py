...
class LinkedInScraper(BaseScraper):
	...
	def _is_blocked(self, html_content: str) -> bool:
		...
		blocked_indicators = ["challenge", "captcha", "blocked", "security check", "unusual activity", "authwall"]
	...
	def _parse_job_listing(self, job_element) -> Optional[Dict[str, Any]]:
		...
		try:
			...
			# Extract location
			location_selectors = [".job-result-card__location", ".jobs-unified-top-card__bullet", ".job-result-card__meta .job-result-card__location"]
			...
			# Extract job URL
			url_selectors = [".job-result-card__title a", "h3 a", '[data-control-name="job_search_job_result_title"]']
			...
			snippet_selectors = [".job-result-card__snippet", ".jobs-unified-top-card__job-insight", ".job-result-card__summary"]
	...
	def _extract_linkedin_metadata(self, job_element, job_data: Dict[str, Any]):
		...
		time_selectors = [".job-result-card__listdate", ".jobs-unified-top-card__posted-date", "time"]
		...
		applicant_selectors = [".job-result-card__meta-item", ".jobs-unified-top-card__applicant-count"]

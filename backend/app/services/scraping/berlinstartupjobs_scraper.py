...
class BerlinStartupJobsScraper(BaseScraper):
	...
	def _parse_job_listing(self, card) -> Optional[dict]:
	...
	def _build_search_url(self, keywords: str, location: str = "Berlin", page: int = 0) -> str:
		...
		if search_terms:
			return f"{self.base_url}/{'/'.join(search_terms)}/"

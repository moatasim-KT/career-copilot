...
from ..models.database_models import User, RoleAssignment, PermissionGrant, SecurityEvent
...
class RBACService:
    ...
    async def check_permission(
        ...
        try:
            ...
            if context and context.resource_type:
                ...
                required_security_level = self._get_required_security_level(
                    context.resource_type,
                    context.action
                )
                security_level_met = self._check_security_level(
                    user_security_level,
            ...
            if context:
                context_validated = await self._validate_context_rules(
                    user_id,
                    required_permission,
                    context
                )
    ...
    async def check_multiple_permissions(
        ...
        try:
            ...
            return AccessControlResult(
                allowed=False,
                reason=f"Multiple permission check error: {str(e)}",
                required_permissions=required_permissions,
                user_permissions=[],
                security_level_met=False,
                context_validated=False
        ...
        try:
            ...
            async with db_manager.get_session() as session:
                ...
                user_result = await session.execute(
                    select(User.roles).where(User.id == user_id)
                )
    ...
    async def get_user_permissions(self, user_id: str) -> List[Permission]:
        ...
        try:
            ...
            async with db_manager.get_session() as session:
                ...
                user_result = await session.execute(
                    select(User.permissions).where(User.id == user_id)
                )
    ...
    async def get_user_security_level(self, user_id: str) -> SecurityLevel:
        ...
        try:
            ...
            async with db_manager.get_session() as session:
                ...
                result = await session.execute(
                    select(User.security_level).where(User.id == user_id)
                )
    ...
    async def _validate_context_rules(
        ...
        try:
            ...
            if context.resource_type and context.resource_id:
                if await self._check_resource_ownership(
                    user_id,
                    context.resource_type,
                    context.resource_id
                ):
            ...
            if context.ip_address and not self._check_ip_restrictions(
                user_id,
                context.ip_address
            ):
                return False
        ...
        try:
            if resource_type == "contract_analysis":
                ...
                async with db_manager.get_session() as session:
                    ...
                    result = await session.execute(
                        select(ContractAnalysis.user_id).where(
                            ContractAnalysis.id == resource_id
                        )
                    )

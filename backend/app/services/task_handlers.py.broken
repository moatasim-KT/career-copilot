...
from ..models.task_models import (
	...
	TaskProgress,
	TaskType,
	ContractAnalysisTaskPayload,
	DocumentProcessingTaskPayload,
...
async def update_task_progress(
	task_id: str, storage: TaskStorage, step: str, percentage: float, message: str, details: Optional[Dict[str, Any]] = None
):
	"""Helper function to update task progress."""
	progress = TaskProgress(
		task_id=task_id, current_step=step, progress_percentage=percentage, message=message, details=details, updated_at=datetime.now(timezone.utc)
	)
	await storage.update_progress(progress)


async def handle_contract_analysis_task(task: TaskDefinition, storage: TaskStorage) -> Dict[str, Any]:
	...
	try:
		...
		await update_task_progress(task.task_id, storage, "initializing", 5.0, "Initializing job application tracking")
		...
		await update_task_progress(task.task_id, storage, "processing_document", 15.0, "Processing contract document")
		...
		await update_task_progress(task.task_id, storage, "ai_analysis", 30.0, "Running AI analysis on contract")
		...
		if payload.enable_risk_assessment:
			await update_task_progress(task.task_id, storage, "risk_assessment", 50.0, "Performing risk assessment")

		# Update progress - precedent search
		if payload.enable_precedent_search:
			await update_task_progress(task.task_id, storage, "precedent_search", 65.0, "Searching for legal precedents")

		# Update progress - redline generation
		if payload.enable_redline_generation:
			await update_task_progress(task.task_id, storage, "redline_generation", 80.0, "Generating redline suggestions")

		# Update progress - report generation
		if payload.generate_report:
			await update_task_progress(task.task_id, storage, "report_generation", 90.0, "Generating analysis report")
		...
		if payload.send_email_notification and payload.email_recipients:
			await update_task_progress(task.task_id, storage, "email_notification", 95.0, "Sending email notifications")
			...
			try:
				...
				for recipient in payload.email_recipients:
					await email_service.send_analysis_complete_notification(
						recipient_email=recipient, contract_name=payload.contract_filename, analysis_result=analysis_result
					)
...
async def handle_document_processing_task(task: TaskDefinition, storage: TaskStorage) -> Dict[str, Any]:
	...
	try:
		...
		# Update progress - initialization
		await update_task_progress(task.task_id, storage, "initializing", 10.0, f"Starting {payload.processing_type} processing")
		...
		if payload.processing_type == "ocr":
			await update_task_progress(task.task_id, storage, "ocr_processing", 30.0, "Performing OCR on document")

			ocr_result = await processor.perform_ocr(document_path=payload.document_path, options=payload.options)
			result["ocr_text"] = ocr_result

		elif payload.processing_type == "table_extraction":
			await update_task_progress(task.task_id, storage, "table_extraction", 40.0, "Extracting tables from document")

			tables = await processor.extract_tables(document_path=payload.document_path, options=payload.options)
			result["tables"] = tables

		elif payload.processing_type == "structure_analysis":
			await update_task_progress(task.task_id, storage, "structure_analysis", 50.0, "Analyzing document structure")

			structure = await processor.analyze_structure(document_path=payload.document_path, options=payload.options)
			result["structure"] = structure

		# Update progress - finalizing
		await update_task_progress(task.task_id, storage, "finalizing", 90.0, "Finalizing processing results")
...
async def handle_batch_processing_task(task: TaskDefinition, storage: TaskStorage) -> Dict[str, Any]:
	...
	try:
		...
		# Update progress - initialization
		await update_task_progress(task.task_id, storage, "initializing", 5.0, f"Starting batch processing of {len(payload.item_ids)} items")
			...
			async with semaphore:
				try:
					...
					await update_task_progress(
						task.task_id, storage, "processing_items", progress_percentage, f"Processing item {index + 1} of {total_items}: {item_id}"
					)
					...
					return {"item_id": item_id, "status": "completed", "result": item_result}

				except Exception as e:
					error_result = {"item_id": item_id, "status": "failed", "error": str(e)}
		...
		# Process all items
		tasks = [process_item(item_id, index) for index, item_id in enumerate(payload.item_ids)]
		...
		# Update progress - finalizing
		await update_task_progress(task.task_id, storage, "finalizing", 95.0, "Finalizing batch processing results")

		# Send completion notification if requested
		if payload.notify_on_completion:
			try:
				            from ..services.scheduled_notification_service import get_scheduled_notification_service
				            notification_service = get_scheduled_notification_service(db=next(get_db()))
				
				            await notification_service.send_notification(
				                user_id=payload.user_id,
				                notification_type="batch_completion",
				                title=f"Batch Processing Complete: {payload.batch_id}",
				                message=f"Your batch processing task {payload.batch_id} has completed. Successful: {successful_items}, Failed: {failed_items}",
				                data={
...
async def handle_report_generation_task(task: TaskDefinition, storage: TaskStorage) -> Dict[str, Any]:
	"""Handle report generation background task."""
	logger.info(f"Starting report generation task {task.task_id}")

	try:
		# Update progress - initialization
		await update_task_progress(task.task_id, storage, "initializing", 10.0, "Initializing report generation")
		...
		await update_task_progress(task.task_id, storage, "data_preparation", 30.0, "Preparing report data")
		...
		# Update progress - report generation
		await update_task_progress(task.task_id, storage, "generating_report", 60.0, f"Generating {report_type} in {output_format} format")

		# Generate report based on type
		if report_type == "analysis_report":
			report_result = await report_service.generate_analysis_report(analysis_data=report_data, output_format=output_format)
		elif report_type == "batch_report":
			report_result = await report_service.generate_batch_report(batch_data=report_data, output_format=output_format)
		else:
			raise ValueError(f"Unknown report type: {report_type}")

		# Update progress - finalizing
		await update_task_progress(task.task_id, storage, "finalizing", 90.0, "Finalizing report")
...
async def handle_email_notification_task(task: TaskDefinition, storage: TaskStorage) -> Dict[str, Any]:
	"""Handle email notification background task."""
	logger.info(f"Starting email notification task {task.task_id}")

	try:
		# Update progress - initialization
		await update_task_progress(task.task_id, storage, "initializing", 10.0, "Preparing email notification")
		...
		await update_task_progress(task.task_id, storage, "preparing_email", 30.0, f"Preparing {email_type} email for {len(recipients)} recipients")
		...
		for i, recipient in enumerate(recipients):
			try:
				...
				await update_task_progress(
					task.task_id, storage, "sending_emails", progress_percentage, f"Sending email to {recipient} ({i + 1}/{len(recipients)})"
				)

				# Send email based on type
				if email_type == "analysis_complete":
					await email_service.send_analysis_complete_notification(recipient_email=recipient, **email_data)
				elif email_type == "risk_alert":
					await email_service.send_risk_alert_notification(recipient_email=recipient, **email_data)
				elif email_type == "batch_complete":
					await email_service.send_batch_complete_notification(recipient_email=recipient, **email_data)
				else:
					await email_service.send_generic_notification(recipient_email=recipient, **email_data)
		...
		# Update progress - finalizing
		await update_task_progress(task.task_id, storage, "finalizing", 95.0, "Finalizing email notifications")

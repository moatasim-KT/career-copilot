...
from .slack_service import EnhancedSlackService, SlackMessage, SlackPriority
...
class SlackBotCommands:
    ...
    def __init__(self, slack_service: EnhancedSlackService):
        ...
        self.commands = {
            ...
            "report": SlackCommand(
                ...
                examples=[
                    "/report contract_123",
                    "/report contract_123 pdf",
                    "/report contract_123 summary"
                ]
            ),
            "help": SlackCommand(
                name="help",
            ...
            "upload": SlackCommand(
                ...
                examples=[
                    "/upload (attach file)",
                    "/upload --priority high (attach file)",
                    "/upload --notify #legal (attach file)"
                ]
            ),
            "workflow": SlackCommand(
                ...
                examples=[
                    "/workflow create contract_123 @user1 @user2",
                    "/workflow status workflow_456",
                    "/workflow approve workflow_456"
                ]
            )
        }
    ...
    async def handle_slash_command(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not await self._check_command_permission(command, user_id, channel_id):
                return await self._send_error_response(
                    f"You don't have permission to use the /{command} command."
                )
    ...
    async def handle_analyze_command(self, payload: Dict[str, Any], text: str) -> Dict[str, Any]:
            ...
            "blocks": [
                ...
                },
                {
                    ...
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Contract:* {contract_input}"
                        },
                        {
                            "type": "mrkdwn",
                        ...
                        {
                            "type": "mrkdwn",
                            "text": f"*Priority:* {options.get('priority', 'normal').title()}"
                        },
                        {
                            "type": "mrkdwn",
                ...
                },
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": "⏳ Analysis in progress... You'll be notified when complete."
                    }
                }
            ]
        }
        ...
        asyncio.create_task(self._simulate_contract_analysis(
            analysis_id, contract_input, user_id, channel_id, options
        ))
    ...
    async def handle_report_command(self, payload: Dict[str, Any], text: str) -> Dict[str, Any]:
        ...
        if not text:
            return await self._send_error_response(
                "Please provide a contract ID. Usage: `/report <contract_id> [format]`"
            )
            ...
            return {
                ...
                "blocks": [
                    ...
                    },
                    {
                        ...
                        "text": {
                            "type": "mrkdwn",
                            "text": f"*Description:* {command.description}\n*Usage:* `{command.usage}`"
                        }
                    },
                    {
                        "type": "section",
                ...
                "blocks": [
                    {
                        ...
                        "text": {
                            "type": "plain_text",
                            "text": "📚 Career Copilot Bot Commands"
                        }
                    },
                    {
                        "type": "section",
    ...
    async def handle_settings_command(self, payload: Dict[str, Any], text: str) -> Dict[str, Any]:
        ...
        if action == "notifications":
            ...
            return await self._send_error_response(
                f"Unknown settings action: {action}. Use `/help settings` for usage."
            )
    ...
    async def handle_workflow_command(self, payload: Dict[str, Any], text: str) -> Dict[str, Any]:
        ...
        if action == "create":
            ...
            return await self._send_error_response(
                f"Unknown workflow action: {action}. Use `/help workflow` for usage."
            )
    ...
    async def _handle_workflow_create(self, payload: Dict[str, Any], args: List[str]) -> Dict[str, Any]:
        ...
        if len(args) < 2:
            return await self._send_error_response(
                "Usage: `/workflow create <contract_name> @user1 @user2 ...`"
            )
        ...
        if not approvers:
            return await self._send_error_response(
                "Please specify at least one approver using @username format."
            )
        ...
        try:
            ...
            return await self._send_error_response(f"Failed to create workflow: {str(e)}")
            ...
            "blocks": [
                ...
                },
                {
                    ...
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Contract:* {workflow['contract_name']}"
                        },
                        {
                            "type": "mrkdwn",
                        ...
                        {
                            "type": "mrkdwn",
                            "text": f"*Created:* {workflow['created_at'].strftime('%Y-%m-%d %H:%M')}"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Progress:* {len(workflow['approved_by'])}/{len(workflow['approvers'])} approved"
                        }
                    ]
                },
                {
    ...
    async def _handle_workflow_list(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for workflow_id, workflow in workflows.items():
            ...
            workflow_list.append(
                f"{status_emoji} `{workflow_id}` - {workflow['contract_name']} ({workflow['status']})"
            )
            ...
            "blocks": [
                ...
                },
                {
                    ...
                    "elements": [
                        {
                            "type": "mrkdwn",
                            "text": "Use `/workflow status <workflow_id>` for detailed information"
                        }
                    ]
                }
            ]
    ...
    async def _simulate_contract_analysis(
        self,
        analysis_id: str,
        contract_input: str,
        user_id: str,
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": f"📊 Contract Status: {contract_id}"
                    }
                },
                {
                    "type": "section",
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Status:* {status_data['status'].title()}"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Progress:* {status_data['progress']}%"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Risk Score:* {status_data['risk_score']}/10"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Created:* {status_data['created_at']}"
                        }
                    ]
                }
            ]
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": f"📋 Recent Analyses (Last {count})"
                    }
                },
                {
                    "type": "section",
    ...
    async def _generate_contract_report(self, contract_id: str, format_type: str, user_id: str) -> Dict[str, Any]:
        ...
        if format_type not in ["summary", "detailed", "pdf"]:
            return await self._send_error_response(
                f"Invalid report format: {format_type}. Use 'summary', 'detailed', or 'pdf'."
            )
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": f"📄 Generating {format_type.title()} Report"
                    }
                },
                {
                    "type": "section",
            ...
            "blocks": [
                ...
                },
                {
                    ...
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Notifications:* {settings['notifications']}"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Default Channel:* {settings['default_channel']}"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Auto Notify:* {'Yes' if settings['auto_notify'] else 'No'}"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Report Format:* {settings['report_format']}"
                        }
                    ]
                },
                {
    ...
    async def _manage_notification_settings(self, user_id: str, setting: str) -> Dict[str, Any]:
        ...
        if setting in ["on", "enable", "enabled"]:
            ...
            return await self._send_error_response(
                "Invalid setting. Use 'on' or 'off' to enable/disable notifications."
            )
    ...
    async def _set_default_channel(self, user_id: str, channel: str) -> Dict[str, Any]:
        ...
        if not channel:
            return await self._send_error_response(
                "Please specify a channel. Usage: `/settings channel #channel-name`"
            )
    ...
    def get_command_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                (self.command_stats["total_commands"] - self.command_stats["errors"]) / 
                self.command_stats["total_commands"] * 100
            ) if self.command_stats["total_commands"] > 0 else 0
        }
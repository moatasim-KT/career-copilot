...
class BulkOperationsService:
	...
	async def bulk_create_jobs(self, jobs_data: List[JobCreate], user_id: int) -> BulkCreateResult:
		"""
			...
			async with self.db.begin_nested():
				for index, job_data in enumerate(jobs_data):
					try:
						...
						errors.append(OperationError(index=index, error=f"Validation error: {e!s}", details={"job_data": job_data.model_dump()}))
						logger.warning(f"Failed to create job at index {index}: {e}")

					except Exception as e:
						errors.append(OperationError(index=index, error=f"Unexpected error: {e!s}", details={"job_data": job_data.model_dump()}))
		...
		return BulkCreateResult(total=total, successful=successful, failed=failed, created_ids=created_ids, errors=errors)

	async def bulk_create_applications(self, applications_data: List[ApplicationCreate], user_id: int) -> BulkCreateResult:
		"""
			...
			async with self.db.begin_nested():
				for index, app_data in enumerate(applications_data):
					try:
						...
						result = await self.db.execute(select(Job).where(Job.id == app_data.job_id, Job.user_id == user_id))
						...
						result = await self.db.execute(
							select(Application).where(Application.job_id == app_data.job_id, Application.user_id == user_id)
						)
						...
						# Create application
						application = Application(**app_data.model_dump(), user_id=user_id)
						...
						errors.append(
							OperationError(index=index, error=f"Validation error: {e!s}", details={"application_data": app_data.model_dump()})
						)
						logger.warning(f"Failed to create application at index {index}: {e}")

					except Exception as e:
						errors.append(
							OperationError(index=index, error=f"Unexpected error: {e!s}", details={"application_data": app_data.model_dump()})
						)
	...
	async def bulk_update_jobs(self, updates: List[tuple[int, JobUpdate]], user_id: int) -> BulkUpdateResult:
		"""
			...
			async with self.db.begin_nested():
				for index, (job_id, job_update) in enumerate(updates):
					try:
						...
						if not job:
							errors.append(OperationError(index=index, id=job_id, error=f"Job with ID {job_id} not found or does not belong to user"))
							continue
		...
		return BulkUpdateResult(total=total, successful=successful, failed=failed, updated_ids=updated_ids, errors=errors)

	async def bulk_update_applications(self, updates: List[tuple[int, ApplicationUpdate]], user_id: int) -> BulkUpdateResult:
		"""
		...
		try:
			...
			async with self.db.begin_nested():
				for index, (app_id, app_update) in enumerate(updates):
					try:
						# Get the application
						result = await self.db.execute(select(Application).where(Application.id == app_id, Application.user_id == user_id))
						app = result.scalar_one_or_none()

						if not app:
							errors.append(
								OperationError(index=index, id=app_id, error=f"Application with ID {app_id} not found or does not belong to user")
							)
							continue
						...
						if "status" in update_data and update_data["status"] == "applied" and app.status != "applied":
							...
							result = await self.db.execute(select(Job).where(Job.id == app.job_id))
							job = result.scalar_one_or_none()
							if job:
								job.status = "applied"
								job.date_applied = datetime.utcnow()
	...
	async def bulk_delete_jobs(self, job_ids: List[int], user_id: int, soft_delete: bool = False) -> BulkDeleteResult:
		"""
			...
			async with self.db.begin_nested():
				for index, job_id in enumerate(job_ids):
					try:
						...
						errors.append(OperationError(index=index, id=job_id, error=f"Integrity error: {e!s}", details={"job_id": job_id}))
						logger.warning(f"Failed to delete job {job_id} at index {index}: {e}")

					except Exception as e:
						errors.append(OperationError(index=index, id=job_id, error=f"Unexpected error: {e!s}", details={"job_id": job_id}))
		...
		return BulkDeleteResult(total=total, successful=successful, failed=failed, deleted_ids=deleted_ids, soft_deleted=soft_delete, errors=errors)

	async def bulk_delete_applications(self, app_ids: List[int], user_id: int, soft_delete: bool = False) -> BulkDeleteResult:
		"""
			...
			async with self.db.begin_nested():
				for index, app_id in enumerate(app_ids):
					try:
						...
						errors.append(OperationError(index=index, id=app_id, error=f"Integrity error: {e!s}", details={"application_id": app_id}))
						logger.warning(f"Failed to delete application {app_id} at index {index}: {e}")

					except Exception as e:
						errors.append(OperationError(index=index, id=app_id, error=f"Unexpected error: {e!s}", details={"application_id": app_id}))

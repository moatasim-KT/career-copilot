...
from ..models.skill import UserSkill, RequiredSkill
...
class UnifiedSkillService:
    ...
    async def analyze_user_skills(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            skill_analysis = await self.analyzer.analyze_skill_proficiency(
            ...
            career_paths = await self.analyzer.suggest_career_paths(
                user_id=user_id,
                current_skills=current_skills,
                gaps=gaps
            )
            ...
            logger.error(f"Error analyzing skills for user {user_id}: {str(e)}")
            return {"error": str(e)}
    ...
    async def track_skill_progress(self, user_id: int, skill_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            effectiveness = await self.analyzer.evaluate_learning_effectiveness(
                user_id=user_id,
                skill_id=skill_id
            )
    ...
    async def update_skill_recommendations(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            market_demand = await self.analyzer.get_market_demand(
                skills=[skill.name for skill in current_skills]
            )

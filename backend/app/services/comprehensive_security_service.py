...
from ..security.threat_detector import ThreatDetector, ThreatDetection
...
class ComprehensiveSecurityService:
	...
	def __init__(self, audit_repository: Optional[AuditRepository] = None, temp_file_manager: Optional[TempFileManager] = None):
	...
	async def assess_file_security(
		self, file_content: bytes, filename: str, user_id: Optional[str] = None, additional_context: Optional[Dict[str, any]] = None
	) -> SecurityAssessment:
		"""
		...
		try:
			...
			validation_result, scan_result = await asyncio.gather(validation_task, scan_task, return_exceptions=True)
			...
			error_assessment = SecurityAssessment(
				...
				metadata={"error": str(e), "user_id": user_id, "assessment_timestamp": datetime.now(timezone.utc).isoformat()},
			)

			# Record error metrics
			metrics_collector.record_security_event(event_type="assessment_error", severity="high")

			return error_assessment

	def _calculate_risk_score(
		self, validation_result: FileValidationResult, scan_result: ScanResult, threat_detections: List[ThreatDetection]
	) -> float:
		...
		overall_score = validation_score * self.VALIDATION_WEIGHT + scan_score * self.MALWARE_WEIGHT + threat_score * self.THREAT_WEIGHT

		return min(overall_score, 10.0)

	def _make_security_decision(
		self, risk_score: float, validation_result: FileValidationResult, scan_result: ScanResult, threat_detections: List[ThreatDetection]
	) -> SecurityDecision:
		"""Make security decision based on assessment results."""

		# Immediate block conditions
		if (
			validation_result.status == ValidationStatus.QUARANTINED
			or scan_result.status == ScanStatus.INFECTED
			or any(t.threat_level.value == "critical" for t in threat_detections)
		):
			return SecurityDecision.BLOCK

		# Quarantine conditions
		if (
			validation_result.status == ValidationStatus.SUSPICIOUS
			or scan_result.status == ScanStatus.SUSPICIOUS
			or risk_score >= self.HIGH_RISK_THRESHOLD
		):
			return SecurityDecision.QUARANTINE

		# Review required conditions
		if risk_score >= self.MEDIUM_RISK_THRESHOLD or len(threat_detections) > 3 or any(t.threat_level.value == "high" for t in threat_detections):
	...
	def _generate_recommendations(
		self, validation_result: FileValidationResult, scan_result: ScanResult, threat_detections: List[ThreatDetection], decision: SecurityDecision
	) -> List[str]:
	...
	async def _handle_security_decision(self, assessment: SecurityAssessment, file_content: bytes) -> None:
		...
		if assessment.decision == SecurityDecision.QUARANTINE:
			...
			# Quarantine the file
			self.temp_file_manager.quarantine_file(file_id, f"Security assessment risk score: {assessment.risk_score:.2f}")
	...
	def _record_security_metrics(self, assessment: SecurityAssessment) -> None:
		...
		metrics_collector.record_security_assessment(
			decision=assessment.decision.value, risk_score=assessment.risk_score, processing_time=assessment.processing_time_seconds
		)

		# Record validation metrics
		metrics_collector.record_file_validation(
			result=assessment.validation_result.status.value, file_type=assessment.validation_result.file_type.value
		)

		# Record malware scan metrics
		metrics_collector.record_malware_scan(result=assessment.scan_result.status.value, engine=assessment.scan_result.scan_engine)

		# Record threat detection metrics
		for threat in assessment.threat_detections:
			metrics_collector.record_threat_detection(
				threat_type=threat.threat_type.value, threat_level=threat.threat_level.value, confidence=threat.confidence
			)
	...
	def get_security_statistics(self) -> Dict[str, any]:
		...
		return {
			...
			"risk_thresholds": {"low": self.LOW_RISK_THRESHOLD, "medium": self.MEDIUM_RISK_THRESHOLD, "high": self.HIGH_RISK_THRESHOLD},
		}

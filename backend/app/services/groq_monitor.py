...
class GROQMonitor:
    ...
    def _update_cost_breakdown(self, metric: PerformanceMetric):
        ...
        breakdown.avg_cost_per_token = (
            breakdown.total_cost / breakdown.total_tokens if breakdown.total_tokens > 0 else 0.0
        )
    ...
    def _update_performance_analysis(self, metric: PerformanceMetric):
        ...
        recent_metrics = [m for m in self.performance_metrics[-100:] 
                        if m.model == metric.model and m.task_type == metric.task_type]
        if recent_metrics:
    ...
    def _update_quality_metrics(self, metric: PerformanceMetric):
        ...
        quality.quality_distribution[score_bucket] = (
            quality.quality_distribution.get(score_bucket, 0) + 1
        )
    ...
    async def _check_system_alerts(self):
        ...
        daily_cost = sum(
            m.cost for m in self.performance_metrics

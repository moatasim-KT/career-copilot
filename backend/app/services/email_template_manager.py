...
This service consolidates email template functionality from multiple modules:
- Template rendering and management (formerly email_template_service.py)
...
- Multi-format support (HTML, text, mixed)
- Template versioning and status management
- Notification queuing with priority-based processing
- Smart batching strategies (time-based, recipient-based, priority-based)
...
Usage:
    ...
    rendered = await manager.render_template(
        "welcome_email",
        {"app_name": "Career Copilot", "user_name": "John"}
    )

Consolidation History:
- October 29, 2025: Consolidated email_template_service.py into this module
- email_template_service.py now serves as a deprecation shim for backward compatibility

Related Services:
- email_service.py - Core email sending functionality (SendGrid, AWS SES)
- email_notification_optimizer.py - Advanced notification optimization (candidate for consolidation)
- scheduled_notification_service.py - Scheduled notification delivery (candidate for consolidation)
"""
...
class EmailTemplate(BaseModel):
	...
	@field_validator("subject_template", "html_template", "text_template")
	@classmethod
...
class EmailTemplateManager:
	...
	def __init__(self):
		...
		# Rate limiting
		self.rate_limiter = {"minute": deque(maxlen=100), "hour": deque(maxlen=1000)}
		...
		self.processing_stats = {"total_queued": 0, "total_sent": 0, "total_failed": 0, "batches_processed": 0, "last_processed": None}

		# Analytics
		self.template_analytics = defaultdict(lambda: {"renders": 0, "sends": 0, "opens": 0, "clicks": 0, "last_used": None})
		...
		for template_data in builtin_templates:
			try:
				...
				template.versions.append(
					TemplateVersion(
						version=template.version, created_at=datetime.now(), created_by="system", changelog="Built-in template", is_active=True
					)
				)
	...
	async def create_template(self, template_data: Dict[str, Any], created_by: str = "system") -> EmailTemplate:
		"""Create a new email template"""
		# Validate template data
		await self._validate_template_data(template_data)

		# Create template
		template = EmailTemplate(**template_data, created_by=created_by, updated_by=created_by)

		# Add initial version
		template.versions.append(
			TemplateVersion(version=template.version, created_at=datetime.now(), created_by=created_by, changelog="Initial version", is_active=True)
		)
	...
	async def list_templates(
		...
		if tags_filter:
			templates = [t for t in templates if any(tag in t.tags for tag in tags_filter)]

		return sorted(templates, key=lambda t: t.updated_at, reverse=True)

	async def render_template(
		self, template_id: str, variables: Dict[str, Any], format_type: TemplateFormat = TemplateFormat.HTML, recipient: Optional[str] = None
	) -> str:
	...
	async def render_subject(self, template_id: str, variables: Dict[str, Any]) -> str:
	...
	async def queue_notification(
		...
		try:
			...
			return {"success": True, "notification_id": notification_id, "scheduled_time": scheduled_time.isoformat(), "priority": priority.value}

		except Exception as e:
			logger.error(f"Failed to queue notification: {e}")
			return {"success": False, "error": "queue_failed", "message": f"Failed to queue notification: {e!s}"}
	...
	async def _get_recipient_preferences(self, email: str) -> Optional[RecipientPreferences]:
		...
		try:
			...
			prefs = RecipientPreferences(email=email, frequency="immediate", max_emails_per_day=10, digest_enabled=False)
		...
		if prefs.delivery_window:
			...
			if current_hour < window.start_hour or current_hour > window.end_hour:
				...
				if current_hour < window.start_hour:
					...
					# Move to next day
					scheduled_time = (scheduled_time + timedelta(days=1)).replace(hour=window.start_hour, minute=0, second=0, microsecond=0)
			...
			if scheduled_time.weekday() + 1 not in window.days_of_week:
				...
				while (scheduled_time + timedelta(days=days_to_add)).weekday() + 1 not in window.days_of_week:
					days_to_add += 1
				scheduled_time = (scheduled_time + timedelta(days=days_to_add)).replace(hour=window.start_hour, minute=0, second=0, microsecond=0)
	...
	async def _process_single_notification(self, notification: EmailNotification):
		...
		try:
			...
			if notification.template_id:
				# Render subject
				subject = await self.render_subject(notification.template_id, notification.template_variables or {})

				# Render HTML body
				if not notification.body_html:
					notification.body_html = await self.render_template(
						notification.template_id, notification.template_variables or {}, TemplateFormat.HTML, notification.recipient
					)
				...
				if template and template.text_template and not notification.body_text:
					notification.body_text = await self.render_template(
						notification.template_id, notification.template_variables or {}, TemplateFormat.TEXT, notification.recipient
					)
	...
	async def _validate_template_variables(self, template: EmailTemplate, variables: Dict[str, Any]):
	...
	async def get_template_analytics(self, template_id: str) -> Dict[str, Any]:
		...
		return {
			...
			"versions": [
				{"version": v.version, "created_at": v.created_at.isoformat(), "created_by": v.created_by, "is_active": v.is_active}
	...
	async def get_queue_status(self) -> Dict[str, Any]:
		...
		try:
			...
			return {
				"success": True,
				"queue_status": {"total_queued": queue_size, "is_processing": self.is_processing, "batch_queues": len(self.batch_queue)},
			...
			return {"success": False, "error": "status_failed", "message": f"Failed to get queue status: {e!s}"}
	...
	async def track_delivery_status(
		...
		try:
			...
			return {"success": True, "tracking_id": tracking_id, "status": status, "provider": provider, "timestamp": datetime.now().isoformat()}
		except Exception as e:
			logger.error(f"Failed to track delivery status: {e}")
			return {"success": False, "error": str(e), "tracking_id": tracking_id}

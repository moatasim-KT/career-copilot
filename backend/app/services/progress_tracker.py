...
class TaskProgress:
	...
	def add_update(self, stage: ProgressStage, progress: float, message: str, details: Optional[Dict[str, Any]] = None):
		...
		update = ProgressUpdate(
			timestamp=now, stage=stage, progress_percent=progress, message=message, details=details, estimated_completion=self.estimated_completion
		)
	...
	def _calculate_eta(self, current_progress: float) -> Optional[datetime]:
		...
		if stage_eta_seconds is not None and recent_eta_seconds is not None:
			combined_eta_seconds = linear_eta_seconds * 0.4 + stage_eta_seconds * 0.4 + recent_eta_seconds * 0.2
	...
	def get_stage_progress(self) -> Dict[str, Any]:
		...
		for stage, (start, end) in stage_ranges.items():
			if stage in self.stage_durations:
				stage_progress[stage.value] = {"completed": True, "duration": self.stage_durations[stage], "progress_range": [start, end]}
				...
				stage_progress[stage.value] = {"completed": False, "progress_range": [start, end]}
...
class ProgressTracker:
	...
	async def start_tracking(self, task_id: str) -> TaskProgress:
		...
		# Add initial update
		task_progress.add_update(ProgressStage.QUEUED, 0.0, "Task queued for processing", {"queued_at": datetime.now(timezone.utc).isoformat()})
	...
	async def update_progress(
		self, task_id: str, stage: ProgressStage, progress: float, message: str, details: Optional[Dict[str, Any]] = None
	) -> bool:
	...
	async def complete_task(self, task_id: str, success: bool = True, final_message: Optional[str] = None) -> bool:
	...
	async def cancel_task(self, task_id: str) -> bool:
		...
		task_progress = self.active_tasks[task_id]
		task_progress.add_update(ProgressStage.CANCELLED, task_progress.progress_percent, "Task cancelled by user")
	...
	def get_dashboard_data(self) -> Dict[str, Any]:
		...
		completed_items = sorted(
			self.completed_tasks.items(), key=lambda x: x[1].updates[-1].timestamp if x[1].updates else x[1].start_time, reverse=True
		)[:10]
...
async def update_analysis_progress(task_id: str, stage: str, progress: float, message: str, details: Optional[Dict[str, Any]] = None):

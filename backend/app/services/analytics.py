...
class AnalyticsService:
    ...
    def get_user_analytics(self, user: User) -> Dict[str, Any]:
        ...
        # Acceptance rate
        accepted_applications = self.db.query(Application).filter(
            Application.user_id == user.id,
        ...
        weekly_applications = self.db.query(Application).filter(
            Application.user_id == user.id,
            Application.applied_date >= one_week_ago
        ).count()
        ...
        status_breakdown_query = self.db.query(Application.status, func.count(Application.id)).filter(
            Application.user_id == user.id

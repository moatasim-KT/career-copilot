...
Consolidation note (Phase 1):
- Email composition and sending is delegated to `EmailService`.
- This module remains as a thin orchestration layer for morning/evening notifications
	and user notification preferences. It will be further reduced in later phases.
- Backward compatibility: a module-level `scheduled_notification_service` facade
	is provided for existing imports (e.g., API routers) and simply delegates calls
	to a per-request instance of `ScheduledNotificationService`.
"""
...
class ScheduledNotificationService:
	...
	def __init__(self, db: Session):
		self.email_service = None  # Lazy-loaded to avoid import-time failures
		self._recommendation_service = None	 # Lazy-loaded

	def _get_email_service(self):
		# Lazy import to keep package import-safe even if email_service has issues
		if self.email_service is None:
			from .email_service import EmailService as _EmailService

			self.email_service = _EmailService()
		return self.email_service

	def _get_recommendation_service(self, db: Session):
		if self._recommendation_service is None:
			from .job_recommendation_service import (
				JobRecommendationService as _JobRecommendationService,
			)

			self._recommendation_service = _JobRecommendationService(db)
		return self._recommendation_service

	async def send_morning_briefing(self, user_id: int, db: Session, force_send: bool = False) -> Dict[str, Any]:
		"""
		...
		try:
			...
			if not user or not user.is_active:
				return {"success": False, "error": "user_not_found_or_inactive", "user_id": user_id}

			# Check user notification preferences
			if not force_send and not self._should_send_morning_briefing(user):
				return {"success": False, "error": "user_opted_out", "user_id": user_id, "message": "User has opted out of morning briefings"}
			...
			result = await self._get_email_service().send_morning_briefing(
			...
			if result["success"]:
				...
				return {"success": False, "error": "email_send_failed", "user_id": user_id, "message": result.get("message")}

		except Exception as e:
			logger.error(f"Error sending morning briefing to user {user_id}: {e}")
			return {"success": False, "error": "unexpected_error", "user_id": user_id, "message": str(e)}

	async def send_evening_update(self, user_id: int, db: Session, force_send: bool = False) -> Dict[str, Any]:
		"""
		...
		try:
			...
			if not force_send and not self._should_send_evening_update(user):
				return {"success": False, "error": "user_opted_out", "user_id": user_id, "message": "User has opted out of evening updates"}
			...
			# Send email using the email service
			result = await self._get_email_service().send_evening_summary(
				recipient_email=user.email,
				user_name=user_name,
	...
	async def send_bulk_morning_briefings(self, db: Session) -> Dict[str, Any]:
		...
		results = {"total_eligible": len(eligible_users), "sent": 0, "failed": 0, "opted_out": 0, "errors": []}
		...
		for i, result in enumerate(briefing_results):
			if isinstance(result, Exception):
				results["failed"] += 1
				results["errors"].append({"user_id": eligible_users[i].id, "error": str(result)})
			elif result["success"]:
				results["sent"] += 1
				...
				results["failed"] += 1
				results["errors"].append({"user_id": result["user_id"], "error": result.get("message", "Unknown error")})
	...
	async def send_bulk_evening_updates(self, db: Session) -> Dict[str, Any]:
		...
		results = {"total_eligible": len(eligible_users), "sent": 0, "failed": 0, "opted_out": 0, "no_activity": 0, "errors": []}
		...
		logger.info(
			f"Bulk evening updates completed: {results['sent']} sent, {results['failed']} failed, {results['opted_out']} opted out, {results['no_activity']} no activity"
		)
		return results
	...
	def _should_send_morning_briefing(self, user: User) -> bool:
		...
		try:
			...
			time_diff = abs((current_time.hour * 60 + current_time.minute) - (preferred_time_obj.hour * 60 + preferred_time_obj.minute))
	...
	async def _get_morning_recommendations(self, user_id: int, db: Session) -> List[Dict[str, Any]]:
		...
		try:
			...
			recommendations = await self._get_recommendation_service(db).get_recommendations(db=db, user_id=user_id, limit=5)
				...
				formatted_recommendations.append(
					{
						...
						if len(rec.get("description", "")) > 200
	...
	async def _get_user_progress(self, user_id: int, db: Session) -> Dict[str, Any]:
		...
		try:
			...
			applications_this_week = (
				db.query(Application).filter(and_(Application.user_id == user_id, Application.applied_date >= week_start.date())).count()
			)
			...
			responses = (
				db.query(Application).filter(and_(Application.user_id == user_id, Application.status.in_(["interview", "offer", "rejected"]))).count()
			)
	...
	async def _get_daily_goals(self, user_id: int, db: Session) -> Dict[str, Any]:
		...
		try:
			...
			recent_applications = (
				...
				.filter(and_(Application.user_id == user_id, Application.applied_date >= (datetime.now() - timedelta(days=7)).date()))
				.count()
			)
	...
	async def _get_daily_activity(self, user_id: int, db: Session) -> Dict[str, Any]:
		...
		try:
			...
			applications_today = db.query(Application).filter(and_(Application.user_id == user_id, Application.applied_date == today)).all()
			...
			for app in applications_today:
				...
				if job:
					applications_details.append(
						{"job_title": job.title, "company": job.company, "location": job.location, "applied_at": app.applied_date}
					)
	...
	async def _get_daily_achievements(self, user_id: int, db: Session) -> List[Dict[str, Any]]:
		...
		try:
			...
			applications_today = db.query(Application).filter(and_(Application.user_id == user_id, Application.applied_date == today)).count()
		...
		try:
			...
			for app in pending_apps:
				...
				if job:
					...
					follow_ups.append({"company": job.company, "type": "Application follow-up", "days_since": days_since})
			...
			return {
				...
				"skill_development": {"activity": "Complete online course module", "skill": "React", "resource": "FreeCodeCamp React Course"},
				"networking": {
					"activity": "Connect with 2 professionals in your field",
					"suggestions": ["Comment on LinkedIn posts in your industry", "Join a relevant professional group discussion"],
				},
			}
	...
	def _get_motivational_message(self, daily_activity: Dict[str, Any], achievements: List[Dict[str, Any]]) -> str:
	...
	async def _update_last_notification_time(self, user: User, notification_type: str, db: Session):
		"""Update user's last notification timestamp"""
		try:
	...
	async def update_user_notification_preferences(self, user_id: int, preferences: Dict[str, Any], db: Session) -> Dict[str, Any]:
		"""
		...
		try:
			...
			return {"success": True, "user_id": user_id, "preferences": notifications}

		except Exception as e:
			logger.error(f"Error updating notification preferences for user {user_id}: {e}")
			db.rollback()
			return {"success": False, "error": "update_failed", "user_id": user_id, "message": str(e)}

	async def get_user_notification_preferences(self, user_id: int, db: Session) -> Dict[str, Any]:
		"""
		...
		try:
			...
			return {"success": False, "error": "get_failed", "user_id": user_id, "message": str(e)}

	async def opt_out_user(self, user_id: int, notification_types: List[str], db: Session) -> Dict[str, Any]:
		"""
		...
		"""
		try:
			valid_types = ["morning_briefing", "evening_update", "job_alerts", "application_reminders", "skill_gap_reports", "all"]

			# Validate notification types
			invalid_types = [t for t in notification_types if t not in valid_types]
			if invalid_types:
				return {"success": False, "error": "invalid_notification_types", "invalid_types": invalid_types}
			...
			return {"success": True, "user_id": user_id, "opted_out_types": notification_types}

		except Exception as e:
			logger.error(f"Error opting out user {user_id}: {e}")
			db.rollback()
			return {"success": False, "error": "opt_out_failed", "user_id": user_id, "message": str(e)}

	async def opt_in_user(self, user_id: int, notification_types: List[str], db: Session) -> Dict[str, Any]:
		"""
		...
		try:
			...
			return {"success": True, "user_id": user_id, "opted_in_types": notification_types}

		except Exception as e:
			logger.error(f"Error opting in user {user_id}: {e}")
			db.rollback()
			return {"success": False, "error": "opt_in_failed", "user_id": user_id, "message": str(e)}
	...
	async def get_notification_statistics(self, db: Session) -> Dict[str, Any]:
		...
		try:
			...
			for user in users:
				...
				if not any(
					[notifications.get("morning_briefing", True), notifications.get("evening_update", True), notifications.get("job_alerts", True)]
				):
			...
			return {"success": False, "error": "statistics_failed", "message": str(e)}


class _ScheduledNotificationFacade:
	"""Facade to preserve backward-compatible module-level instance usage.

	Methods mirror ScheduledNotificationService and instantiate a service per call
	using the provided Session.
	"""

	async def send_morning_briefing(self, user_id: int, db: Session, force_send: bool = False) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).send_morning_briefing(user_id=user_id, db=db, force_send=force_send)

	async def send_evening_update(self, user_id: int, db: Session, force_send: bool = False) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).send_evening_update(user_id=user_id, db=db, force_send=force_send)

	async def send_bulk_morning_briefings(self, db: Session) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).send_bulk_morning_briefings(db=db)

	async def send_bulk_evening_updates(self, db: Session) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).send_bulk_evening_updates(db=db)

	async def update_user_notification_preferences(self, user_id: int, preferences: Dict[str, Any], db: Session) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).update_user_notification_preferences(user_id=user_id, preferences=preferences, db=db)

	async def get_user_notification_preferences(self, user_id: int, db: Session) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).get_user_notification_preferences(user_id=user_id, db=db)

	async def opt_out_user(self, user_id: int, notification_types: List[str], db: Session) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).opt_out_user(user_id=user_id, notification_types=notification_types, db=db)

	async def opt_in_user(self, user_id: int, notification_types: List[str], db: Session) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).opt_in_user(user_id=user_id, notification_types=notification_types, db=db)

	async def get_notification_statistics(self, db: Session) -> Dict[str, Any]:
		return await ScheduledNotificationService(db).get_notification_statistics(db=db)


scheduled_notification_service = _ScheduledNotificationFacade()

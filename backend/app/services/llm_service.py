...
class LLMQuotaExceededError(LLMError): pass
class LLMModelNotFoundError(LLMError): pass
class LLMInvalidRequestError(LLMError): pass
...
class ErrorClassifier:
    ...
    def __init__(self):
        self.classification_rules = {
            "authentication": {
                ...
                "status_codes": [401, 403], "category": ErrorCategory.AUTHENTICATION,
                "severity": ErrorSeverity.HIGH, "retry_config": RetryConfig.no_retry()
            },
            "rate_limit": {
                "keywords": ["rate limit", "too many requests", "quota exceeded"],
                "status_codes": [429], "category": ErrorCategory.RATE_LIMIT,
                "severity": ErrorSeverity.MEDIUM, "retry_config": RetryConfig.exponential(max_attempts=5, base_delay=2.0, max_delay=120.0)
            },
            "model_not_found": {
                "keywords": ["model not found", "model does not exist", "invalid model"],
                "status_codes": [404], "category": ErrorCategory.MODEL_NOT_FOUND,
                "severity": ErrorSeverity.HIGH, "retry_config": RetryConfig.no_retry()
            },
            "invalid_request": {
                "keywords": ["invalid request", "bad request", "validation error"],
                "status_codes": [400], "category": ErrorCategory.INVALID_REQUEST,
                "severity": ErrorSeverity.MEDIUM, "retry_config": RetryConfig.no_retry()
            },
            "timeout": {
                "keywords": ["timeout", "timed out", "deadline exceeded"],
                "status_codes": [408, 504], "category": ErrorCategory.TIMEOUT,
                "severity": ErrorSeverity.MEDIUM, "retry_config": RetryConfig.exponential(max_attempts=3, base_delay=1.0, max_delay=30.0)
            },
            "connection": {
                "keywords": ["connection", "network", "unreachable", "dns"],
                "status_codes": [502, 503], "category": ErrorCategory.CONNECTION,
                "severity": ErrorSeverity.MEDIUM, "retry_config": RetryConfig.exponential(max_attempts=3, base_delay=2.0, max_delay=60.0)
            },
            "server_error": {
                "keywords": ["internal server error", "service unavailable"],
                "status_codes": [500, 502, 503, 504], "category": ErrorCategory.SERVER_ERROR,
                "severity": ErrorSeverity.HIGH, "retry_config": RetryConfig.exponential(max_attempts=3, base_delay=5.0, max_delay=120.0)
            }
        }
    ...
    def classify_error(self, exception: Exception, provider: str, context: Dict[str, Any] = None) -> ErrorInfo:
        ...
        for error_type, rules in self.classification_rules.items():
            if any(keyword in error_message for keyword in rules["keywords"]) or \
               (status_code and status_code in rules["status_codes"]):
                return self._create_error_info(exception, provider, error_type, rules, context)
        return ErrorInfo(
            error_id=str(uuid.uuid4()), provider=provider, error_type="unknown",
            category=ErrorCategory.UNKNOWN, severity=ErrorSeverity.MEDIUM,
            message=str(exception), original_exception=exception,
            timestamp=datetime.now(), context=context or {}
        )
    ...
    def _create_error_info(self, exception: Exception, provider: str, error_type: str,
        ...
        return ErrorInfo(
            error_id=str(uuid.uuid4()), provider=provider, error_type=error_type,
            category=rules["category"], severity=rules["severity"], message=str(exception),
            original_exception=exception, timestamp=datetime.now(), context=context or {}
        )
...
class LLMService:
    ...
    def __init__(self):
        ...
        self.circuit_breakers = {
            provider.value: CircuitBreaker(provider.value) for provider in ModelProvider
        }
    ...
    def _initialize_models(self) -> Dict[ModelType, List[ModelConfig]]:
        ...
        if hasattr(settings, 'openai_api_key') and settings.openai_api_key:
            models[ModelType.CONTRACT_ANALYSIS].append(ModelConfig(
                provider=ModelProvider.OPENAI, model_name="gpt-4-turbo-preview", temperature=0.1, max_tokens=4000,
                cost_per_token=0.00001, capabilities=["analysis", "reasoning", "json", "long_context"],
                priority=1, complexity_level=TaskComplexity.COMPLEX, tokens_per_minute=15000, requests_per_minute=30
            ))
            models[ModelType.GENERAL].append(ModelConfig(
                provider=ModelProvider.OPENAI, model_name="gpt-3.5-turbo", temperature=0.2, max_tokens=2000,
                cost_per_token=0.000002, capabilities=["generation", "reasoning"], priority=3,
                complexity_level=TaskComplexity.SIMPLE, tokens_per_minute=40000, requests_per_minute=60
            ))
        # Anthropic models
        if hasattr(settings, 'anthropic_api_key') and settings.anthropic_api_key:
            models[ModelType.CONTRACT_ANALYSIS].append(ModelConfig(
                provider=ModelProvider.ANTHROPIC, model_name="claude-3-opus-20240229", temperature=0.1, max_tokens=4000,
                cost_per_token=0.000075, capabilities=["analysis", "reasoning", "long_context"],
                priority=1, complexity_level=TaskComplexity.COMPLEX, tokens_per_minute=8000, requests_per_minute=15
            ))
        # Groq models
        if hasattr(settings, 'groq_api_key') and settings.groq_api_key:
            models[ModelType.COMMUNICATION].append(ModelConfig(
                provider=ModelProvider.GROQ, model_name="mixtral-8x7b-32768", temperature=0.3, max_tokens=2000,
                cost_per_token=0.0000002, capabilities=["generation", "speed"], priority=4,
                complexity_level=TaskComplexity.SIMPLE, tokens_per_minute=100000, requests_per_minute=200
            ))
        return models

    async def analyze_with_fallback(
        self, model_type: ModelType, prompt: str, criteria: str = "cost", max_retries: int = 3,
        context: Optional[str] = None, user_id: Optional[str] = None, session_id: Optional[str] = None,
        budget_limit: Optional[float] = None, enable_streaming: bool = False,
        streaming_mode: StreamingMode = StreamingMode.BUFFERED, token_budget: Optional[TokenBudget] = None,
        ...
        if enable_streaming:
            return self._handle_streaming_request(
                models, messages, model_type, task_complexity, cost_category, streaming_mode,
                user_id, session_id, budget_limit, optimization_result
            )

        cache_key = f"ai_response:{model_type.value}:{task_complexity.value}:{hash(prompt)}"
        return await self._handle_regular_request(
            models, messages, model_type, task_complexity, cost_category, max_retries,
            user_id, session_id, budget_limit, optimization_result, cache_key
        )
    ...
    async def _handle_regular_request(self, sorted_models, messages, model_type, task_complexity, cost_category, max_retries, user_id, session_id, budget_limit, optimization_result, cache_key):
        last_error = None
    ...
    async def _handle_streaming_request(self, sorted_models, messages, model_type, task_complexity, cost_category, streaming_mode, user_id, session_id, budget_limit, optimization_result):
    ...
    async def _create_llm_instance(self, model_config: ModelConfig):
        if model_config.provider == ModelProvider.OPENAI:
            return ChatOpenAI(model=model_config.model_name, temperature=model_config.temperature, max_tokens=model_config.max_tokens, api_key=settings.openai_api_key.get_secret_value())
        elif model_config.provider == ModelProvider.ANTHROPIC:
            return ChatAnthropic(model=model_config.model_name, temperature=model_config.temperature, max_tokens=model_config.max_tokens, api_key=settings.anthropic_api_key)
        elif model_config.provider == ModelProvider.GROQ:
            # Assuming Groq uses OpenAI's client
            return ChatOpenAI(model=model_config.model_name, temperature=model_config.temperature, max_tokens=model_config.max_tokens, api_key=settings.groq_api_key.get_secret_value(), base_url="https://api.groq.com/openai/v1")
        raise ValueError(f"Unsupported provider: {model_config.provider}")

    async def _call_model_with_performance_tracking(self, model_config, messages, task_complexity, cost_category, user_id, session_id, request_context, optimization_result):
    ...
    async def generate_response(self, prompt: str, **kwargs) -> str:
        ...
        response = await self.analyze_with_fallback(

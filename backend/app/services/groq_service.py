...
from ..core.exceptions import ExternalServiceError, ValidationError, NetworkError, RateLimitError, AuthenticationError
...
class GROQService:
	...
	def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None):
		...
		self.rate_limiters = {
			model.value: GROQRateLimiter(config.requests_per_minute_limit, config.tokens_per_minute_limit)
	...
	def _initialize_model_configs(self) -> Dict[GROQModel, GROQModelConfig]:
		...
		return {
			...
			GROQModel.LLAMA3_1_8B: GROQModelConfig(
				...
				optimal_tasks=[GROQTaskType.CONVERSATION, GROQTaskType.TRANSLATION, GROQTaskType.SUMMARIZATION, GROQTaskType.FAST_ANALYSIS],
				quality_score=0.85,
				speed_score=0.98,
				metadata={"description": "Fast and efficient Llama 3.1 8B model", "best_for": "High-throughput applications and general tasks"},
			),
			GROQModel.LLAMA3_2_3B: GROQModelConfig(
				...
				optimal_tasks=[GROQTaskType.CONVERSATION, GROQTaskType.CLASSIFICATION, GROQTaskType.SUMMARIZATION],
				quality_score=0.75,
				speed_score=0.99,
				metadata={"description": "Ultra-fast Llama 3.2 3B model for simple tasks", "best_for": "High-speed processing and simple analysis"},
			),
			GROQModel.LLAMA3_2_11B: GROQModelConfig(
				...
				optimal_tasks=[GROQTaskType.REASONING, GROQTaskType.FAST_ANALYSIS, GROQTaskType.CODE_GENERATION],
				quality_score=0.88,
				speed_score=0.95,
				metadata={"description": "Balanced Llama 3.2 11B model for complex tasks", "best_for": "Balanced performance and quality"},
			),
			GROQModel.MIXTRAL_8X7B: GROQModelConfig(
				...
				optimal_tasks=[GROQTaskType.FAST_ANALYSIS, GROQTaskType.REASONING, GROQTaskType.CODE_GENERATION],
				quality_score=0.9,
				speed_score=0.95,
				metadata={"description": "High-performance mixture of experts model", "best_for": "Complex reasoning and analysis tasks"},
			),
			GROQModel.GEMMA2_9B: GROQModelConfig(
				...
				optimal_tasks=[GROQTaskType.CONVERSATION, GROQTaskType.CLASSIFICATION, GROQTaskType.TRANSLATION],
				quality_score=0.82,
				speed_score=0.96,
				metadata={"description": "Efficient Gemma 2 model for lightweight tasks", "best_for": "Fast processing and classification"},
			),
			...
			GROQModel.LLAMA3_1_70B: GROQModelConfig(
				...
				tokens_per_minute_limit=6000,
				requests_per_minute_limit=30,
				optimal_tasks=[GROQTaskType.REASONING, GROQTaskType.CODE_GENERATION, GROQTaskType.FAST_ANALYSIS],
				quality_score=0.92,
				speed_score=0.93,
			...
			GROQModel.LLAMA3_8B: GROQModelConfig(
				...
				tokens_per_minute_limit=30000,
				requests_per_minute_limit=100,
				optimal_tasks=[GROQTaskType.CONVERSATION, GROQTaskType.TRANSLATION, GROQTaskType.SUMMARIZATION],
				quality_score=0.8,
				speed_score=0.98,
				enabled=False,  # Deprecated
				metadata={"description": "DEPRECATED: Use llama-3.1-8b-instant instead", "best_for": "Deprecated model"},
			),
			GROQModel.LLAMA3_70B: GROQModelConfig(
				...
				speed_score=0.93,
				enabled=False,  # Deprecated
				metadata={"description": "DEPRECATED: Use llama-3.2-11b-text-preview instead", "best_for": "Deprecated model"},
			),
			GROQModel.GEMMA_7B: GROQModelConfig(
				...
				speed_score=0.96,
				enabled=False,  # Deprecated
				metadata={"description": "DEPRECATED: Use gemma2-9b-it instead", "best_for": "Deprecated model"},
			),
		}
	...
	async def get_optimal_model(
		...
		if not suitable_models:
			...
			suitable_models = [(model, config) for model, config in self.model_configs.items() if config.enabled]
		...
		if priority == "speed":
			...
			suitable_models.sort(key=lambda x: (x[1].quality_score + x[1].speed_score) / 2, reverse=True)
		...
		**kwargs,
		...
		if not messages:
			raise ValidationError(
				"Messages list cannot be empty", field="messages", details={"task_type": task_type.value, "model": model.value if model else None}
			)

		if not all(isinstance(msg, dict) and "content" in msg for msg in messages):
			raise ValidationError(
				"All messages must be dictionaries with 'content' field", field="messages", details={"message_count": len(messages)}
			)

		# Check API key
		if not self.api_key:
			raise AuthenticationError("GROQ API key not configured", details={"service": "groq"})
		...
		try:
			...
			raise ExternalServiceError(
				f"Failed to select optimal GROQ model: {e!s}", service_name="groq", details={"task_type": task_type.value, "priority": priority}
			)
		...
		if not model_config.enabled:
			raise ExternalServiceError(
				f"GROQ model {model.value} is currently disabled", service_name="groq", details={"model": model.value, "reason": "model_disabled"}
			)
		...
		if not await rate_limiter.acquire(int(estimated_tokens)):
			...
			raise RateLimitError(
				...
				window="1 minute",
				details={"model": model.value, "estimated_tokens": int(estimated_tokens), "wait_time": wait_time},
			)

		# Prepare request
		request_data = GROQRequest(
			model=model.value, messages=messages, temperature=temperature, max_tokens=max_tokens or min(model_config.max_tokens, 4000), **kwargs
		)
		...
		try:
			...
			if not response.choices or not response.choices[0]:
				raise ExternalServiceError(
					"Invalid response structure from GROQ API", service_name="groq", details={"model": model.value, "request_id": request_id}
				)
			...
			if "message" not in choice or "content" not in choice["message"]:
				raise ExternalServiceError(
					"Missing content in GROQ API response", service_name="groq", details={"model": model.value, "choice_keys": list(choice.keys())}
				)
			...
			result = {
				...
				"metadata": {
					...
					"model_config": {"quality_score": model_config.quality_score, "speed_score": model_config.speed_score},
				},
			}

			logger.info(f"GROQ completion successful: {model.value} ({processing_time:.2f}s, {total_tokens} tokens, ${cost:.6f})")
			...
			raise NetworkError(
				...
				details={"model": model.value, "request_id": request_id, "processing_time": processing_time},
			)
			...
			raise ExternalServiceError(
				...
				service_name="groq",
				details={"model": model.value, "request_id": request_id, "processing_time": processing_time, "timeout_duration": self.timeout},
			)
			...
			if e.response.status_code == 401:
				raise AuthenticationError("Invalid GROQ API key", details={"model": model.value, "status_code": e.response.status_code})
			elif e.response.status_code == 429:
				raise RateLimitError("GROQ API rate limit exceeded", details={"model": model.value, "status_code": e.response.status_code})
			...
			if any(keyword in error_str for keyword in ["decommissioned", "no longer supported", "deprecated", "not found"]):
				logger.warning(f"Model {model.value} appears to be deprecated: {e!s}")
			...
			raise ExternalServiceError(
				f"Unexpected GROQ API error: {e!s}",
				service_name="groq",
				details={"model": model.value, "request_id": request_id, "error_id": error_context.error_id},
				cause=e,
			)
	...
	def _update_metrics(self, model: GROQModel, processing_time: float, usage: Dict[str, int], cost: float, success: bool):
		...
		if success:
			...
			if self.metrics.successful_requests > 0:
				self.metrics.avg_tokens_per_request = self.metrics.total_tokens_used / self.metrics.successful_requests
				self.metrics.avg_cost_per_request = self.metrics.total_cost / self.metrics.successful_requests
	...
	def get_metrics_summary(self) -> Dict[str, Any]:
		...
		return {
			...
			"task_performance": {k: round(v, 3) for k, v in self.metrics.task_performance.items()},
	...
	def get_performance_analysis(self) -> Dict[str, Any]:
		...
		return {
			...
			"fastest_model": min(self.metrics.task_performance.items(), key=lambda x: x[1], default=("none", 0))[0].split(":")[0]
			if self.metrics.task_performance
			else "none",
		}

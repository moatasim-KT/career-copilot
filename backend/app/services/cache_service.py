...
class SessionData:
    ...
    def __init__(self, session_id: str, user_id: str, username: str, email: str, roles: List[str], created_at: datetime, last_accessed: datetime, expires_at: datetime, ip_address: str, user_agent: str, metadata: Optional[Dict[str, Any]] = None):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "session_id": self.session_id, "user_id": self.user_id, "username": self.username,
            "email": self.email, "roles": self.roles, "created_at": self.created_at.isoformat(),
            "last_accessed": self.last_accessed.isoformat(), "expires_at": self.expires_at.isoformat(),
            "ip_address": self.ip_address, "user_agent": self.user_agent, "metadata": self.metadata
        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SessionData":
        return cls(
            session_id=data["session_id"], user_id=data["user_id"], username=data["username"],
            email=data["email"], roles=data["roles"], created_at=datetime.fromisoformat(data["created_at"]),
            last_accessed=datetime.fromisoformat(data["last_accessed"]), expires_at=datetime.fromisoformat(data["expires_at"]),
            ip_address=data["ip_address"], user_agent=data["user_agent"], metadata=data.get("metadata", {})
        )
...
class SessionCacheService:
    ...
    async def create_session(self, user_id: str, username: str, email: str, roles: List[str], ip_address: str, user_agent: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        session_id = str(uuid.uuid4())
        now = datetime.utcnow()
        expires_at = now + timedelta(minutes=self.session_timeout_minutes)
        session_data = SessionData(
            session_id=session_id, user_id=user_id, username=username, email=email, roles=roles,
            created_at=now, last_accessed=now, expires_at=expires_at, ip_address=ip_address,
...
class RecommendationCacheService:
    ...
    def generate_and_cache_recommendations(self, db: Session, user_id: int, force_refresh: bool = False) -> List[Dict]:
        ...
        recommendations = recommendation_service.generate_recommendations_for_user(db, user_id, limit=20, min_score=0.4, diversify=True)

...
class AnalyticsService:
	...
	async def get_application_counts_by_status(self, user_id: int) -> List[ApplicationStatusCount]:
		...
		result = await self.db.execute(
			select(Application.status, func.count(Application.id)).where(Application.user_id == user_id).group_by(Application.status)
		)
	...
	async def calculate_rate_metrics(self, user_id: int) -> RateMetrics:
		...
		# Total applications
		total_apps_result = await self.db.execute(select(func.count()).select_from(Application).where(Application.user_id == user_id))
		...
		interviews_result = await self.db.execute(
			...
			.where(and_(Application.user_id == user_id, Application.status.in_(["interview", "offer", "accepted"])))
		)
		interviews = interviews_result.scalar() or 0

		# Applications that led to offers
		offers_result = await self.db.execute(
			select(func.count()).select_from(Application).where(and_(Application.user_id == user_id, Application.status.in_(["offer", "accepted"])))
		)
		offers = offers_result.scalar() or 0

		# Offers that were accepted
		accepted_result = await self.db.execute(
			select(func.count()).select_from(Application).where(and_(Application.user_id == user_id, Application.status == "accepted"))
		)
		...
		return RateMetrics(interview_rate=round(interview_rate, 2), offer_rate=round(offer_rate, 2), acceptance_rate=round(acceptance_rate, 2))

	async def calculate_trend_for_period(
		self, user_id: int, current_start: date, current_end: date, previous_start: date, previous_end: date
	) -> TrendAnalysis:
		...
		current_result = await self.db.execute(
			select(func.count())
			.select_from(Application)
			.where(and_(Application.user_id == user_id, Application.applied_date >= current_start, Application.applied_date <= current_end))
		)
		...
		previous_result = await self.db.execute(
			select(func.count())
			.select_from(Application)
			.where(and_(Application.user_id == user_id, Application.applied_date >= previous_start, Application.applied_date < current_start))
		)
		...
		return TrendAnalysis(
			direction=direction, percentage_change=round(percentage_change, 2), current_value=current_value, previous_value=previous_value
		)
	...
	async def calculate_application_trends(self, user_id: int) -> ApplicationTrends:
		...
		daily_trend = await self.calculate_trend_for_period(
			user_id=user_id, current_start=today, current_end=today, previous_start=today - timedelta(days=1), previous_end=today - timedelta(days=1)
		)
		...
		weekly_trend = await self.calculate_trend_for_period(
			user_id=user_id, current_start=week_start, current_end=today, previous_start=last_week_start, previous_end=week_start - timedelta(days=1)
		)
		...
		return ApplicationTrends(daily=daily_trend, weekly=weekly_trend, monthly=monthly_trend)
	...
	async def get_top_skills_in_jobs(self, user_id: int, limit: int = 10) -> List[SkillData]:
		...
		result = await self.db.execute(select(Job.tech_stack).where(Job.user_id == user_id))
		...
		for skill, count in skill_counts.most_common(limit):
			...
			top_skills.append(SkillData(skill=skill, count=count, percentage=round(percentage, 2)))
	...
	async def get_comprehensive_summary(self, user_id: int, analysis_period_days: int = 90) -> ComprehensiveAnalyticsSummary:
		"""Get comprehensive analytics summary with all metrics (cached)."""
		# Check cache
		if self.cache:
			cache_key = self.cache._make_key("comprehensive_summary", user_id, days=analysis_period_days)
		...
		# Basic counts
		total_jobs_result = await self.db.execute(select(func.count()).select_from(Job).where(Job.user_id == user_id))
		...
		weekly_result = await self.db.execute(
			select(func.count()).select_from(Application).where(and_(Application.user_id == user_id, Application.applied_date >= week_start))
		)
		weekly_applications = weekly_result.scalar() or 0

		month_start = today - timedelta(days=30)
		monthly_result = await self.db.execute(
			select(func.count()).select_from(Application).where(and_(Application.user_id == user_id, Application.applied_date >= month_start))
		)
		monthly_applications = monthly_result.scalar() or 0

		user_result = await self.db.execute(select(User.daily_application_goal).where(User.id == user_id))
	...
	async def get_time_series_data(self, user_id: int, start_date: date, end_date: date) -> List[TrendDataPoint]:
		...
		result = await self.db.execute(
			...
			.where(and_(Application.user_id == user_id, Application.applied_date >= start_date, Application.applied_date <= end_date))
	...
	async def analyze_skill_gaps(self, user_id: int) -> SkillGapAnalysis:
		...
		user_result = await self.db.execute(select(User.skills).where(User.id == user_id))
		user_data = user_result.first()
		...
		recommendations = [f"Consider learning {skill.skill} (appears in {skill.percentage}% of jobs)" for skill in missing_skills[:5]]
	...
	def process_user_funnel(self, user_id: int, days: int = 30) -> Dict[str, Any]:
		...
		try:
			...
			if hasattr(self.db, "execute"):  # Sync session
				total_viewed = (
					self.db.query(func.count(Application.id)).filter(and_(Application.user_id == user_id, Application.created_at >= cutoff)).scalar()
					or 0
				)
	...
	def generate_user_insights(self, user_id: int, days: int = 30) -> Dict[str, Any]:
		...
		try:
			...
			if hasattr(self.db, "query"):  # Sync session
				...
				interviews = (
					self.db.query(InterviewSession).filter(and_(InterviewSession.user_id == user_id, InterviewSession.scheduled_at >= cutoff)).all()
				)
	...
	async def get_summary_metrics(self, user: User) -> Dict[str, Any]:
		...
		"""
		total_jobs_result = await self.db.execute(select(func.count()).select_from(Job).where(Job.user_id == user.id))
		total_jobs = total_jobs_result.scalar() or 0

		total_apps_result = await self.db.execute(select(func.count()).select_from(Application).where(Application.user_id == user.id))
		total_applications = total_apps_result.scalar() or 0

		pending_result = await self.db.execute(
			select(func.count()).select_from(Application).where(Application.user_id == user.id, Application.status == "pending")
		)
		pending = pending_result.scalar() or 0

		interviews_result = await self.db.execute(
			select(func.count()).select_from(Application).where(Application.user_id == user.id, Application.status == "interview")
		)
		interviews = interviews_result.scalar() or 0

		offers_result = await self.db.execute(
			select(func.count()).select_from(Application).where(Application.user_id == user.id, Application.status == "offer")
		)
		offers = offers_result.scalar() or 0

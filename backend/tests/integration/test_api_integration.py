...
def create_test_user(db: Session, username: str = "testuser", email: str = "test@example.com", password: str = "pass", skills: list | None = None) -> User:
	user = User(username=username, email=email, hashed_password=password, skills=skills or ["Python", "FastAPI"])
	db.add(user)
	db.commit()
	db.refresh(user)
	return user

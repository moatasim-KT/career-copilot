...
async def test_morning_briefing_complete_flow(notification_system):
    ...
    with patch.multiple(
        ...
        briefing_data = {
            ...
            "skill_recommendations": ["Python", "React"]
        }
...
async def test_job_match_notification_pipeline(notification_system):
    ...
    with patch.multiple(
        ...
        test_user = {
            ...
            "email": "test@example.com",
            "preferences": {
                "job_match_threshold": 0.8,
                "notification_channels": ["email", "in_app"]
            }
        }
        
        # Trigger job match notification
        await manager.notify_job_match(
            user_id=test_user["id"],
            email=test_user["email"],
            job_data=test_job,
            match_score=test_job["match_score"]
        )
...
async def test_notification_delivery_tracking(notification_system):
    ...
    with patch.multiple(
        'app.services.notification_manager.NotificationManager',
        _send_email_notification=AsyncMock(return_value={
            "success": True,
            "message_id": "email123"
        }),
        _send_in_app_notification=AsyncMock(return_value={
            "success": True,
            "notification_id": "inapp123"
        }),
        ...
        assert inapp_status["status"] == "delivered"        assert inapp_status["status"] == "delivered"
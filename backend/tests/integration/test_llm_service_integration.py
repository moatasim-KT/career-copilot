...
from app.services.llm_service import LLMService, ModelType, get_llm_service
...
def mock_external_dependencies():
	...
	with (
		patch("app.services.llm_service.get_settings") as mock_settings,
		...
		yield {"settings": settings, "cache": cache_service, "complexity": complexity_analyzer}
...
def integrated_llm_service(mock_external_dependencies):
	...
	mock_response.response_metadata = {"token_usage": {"prompt_tokens": 15, "completion_tokens": 25, "total_tokens": 40}}
...
def temp_config_for_integration():
	...
	config_data = {
		...
		"task_routing": {"contract_analysis": ["test-openai-gpt4", "test-openai-gpt35"], "general": ["test-openai-gpt35", "test-openai-gpt4"]},
...
class TestLLMServiceIntegration:
	...
	async def test_multiple_model_types_integration(self, integrated_llm_service):
		...
		for model_type, prompt in test_cases:
			result = await integrated_llm_service.analyze_with_fallback(model_type=model_type, prompt=prompt)
	...
	async def test_error_handling_and_fallback_integration(self, integrated_llm_service):
		...
		result = await integrated_llm_service.analyze_with_fallback(model_type=ModelType.GENERAL, prompt="Test fallback mechanism")
	...
	async def test_caching_integration(self, integrated_llm_service, mock_external_dependencies):
		...
		result1 = await integrated_llm_service.analyze_with_fallback(model_type=ModelType.CONTRACT_ANALYSIS, prompt=prompt)
		...
		result2 = await integrated_llm_service.analyze_with_fallback(model_type=ModelType.CONTRACT_ANALYSIS, prompt=prompt)
	...
	async def test_legacy_compatibility_integration(self, integrated_llm_service):
		...
		result = await integrated_llm_service.generate_response(
			"Generate a summary of contract terms", model_type=ModelType.CONTRACT_ANALYSIS, context="You are a legal assistant"
		)
...
class TestLLMOperationsManagerIntegration:
	...
	async def test_cache_operations_integration(self, integrated_operations_manager):
		...
		model = "gpt-3.5-turbo"
		response = {"content": "This is a comprehensive response for integration testing", "success": True}

		# Test caching
		cache_result = await integrated_operations_manager.cache_response(
			messages=messages, model=model, response=response, temperature=0.1, task_type="contract_analysis"
		)
		...
		# Test retrieval
		cached_response = await integrated_operations_manager.get_cached_response(messages=messages, model=model, temperature=0.1)
	...
	async def test_benchmark_integration(self, integrated_operations_manager):
		...
		benchmark_result = await integrated_operations_manager.run_benchmark(
			provider_func=mock_provider_func, provider_name="test-provider", model="test-model", tests=test_subset
		)
...
class TestServiceInteroperability:
	...
	async def test_service_and_manager_integration(self, integrated_llm_service, integrated_operations_manager):
		...
		result = await integrated_llm_service.analyze_with_fallback(
			model_type=ModelType.CONTRACT_ANALYSIS, prompt="Test interoperability between services"
		)
		...
		cache_result = await integrated_operations_manager.cache_response(
			messages=messages, model=result.model_used, response={"content": result.content, "success": True}
		)
	...
	async def test_performance_under_load(self, integrated_llm_service):
		...
		for i in range(5):  # Reduced from 10 for faster testing
			task = integrated_llm_service.analyze_with_fallback(
				model_type=ModelType.GENERAL, prompt=f"Test concurrent request {i}", user_id=f"user_{i}"
			)
			tasks.append(task)

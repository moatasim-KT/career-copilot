...
async def test_notify_interview_reminder(async_session, test_user, test_job):
	...
	interview_date = utc_now() + timedelta(hours=2)
...
async def test_cleanup_expired_notifications(async_session, test_user):
	...
	result = await async_session.execute(select(Notification).where(Notification.id == expired_notification.id))
	assert result.scalar_one_or_none() is None

	# Verify active notification still exists
	result = await async_session.execute(select(Notification).where(Notification.id == active_notification.id))

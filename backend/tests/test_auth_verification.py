...
def test_jwt_token_generation():
	...
	assert 1 <= time_diff_hours <= 48, f"Token expiration should be reasonable (1-48 hours), got {time_diff_hours:.2f} hours"
...
def test_jwt_token_validation():
	...
	wrong_key_token = jwt.encode(
		{"sub": username, "user_id": user_id, "exp": datetime.utcnow() + timedelta(hours=1)}, "wrong_secret_key", algorithm=settings.jwt_algorithm
	)

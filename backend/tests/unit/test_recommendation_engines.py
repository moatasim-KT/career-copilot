...
class TestAdaptiveRecommendationEngine:
	def test_get_user_algorithm_variant(self, adaptive_recommendation_engine, mock_user):
		...
		with patch.object(
			adaptive_recommendation_engine, "ab_test_configs", {"test_ab": {"active": True, "traffic_split": 0.5, "variant_a": {}, "variant_b": {}}}
		):
	...
	def test_get_algorithm_weights_ab_test_active(self, adaptive_recommendation_engine, mock_user, mocker):
		...
		mocker.patch(
			"app.services.adaptive_recommendation_engine.hashlib.md5",
			return_value=mocker.MagicMock(hexdigest=mocker.MagicMock(return_value="fedcba0987654321fedcba0987654321")),
		)  # Variant B
		weights = adaptive_recommendation_engine.get_algorithm_weights(mock_user.id)

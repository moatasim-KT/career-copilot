...
async def test_analyze_with_fallback_success(ai_service_manager):
	...
	result = await ai_service_manager.analyze_with_fallback(model_type=ModelType.GENERAL, prompt="test prompt", user_id=1)
...
async def test_analyze_with_fallback_all_fail(ai_service_manager):
	...
	with pytest.raises(Exception, match="All AI models failed"):
		await ai_service_manager.analyze_with_fallback(model_type=ModelType.GENERAL, prompt="test prompt", user_id=1)

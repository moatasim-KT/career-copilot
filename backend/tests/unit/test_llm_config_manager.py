...
from app.services.llm_config_manager import (
    LLMOperationsManager, ProviderConfig, LLMManagerConfig, BenchmarkTest,
    BenchmarkResult, ProviderBenchmark, CacheEntry, SemanticSimilarityMatcher,
    CacheOptimizer, QualityEvaluator, BenchmarkTestSuite,
    LLMProvider, TaskType, RoutingCriteria, get_llm_operations_manager
)
...
def llm_operations_manager(temp_config_file, mock_settings):
    ...
    with patch('app.services.llm_config_manager.get_settings', return_value=mock_settings), \
...
class TestLLMOperationsManager:
    ...
    async def test_cache_response_success(self, llm_operations_manager):
        ...
        result = await llm_operations_manager.cache_response(
            messages=messages,
            model=model,
            response=response,
            temperature=0.1
...
class TestCacheOptimizer:
    ...
    def test_should_cache_response_valid(self):
        ...
        optimizer = CacheOptimizer()
        response = {"content": "This is a comprehensive response that is definitely long enough to be cached by the system and meets all requirements", "success": True}
...
class TestQualityEvaluator:
    ...
    def test_evaluate_response_overall(self):
        ...
        test = BenchmarkTest(
            ...
            expected_keywords=["analysis", "important"]
        )
...
def test_get_llm_operations_manager_singleton():
    ...
    with patch('app.services.llm_config_manager.get_settings'), \
         patch('app.services.llm_config_manager.get_cache_service'):

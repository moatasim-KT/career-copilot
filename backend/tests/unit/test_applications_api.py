...
def test_list_applications(client: TestClient, db: Session):
	...
	application = Application(user_id=user.id, job_id=job.id, status="applied", notes="Test application")
	db.add(application)
	db.commit()
...
def test_update_application_status_to_applied(client: TestClient, db: Session):
	...
	application = Application(user_id=user.id, job_id=job.id, status="interested", notes="Test application")
	db.add(application)
	db.commit()

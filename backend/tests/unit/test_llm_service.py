...
from app.services.llm_service import (
    LLMService, ModelType, ModelProvider, ModelConfig, AIResponse,
    ErrorClassifier, CircuitBreaker, ErrorCategory, ErrorSeverity,
    LLMError, LLMRateLimitError, get_llm_service
)
    ...
    async def test_analyze_with_fallback_with_context(self, llm_service):
        ...
        with patch.object(llm_service, '_check_cache', return_value=None):
            result = await llm_service.analyze_with_fallback(
                ...
                context="You are a legal expert"
            )
    ...
    async def test_analyze_with_fallback_no_models(self, llm_service):
        ...
        with patch.object(llm_service, '_check_cache', return_value=None):
            with pytest.raises(ValueError, match="No models available"):
                await llm_service.analyze_with_fallback(
                    model_type=ModelType.GENERAL,
                    prompt="Test prompt"
                )

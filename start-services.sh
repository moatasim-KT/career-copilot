...
cd backend
python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload > ../data/logs/backend.log 2>&1 &
BACKEND_PID=$!
cd ..
...
if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
    ...
    echo "Check data/logs/backend.log for errors"
    exit 1
fi
...
cd backend
celery -A app.core.celery_app worker --loglevel=info --concurrency=2 > ../data/logs/celery.log 2>&1 &
CELERY_PID=$!
cd ..
echo -e "${GREEN}✅ Celery worker started (PID: $CELERY_PID)${NC}"
echo "   Logs: tail -f data/logs/celery.log"
...
echo "   Logs: tail -f data/logs/frontend.log"
...
echo "   Celery:   tail -f data/logs/celery.log"

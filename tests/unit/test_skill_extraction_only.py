...
class SimpleSkillAnalyzer:
	...
	def get_skill_frequency_stats(self, jobs: List[Dict]) -> Dict:
		...
		skill_percentages = {}
		for skill, count in skill_frequency.items():
			skill_percentages[skill] = {"count": count, "percentage": round((count / total_jobs * 100), 2) if total_jobs > 0 else 0}
...
def test_skill_frequency_stats():
	...
	jobs = [
		...
		{"job_id": "job2", "title": "Python Engineer", "tech_stack": ["Python", "Flask", "MySQL"], "requirements": "Python and Flask experience"},
		{
			"job_id": "job3",
			"title": "Full Stack Developer",

...
from app.services.job_scraping_service import JobCreate, JobScrapingService
...
def mock_user():
	...
	user.id = 1
	user.profile = {"skills": ["Python", "FastAPI"], "preferences": {"job_titles": ["Software Engineer"]}, "locations": ["Remote"]}
	return user
...
class TestJobScrapingService:
	...
	async def test_scrape_jobs_success(self, job_scraping_service, mock_job_create_data):
		with (
			patch.object(
				job_scraping_service, "apis", {"adzuna": {"enabled": True, "base_url": "http://mock.adzuna", "app_id": "id", "app_key": "key"}}
			),
	...
	async def test_ingest_jobs_for_user_success(self, job_scraping_service, mock_db_session, mock_user, mock_job_create_data, mock_job):
		...
		job_scraping_service._extract_search_params = MagicMock(return_value={"keywords": ["Python"], "locations": ["Remote"]})
		...
		with (
			patch.object(job_scraping_service, "db", mock_db_session),
	...
	async def test_get_ingestion_stats_success(self, job_scraping_service, mock_db_session, mock_user, mock_job):
		...
		with (
			patch.object(job_scraping_service, "_get_scraper_manager") as mock_get_scraper_manager,

...
def job_recommendation_service(mock_db_session):
    with patch('app.services.recommendation_engine.RecommendationEngine') as MockRecommendationEngine,
...
def mock_user():
    ...
    user.profile = {"skills": ["Python"], "experience_level": "mid", "locations": ["Remote"], "career_goals": [], "preferences": {}, "salary_expectations": {}}
    return user
...
def mock_feedback_create_data():
    return JobRecommendationFeedbackCreate(
        job_id=1,
        is_helpful=True,
        comment="Great job!"
    )
...
class TestJobRecommendationService:
    ...
    async def test_parse_job_description_success(self, job_recommendation_service):
        ...
        with patch.object(job_recommendation_service, '_scrape_job_description', return_value=description_text),
             patch.object(job_recommendation_service, '_parse_with_llm', return_value={
                 "tech_stack": ["Python", "FastAPI"], "experience_level": "mid"}),
    ...
    async def test_get_personalized_recommendations_success(self, job_recommendation_service, mock_db_session, mock_user, mock_job):
        ...
        job_recommendation_service.generate_enhanced_recommendation = MagicMock(return_value={
            "overall_score": 0.8, "confidence": 0.9, "score_breakdown": {"market_timing": {"score": 0.9}}})

...
async def test_recommendation_api_client():
	...
	with patch("app.clients.recommendation_api.RecommendationAPIClient.get_recommendations") as mock_api:
...
@pytest.mark.asyncio
async def test_recommendation_relevance(recommendation_service, test_user_profile, job_service):
	...
	# Get recommendations
	recommendations = await recommendation_service.get_recommendations_for_user(user_id=test_user_profile.user_id, limit=10)
	...
	high_relevance_score = next((rec.score for rec in recommendations if rec.job_id == high_relevance_job.id), 0)
	low_relevance_score = next((rec.score for rec in recommendations if rec.job_id == low_relevance_job.id), 0)
...
async def test_recommendation_filters(recommendation_service, test_user_profile):
	...
	recommendations = await recommendation_service.get_recommendations_for_user(user_id=test_user_profile.user_id, limit=50)
	...
	min_salary, _max_salary = map(int, test_user_profile.salary_expectation.split("-"))
	for rec in recommendations:
		_job_min, job_max = map(int, rec.job.salary_range.split("-"))
...
async def test_recommendation_explanation(recommendation_service, test_user_profile):
	...
	recommendations = await recommendation_service.get_recommendations_for_user(user_id=test_user_profile.user_id, limit=5, include_explanation=True)
	...
	for rec in recommendations:
		...
		if score_components:
			...
			assert abs(total_score - rec.score) < 0.01  # Allow for small floating point differences

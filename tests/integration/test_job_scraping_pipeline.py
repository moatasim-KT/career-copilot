...
async def test_job_scraping_task_trigger():
	...
	with patch("app.tasks.job_scraping_tasks.scrape_jobs_for_user_async.delay") as mock_task:
		# Configure mock
...
async def test_job_data_quality_validation(job_service):
	...
	with patch("app.services.job_scraper.JobScraper.scrape_jobs") as mock_scrape:
		...
		for job in stored_jobs:
			...
			assert job.posted_date <= current_time
			assert job.posted_date >= current_time - timedelta(days=30)
...
async def test_duplicate_job_handling(job_service):
	...
	with patch("app.services.job_scraper.JobScraper.scrape_jobs") as mock_scrape:
		...
		assert len(stored_urls) == len(set(stored_urls))  # No duplicate URLs
